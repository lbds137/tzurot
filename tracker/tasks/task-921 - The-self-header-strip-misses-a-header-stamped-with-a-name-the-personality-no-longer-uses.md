...
Fourth member, from the beta.221 pre-release audit (2026-09-09): the matcher ports exactly one of the operations sanitizeHeaderName applies to the rendered name (the trailing trim). The render side also strips invisible formatting, folds fullwidth confusables, maps brackets to parens, collapses newlines to a space, deletes forged id tags, neutralises the dash class, and collapses runs of spaces; the comparand gets none of that, so every one of those transforms makes the rendered name diverge from the raw roster name and every divergence is a silent miss (headerLinesStripped stays 0). Probed: a roster name with a double space renders collapsed and does not match; a roster name with a bracket renders as a paren and does not match. This family is larger than the three above and the stated fix shape dissolves it the same way. The separate delete-direction defect in the same matcher (a first line of prose ending in a self-named aside is deleted whole) is its own high task rather than a member here, because this task owns the miss direction only.
