...
Second member (claude-review on PR #2491): services/api-gateway/src/routes/user/conversationLookup.ts (handleLookupPersonalityFromMessage) is mounted only at /api/internal/conversation/message-personality (mounts.ts, mountInternalRoutes) and wrapped with internalRoutes.lookupPersonalityFromMessage, yet lives under routes/user/. Move it to routes/internal/ in the same pass.
Acceptance: no handler under routes/admin/ or routes/user/ is mounted under a non-admin audience (compare mounts.ts); pnpm ops codegen:routes --check is up to date.
