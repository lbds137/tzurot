...
CLASS SWEEP (owner-requested, payload + code verified): other restructure-confusion candidates checked.
CLEAN, no action: participants block instruction is flag-aware (names the [Name - timestamp] headers); the current message is bound by the <from id/pronouns> marker plus the "respond to the current message" constraint; zero scaffolding references to chat_log render flag-on (the 21 "chat log" strings in the payload are vision-description content plus two personality-authored directives); the quote legend's "appears earlier in the conversation" wording is mode-neutral.
SCOPE NOTE for (c): the class is half-fixed already - usableLocationContext (services/ai-worker/src/services/prompt/storedReference.ts:226) filters the legacy "This conversation is taking place" markdown on the stored-REFERENCE path; the memory-CONTENT path is the unswept sibling. No live producer writes the string (only the filter and a doc comment reference it), so (c) is render-time-only; consider stripping the whole legacy location span from memory text rather than tense-shifting it.
WATCH, not building: flag-on has no deep-history/live-conversation boundary (flag-off's <current_conversation> semantics) - the first history message can be a month old and renders as a fresh user turn, mitigated by per-message timestamps + time-gap markers + the datetime block. If prod shows temporal confusion (treating month-old exchanges as just-said), this is the suspect. Also: the personality's own directives say "chat log" twice - owner-authored prose, mildly anachronistic flag-on, owner's call.
