...
Why it was deferred rather than fixed inline: breadth, not merit. fromStoredReference is already a four-parameter positional function and its own callers sit in jobs/utils/xmlMetadataFormatters.ts at lines 96 and 110, which would need the same threading, and so on upward. Threading it properly wants an options-object refactor of fromStoredReference (it reaches the max-params ceiling otherwise), which is a larger and independently reviewable change than the turn-header fix it would have ridden along with.
