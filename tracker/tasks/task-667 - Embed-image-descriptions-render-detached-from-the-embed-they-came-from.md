...
2. A URL join is NOT available. extractEmbedImages prefers embed.image.proxyURL (embedImageExtractor.ts:30, deliberately, to satisfy the CDN allowlist), while EmbedParser emits the ORIGINAL embed.image.url (EmbedParser.ts:104). For any externally-hosted embed those differ, so the two halves share no key at all today except the filename. Worth its own look: the embed XML hands the model a URL our own allowlist would reject.
