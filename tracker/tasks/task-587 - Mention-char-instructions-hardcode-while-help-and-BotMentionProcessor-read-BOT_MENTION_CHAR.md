...
PREMISE VERIFIED 2026-09-08 (Railway variables, bot-client, key value read directly): development sets BOT_MENTION_CHAR to the ampersand, production keeps the at-sign default. So the bug is LIVE on dev — the NSFW-verification and DM-session instructions on dev tell users to type a sigil that does not work there — and latent on prod. Priority stays medium (dev-only exposure); the fix shape above stands.
