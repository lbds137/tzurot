...
🔷 OWNER RULING 2026-09-12 — the per-user-GLOBAL vs per-user-per-CHARACTER question listed above as open is CLOSED: it is a tiered cascade field, both and neither, resolved the way the existing settings cascade already resolves. Full ruling and reasoning in doc-26 § "User System Prompts (Sidecar Prompts)".

This shrinks this task rather than advancing it. The layering designed here (platform → channel → personality → user-overrides) is nearly the same tier set the config cascade already resolves (user-per-personality → user-default → personality-default → global-default), which doc-15's accepted design extends with a guild tier. Two independently-designed cascades, in docs that do not cite each other. If sidecar lands as a cascade field, the layered-composition engine largely dissolves into doc-15's work and what remains HERE is the channel layer — which was trigger-gated already. Re-scope against doc-15 before treating this as an architecture build.

The question that replaces the closed one: a config cascade SELECTS ONE winner, while a prompt layer may want to ACCUMULATE (platform and character and user text simultaneously). Select-one vs concatenate is now the open design fork; its home is doc-15 item 2, alongside the off-vs-inherit semantics work.

The caching tension recorded above also wants re-measuring before it is treated as a constraint. It cites a 0.62 prod cache-hit rate. A debug payload from a long conversation on 2026-09-12 read 768 cached of 134,653 prompt tokens for that turn — under one percent. One sample, one model, and TASK-651 already names S1 participants churn as the dominant miss, so this is a measurement order rather than a finding. But if long conversations routinely cache near zero, the objection to placing user overrides in S1 is protecting a prefix cache that is not working on the conversations that matter most.
