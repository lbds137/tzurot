...
Second member, the same class on the params side (claude-review round 3 on PR #2483): resolveCascade, resolveChannelCascade, setPersonalityDefaultConfig, clearPersonalityDefaultConfig and deleteModelOverride declare a manifest params schema that withManifestInput parses, but their handlers still read req.params through getValidatedPersonalityId / getParam, so the parsed params are unused. Today every such schema is a bare z.string() and can never reject, so this is latent; a future .uuid() on one would silently do nothing. Fix: read the parsed params in the wrapped handlers. The shared helpers are also called by unwrapped routes, so change the callers, not the helpers' contract.
Acceptance: no user route handler reads req.query directly (git grep req.query under services/api-gateway/src/routes/user returns only tests), PENDING is empty, and no withManifestInput-wrapped handler reads req.params.
