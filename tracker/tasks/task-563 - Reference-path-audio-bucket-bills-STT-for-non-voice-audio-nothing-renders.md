...
THIRD CORRECTION 2026-09-24: the two paragraphs above are stale, and there is nothing left to resume. The fix shipped on develop as f5ec6715c (#2176, 2026-08-21), together with the fixture strengthening the blocker called for. `git diff f5ec6715c 673d372dc` over jobChainOrchestrator.ts and its .test.ts is empty. The only difference is the producer contract test, where the shipped commit has the strengthened fixtures and the parked commit does not. So the local branch had no unique content. With the owner's OK it was deleted the same day; it was at 673d372dc, which stays in the reflog for about 90 days. The patch-id flag of 2026-08-26 was expected: the shipped commit also carries the fixture changes, so its patch id differs.
