...
---
Pass 2026-09-04 (TASK-888 half 1, priority-low digest): KEEP. `globalConfigCache` in `preset/autocomplete.ts` still has its own dedicated 30s TTL (`GLOBAL_CONFIG_CACHE_TTL_MS`) and bot-client still does not subscribe to `LlmConfigCacheInvalidationService` anywhere — the interim fix (30s bound) stands, the proper pub/sub fix hasn't landed. Promote-when (the 30s window still bites the owner, or next preset-autocomplete touch) hasn't been reported as fired. Evidence: `sed -n '1,60p' preset/autocomplete.ts` shows the 30s TTL cache; `git grep -rn LlmConfigCacheInvalidationService -- services/bot-client/src` → zero hits.
---
<!-- COMMENTS:END -->
