...
RUN IN PROGRESS 2026-09-16 — A ROTATION WINDOW IS OPEN ON DEV. The owner granted `Bash(pnpm ops secrets:rotate-env --env dev *)` and stage 1 ran at 00:07Z: `Deploy gate: PASS`, `_PREVIOUS` written at the verifier's service scope, a fresh primary minted, api-gateway redeployed (boot line `Server listening` at 00:07:35Z), no value in stdout. **The read-back gate passed on its first live exercise** — the check whose absence caused the 2026-09-14 outage. STAGES 2 AND 3 ARE STILL PENDING: presenters (bot-client, ai-worker) still hold the OUTGOING value, which the gateway accepts as `_PREVIOUS`, so the window is safe but must be closed. If this session died here, resume by running stage 2 then stage 3 with the commands above; do NOT re-run stage 1, which would demote the current primary and strand the original.

OBSERVABILITY, learned the hard way this run: `pnpm ops logs` emits `TIMESTAMP [LEVEL] Message key=value …`, NOT pino JSON. Greps for `"msg":` or `"path":` match nothing and read as a clean zero — several of this run's early counts were vacuous for that reason. Grep the MESSAGE TEXT instead, and positive-control every pattern. The strongest acceptance signal is on the PRESENTER side, not the absence of a gateway failure line: ai-worker logs `Triggering release reconcile sweep` → `Release reconcile summary received` → `Job release-reconcile completed` on success, and would throw `Release reconcile trigger failed: HTTP 401` on a rejected secret. Pre-rotation positive control: that full success triple is present at 2026-09-15T23:41:09Z. Also confirmed in passing: #2417's redaction is live on dev — the one gateway line carrying the header reads `x-service-auth":"[REDACTED]`, with zero hex-value forms.
