...
- eslint-on-edit: decide between dropping it and keeping a 4.8s-per-edit no-op. lint-staged and CI already enforce it. DECIDED 2026-09-16 (owner): dropped — the hook file and its registry row were deleted in the session-mining operationalizations PR, and `guard:hook-probes` now fails on any registered hook that nothing invokes.
