...
(b) packages/tooling/src/eslint/ custom plugin -- 8 rules there already, each with a colocated test (see component-handler-ack-first.ts). Heavier but unit-testable, and authoring-time enforcement is lint-staged at pre-commit plus the CI lint job (there is no lint-on-edit hook).
