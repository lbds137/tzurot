...
---
Pass 2026-09-04 (TASK-888 half 1, priority-low digest): KEEP. No `satisfies` constraint or `*.test-d.ts` witness type exists between `PgvectorMemoryDocument` and the RAG-layer `MemoryDocument` — the implicit structural-widening assignment the task flags is unchanged. Evidence: `git grep -n "PgvectorMemoryDocument" services/ai-worker` → only usages/tests, no `satisfies` guard found.
---
<!-- COMMENTS:END -->
