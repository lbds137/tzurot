...
Member RESOLVED 2026-08-28 (PR #2245, the ops-alert-embed enrichment): services/bot-client/src/observability/ErrorChannelReporter.ts was on NEITHER list here - not a member, not verified-safe - so it was simply unswept. Its buildEmbed now routes every dynamic field (Personality, Model, Provider, and the pre-existing Command and Request ID) through cappedInlineField; Duration and Occurrence are computed strings with bounded shapes and stay literal. Worth noting for the sweep because this site has the worst failure mode of any of them: the throw lands in reportError own fail-open catch, so the report is silently dropped, and this module is how a failure becomes visible at all. When enumerating the remaining sites, check for other build sites sitting inside a swallowing try/catch - a clamp gap there costs the whole signal rather than one broken reply. Pinned by a test asserting an over-long wire-sourced value clamps to EMBED_CAPS.fieldValue with the shared ellipsis, canaried against a pass-through.
...
Member added from the #2331 round-2 review (2026-09-04): services/bot-client/src/commands/inspect/embed.ts carries two hand-rolled `substring(0, 200)` truncations ahead of toInertCodeSpan — the Error field's message and, since #2331, the finish-reason default arm (grep `substring(0, 200)` in that file). Both cut on UTF-16 code units, so a 200-boundary inside an astral character leaves an unpaired surrogate; cosmetic only (a replacement glyph, no throw — the same class as member (1) above, resolved for clampEmbedText in #2163). Swap both to clampEmbedText during the sweep; #2331 mirrored the sibling on purpose rather than introducing the helper into that file alone.
Member added from the browse-truncation PR round 3 review (2026-09-09): services/bot-client/src/commands/memory/detailModals.ts buildTruncationWarningEmbed cuts memory.content with substring(0, 200) into an embed description (grep substring(0, 200) in that file) — the same UTF-16 cut as the inspect/embed.ts pair above. Fix shape for every member of this class is now one line: the shared truncateToUtf16Units in packages/common-types/src/utils/codePointTruncation.ts (charged suffix, surrogate back-off, empty for a budget under the suffix) — the module that PR created. The (b) detailCard code-point-vs-UTF-16 mismatch above stays open and has the same fix shape.
<!-- SECTION:DESCRIPTION:END -->

Four more members from the beta.221 pre-release audit (2026-09-09), all the same UTF-16 display cut, cosmetic only: services/bot-client/src/commands/memory/detail.ts (grep: memory.content.substring(0, 200)), services/bot-client/src/commands/inspect/views.ts (grep: m.preview.substring(0, 100)), services/bot-client/src/commands/persona/view.ts (grep: contentText.substring(0, CONTENT_PREVIEW_LENGTH)), services/bot-client/src/utils/HistoryLinkResolver.ts (grep: substring(0, 497)). Enumeration command that found them: grep -rn for .substring(0, followed by a constant or literal across services/bot-client/src excluding tests. Fix shape unchanged: truncateToUtf16Units.
