...
What: periodically (or when a user reports a truncated/fragment reply) query Railway ai-worker logs for the WARN message "Suspect reasoning mis-channel" and confirm hits against the llm_diagnostic_logs row (7d retention as of 2026-08-26, was 24h — a user report now has a week to arrive before the corroborating row is gone). The predicate is family-scoped to GLM in services/ai-worker/src/utils/reasoningMischannel.ts.
