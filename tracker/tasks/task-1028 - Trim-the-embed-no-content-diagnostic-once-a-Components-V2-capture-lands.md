...
TRIGGER FIRED 2026-09-21: the prod capture landed (TASK-1024 carries it) and answered the question the other way — the tree is on `embed.components`, and `message.components` was `[]`, so the per-message component summary never had anything to show. Folded into the TASK-1024 step 2 unit, dispatched the same day: the `message?` parameter becomes `messageId?: string` (keeps the correlation key that made the capture findable, drops the tree walk), both live call sites pass `message.id`, the snapshot sites stay three-argument, the module header is rewritten as permanent observability. Log level DECISION: stays `warn` — once the renderer lands the marker fires only on genuinely content-free embeds, and the prod watch saw one occurrence in its whole window, so the per-message batching question is moot (nothing message-level is logged any more). Close with that PR — **PR #2461**, opened 2026-09-21.
