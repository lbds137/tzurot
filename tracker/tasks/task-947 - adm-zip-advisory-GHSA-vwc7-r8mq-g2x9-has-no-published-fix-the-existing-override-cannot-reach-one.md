...
CLOSED 2026-09-21 — the observation landed. beta.227 (PR #2460, main `56e36674d`) carried #2457's override to `main` at 00:34Z, and `pnpm ops security:advisories` run minutes later printed `✅ No open Dependabot advisories.` — both GHSA-7q85-xj36-vmfc and GHSA-vwc7-r8mq-g2x9 gone from the default-branch alert set. The acceptance line ("reports no open adm-zip advisory") is met as written. The two mechanism notes above stay as the record of why a green develop merge could not close this on its own.
