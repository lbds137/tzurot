...
Still unobserved and the last gate on step 2: the node shape INSIDE `embed.components` — whether the top level is `[Container]` or the page script's `{component: Container}` wrapper, and whether Discord adds `proxy_url` on the MediaGallery items (that decides whether the vision path can use the Discord CDN proxy the way `extractEmbedImages` does for legacy `image.proxyURL`, or must go through the external-image fallback for `i.redd.it`). A read-only REST probe (`GET /channels/1226207363342663781/messages/1550900659652403344` with the bot token, script in the session scratchpad) was blocked by the auto-mode prod-read classifier; the owner runs it by hand. Step 2 scope confirmed from the grounding: render TextDisplay/Section text and MediaGallery media inside `formatEmbedElement` (single hook, all four wrapper sites), AND extend `embedImageExtractor.extractEmbedImages` (`services/bot-client/src/utils/embedImageExtractor.ts`) to emit MediaGallery items as `isEmbedPreview` attachments so the vision pipeline describes the art.
