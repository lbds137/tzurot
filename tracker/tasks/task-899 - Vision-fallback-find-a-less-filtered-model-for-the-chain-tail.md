...
Candidate from owner observation 2026-09-21: Gemini Flash Lite. A community thread showed it describing hentai and recognizing characters and fandoms under a plain context prompt, and the owner reports the newer Qwen and GLM tiers refusing more often lately. Resolve the exact OpenRouter id and its moderated flag by live probe (the thread named it as the 3.5 generation; unverified here) and include it in the shortlist. The prompt-context half of the same complaint is TASK-1033, filed the same day; this task stays model choice only.
