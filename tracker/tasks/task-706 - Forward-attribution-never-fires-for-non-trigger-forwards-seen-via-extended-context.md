...
SHIPPED PENDING SMOKE — PR 2166, merged 2026-08-21. Do NOT rebuild any of this. Acceptance clauses 2 and 3 are met (the persistence path is untouched and green; the new path receives the same resolver, now built from one shared definition so the access semantics cannot drift between the two consumers). Clause 1 — renders author plus t= on subsequent turns — is code-complete and pinned at the seam by canaried unit tests, but has no runtime observation, which is exactly the gap that created this bug in the first place. The task stays open until the dev smoke in CURRENT.md's smoke queue passes; close it then.

What landed: a bounded parallel pre-pass primes a two-tier process-local cache before the conversion loop, which reads cache-only; mergeWithHistory heals a DB row's origin field by field (per-field, because a fetch failure persists a timestamp-only origin that a whole-object gate would strand forever); resolveForwardedOrigin falls back to the fetched original's createdAt when the snapshot carries none; and the resolver signature that was re-declared at seven call sites is now one exported type, ForwardedAuthorPersonalityResolver.
