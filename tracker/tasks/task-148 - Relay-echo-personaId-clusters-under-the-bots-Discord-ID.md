...
---
Pass 2026-09-04 (TASK-888 half 1, priority-low digest): KEEP. `DiscordChannelFetcher.convertMessage` still derives `personaId` as `INTERNAL_DISCORD_ID_PREFIX + msg.author.id` (the bot's id) for user-role messages, unchanged. Dependent trigger (a personaId-keyed feature needing relay echoes attributed to the real user) hasn't fired. Evidence: `git grep -n INTERNAL_DISCORD_ID_PREFIX -- DiscordChannelFetcher.ts` → line 490, same expression as described.
---
<!-- COMMENTS:END -->
