---
id: doc-26
title: 'Theme: User-Requested Features'
type: other
...
**Known dependency, unchanged**: Delete's memory propagation needs the accepted memory design's Phase-0 `messageIds` linkage. Regenerate needs `triggerMessageId` rows (Phase-0 FK) with a diagnostics fallback for pre-FK rows (that window is 7d as of 2026-08-26, was 24h — the fallback reaches back a week further than this line assumed). Neither is satisfied by a context-menu command alone — check both before scheduling.
...
- [ ] **🚩 Report Issue** (owner idea 2026-07-27, from the GLM `<action>`-tag leak report) — panel action (and/or `/inspect` button) that posts to the owner's audit-log channel: message link, character, reporter, optional modal note. **Load-bearing detail: reporting PINS the evidence** — snapshot or TTL-extend that message's diagnostic log at report time, so a user report beats the inspect window without changing global retention. **Motivation weakened but not removed as of 2026-08-26**: global retention went 24h → 7d, and the incident this cites (the tag-leak report, which arrived >24h late with the log already gone) would now be caught. A report arriving after a week still loses the log, so pinning keeps its value for the tail — it is just no longer the only thing standing between a late report and no evidence. Rate-limit per reporter.
