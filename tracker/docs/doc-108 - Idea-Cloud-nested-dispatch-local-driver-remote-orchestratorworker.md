...
**Lane routing (owner proposal, 2026-09-24, adopted as the default):** backlog-drain items go to the cloud lane, and feature and theme work stays in the local lane. Feature work needs owner rulings mid-build, dev deploys and smoke sittings, migrations applied to dev, and Railway logs, and it runs review rounds that come back to the local tree. Exception: a drain item needing Railway, prod data, real `.env` secrets, or a local-only tool (for example, the council MCP server) stays local. **Owner ruling, 2026-09-24 (the slot count):** while the local lane runs no gate-running unit, up to TWO cloud units may be in flight; otherwise it is one cloud unit plus one local unit. The one-cloud cap existed to keep two gate runs from competing for the Deck's RAM. The brake that remains is the driver's weekly usage for specs and diff reads. Fold both rules into `/tzurot-orchestration` § Cloud dispatch after #2512 merges. **Refinement, owner ruling 2026-09-24 (evening, Fable planning pass):** route by SHAPE, not label. A spec-precise sweep goes to the cloud lane even when it belongs to a feature epic (the first case: `doc-14` waves 4–6, the factory sweep and picker hygiene); design-heavy units (a council pass, a Phase 0 with owner rulings, anything needing a dev smoke sitting) stay local whatever their label. The exception list above is unchanged.
