...
## 2026-09-25 spike part 2 result (TASK-1108 closes; long form in the gitignored `docs/local/spacebar-probe.md`; Spacebar d211b0d run on the Deck against its own `spacebar-postgres` container on 5433, driven by a standalone discord.js 14.27 script)

- **Runtime verdict**: the server does more than the code-read predicted; the bigger gap is between discord.js's shapes and Spacebar's. PASS: gateway identify + reconnect (after `security.cdnSignatureIncludeUserAgent=false`, discord.js sends no User-Agent), voice messages via the cloud-upload flow. FAIL: command PUT with an `autocomplete` option (400, one bad option fails the whole deploy), the persona webhook path on the discord.js side (server returns 200 for file, thread, edit; discord.js throws on `install_params: null` and attachment `flags: null`), defer then editReply (`@original` and followUp 404 Unknown Webhook, as predicted), ephemeral (discord.js body rejected on `enforce_nonce`; raw ephemeral 500s), button round trip (discord.js crashes on `thread: null`), modal and autocomplete (types 8 and 9 rejected, autocomplete arrives without `data`). BLOCKED: the Fermo client (owner, five minutes in a browser, steps in the doc). SECURITY: an unauthenticated callback with a forged token posts as the bot (204).
- **First patches, in order** (the fork's first milestone): (1) response shapes discord.js can parse (`install_params`, attachment `flags`, `thread`); (2) optional `attachments[].filename`; (3) accept all Discord command-option fields, `autocomplete` first; (4) the interaction-token webhook routes (`@original`, followUp) plus the deferred-reply placeholder; (5) callback schema and handlers: `enforce_nonce`, type 6 without data, content on type 7, types 8 and 9, forward `data` on autocomplete; (6) the ephemeral `Or()` crash; (7) token check on the callback route, before any instance is reachable by others; (8) `member.permissions` on the interaction member; (9) real gateway resume (discord.js already recovers by re-identifying).
- **Beyond the spec, recorded**: two of upstream's four npm patches fail under `npm ci` at d211b0d and were hand-applied; a file config (`CONFIG_PATH`) with five endpoint keys; a forged-token probe and a second user for a leak check. `spacebar-postgres` stays up for later probes (teardown commands in the doc).
- **Tzurot-side work the fork needs** (from part 1): the Client's `rest` option, `utils/deployCommands.ts`'s bare `new REST()`, and the hard-coded CDN hosts in `discordCdnGuard.ts` and ai-worker's `attachmentFetch.ts`.
