...
The pass landed as #2373: the `LOG_CONTENT_PREVIEWS` flag, the shared `contentPreview` / `contentDigest` / `contentPreviewsEnabled` helper in common-types, thirteen sites gated (the eleven enumerated here plus two the structural sweep found — the typed memory-search query logged by the api-gateway route and by the bot-client command), always-on lengths everywhere, always-on hashes at the two duplication-diagnosis sites, and the deployed-service boot refusal for both local-only flags. TASK-593 Done; every member of this idea is shipped. The residue grep (`substring(0, N)` / `slice(0, N)` inside logger payloads) leaves only id prefixes, URL prefixes, token prefixes, and Zod issue slices — none is message content.
