...
id: doc-1
title: >-
  Theme: Adjacent CPD Follow-Up Campaigns (deferred from 2026-05-16 campaign
  close-out)
type: other
...
_Focus: four distinct DRY-extraction campaigns that the api-gateway CRUD-config campaign explicitly LEFT for separate council passes._

**THEME COMPLETE 2026-07-06** — all four campaigns resolved in one arc: 1 OBSOLETE-verified, 2 + 4 shipped (#1517/#1518), 3 shipped as three PRs (#1519/#1520/PR-3). `cpd:filtered` 1648 → 1367 across the arc; every extraction council-gated, every rejected abstraction documented with its reason, every surviving clone classified (import blocks, different-schema skeletons, structural uniformity — see the per-campaign rule-outs below). Item 5 (in-file local-helper sweep) remains opportunistic next-touch work by design, not a campaign. The classification detail below is retained as the audit record. (Original audit: [`docs/reference/CPD_CAMPAIGN_AUDIT.md`](../../docs/reference/CPD_CAMPAIGN_AUDIT.md).)
...
3. ✅ **🟡 bot-client command-pattern campaign — COMPLETE 2026-07-06 (all three PRs shipped same day the plan was locked)**
   ...
   - ✅ **PR 2 SHIPPED 2026-07-06: `character/overrides` ↔ `character/settings`** — the twins differed ONLY in configuration values (dashboard config, cascade endpoint, source tier, error noun); the whole open-dashboard flow was identical. `openCharacterCascadeDashboard(context, slug, spec)` in directory-local `characterDashboardShared.ts` (1 callback: which cascade endpoint backs the view). Both command suites pass unchanged; twin clones 4 → 1 (the remaining one is the import block). Ride-along for PR 3 or the next touch: widen the spec's `resolveCascade` failure variant to `{ ok: false; error?: unknown }` so the cascade-failure warn can carry the underlying error (reviewer nit, trigger: cascade failure needing log-only debugging).
   - ✅ **PR 3 SHIPPED 2026-07-06 — CAMPAIGN 3 AND THE THEME COMPLETE: truncationWarning glue** — the per-entity two-click edit-flow handlers left behind when `utils/dashboard/truncationGate/` (480 lines, tested) was extracted. Council RULING: a cohesive `EntitySectionAdapter<TData>` interface (entityType + findSection/loadSectionData/resolveSectionContext, generic-linked) is ONE seam per the TtsProvider precedent — NOT three callbacks over the ceiling; the cohesion test is that removing any method makes the rest meaningless. Constraint: adapter IMPLEMENTATIONS live entity-local (`character/`, `persona/`); the shared module gets only the interface + unified flow.
   ...
   - Top pairs: `character/overrides` ↔ `character/settings` (93 lines), `character/truncationWarning` ↔ `persona/truncationWarning` (88 lines), `commands/preset/override/autocomplete` ↔ `commands/voice/tts/autocomplete` (38 + 27 lines)
