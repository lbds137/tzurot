...
const MOCK_PATTERNS = [/jest\.mock\(['"`](.+?)['"`]/g, /jest\.doMock\(['"`](.+?)['"`]/g];
...
function checkTestFile(testFilePath) {
  ...
  MOCK_PATTERNS.forEach(pattern => {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (
        mockedModule.endsWith(moduleBeingTested) ||
      ...
      if (
        moduleBeingTested.includes(mockedModule) &&
  ...
  if (testFilePath.includes('/domain/')) {
    ...
    if (mockCount > 2) {
      // Allow logger and maybe one other
      issues.push({
        line: 0,
        severity: 'warning',
...
function main() {
  ...
  console.log(
    `  ${colors.yellow}⚠️  ${warningCount} warnings${colors.reset} (questionable mocking)\n`
  );
  ...
  Object.entries(fileIssues).forEach(([file, issues]) => {
    ...
    issues.forEach(issue => {
      ...
      if (issue.line) {
        console.log(
          `  ${color}${icon} Line ${issue.line}: ${issue.message || 'Mocking module under test'}${colors.reset}`
        );
