...
const TIMEOUT_ANTI_PATTERNS = [
  ...
  },
  {
    ...
    message:
      'Request tracker scheduling creates real timeouts. Ensure jest.useFakeTimers() is called.',
  },
];
...
function checkFile(filePath) {
  ...
  const usesFakeTimers = content.includes('useFakeTimers') || content.includes('setupFakeTimers');
  ...
  if (!usesFakeTimers) {
    const shouldUseFakeTimers = SHOULD_USE_FAKE_TIMERS.some(pattern => pattern.test(content));
