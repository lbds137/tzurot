...
describe('WebhookManager - Message Sending Tests', () => {
  ...
  it('should split long messages into chunks', async () => {
    ...
    expect(prepareAndSplitMessage).toHaveBeenCalledWith(longMessage, expect.any(Object), 'Webhook');
  ...
  it('should add embeds and attachments to the last chunk only', async () => {
    ...
    await webhookManager.sendWebhookMessage(mockChannel, longMessage, personality, embedOptions);
