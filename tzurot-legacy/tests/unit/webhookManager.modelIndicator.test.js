...
describe('WebhookManager - Model Indicator with Message Splitting', () => {
  ...
  it('should handle model indicator with multiple chunks correctly', async () => {
    ...
    await webhookManager.sendWebhookMessage(mockChannel, longMessage, mockPersonality, {
      modelIndicator,
    });
  ...
  it('should not add model indicator if not provided', async () => {
    ...
    expect(prepareAndSplitMessage).toHaveBeenCalledWith(message, expect.any(Object), 'Webhook');
  ...
  it('should handle edge case where message + indicator exactly equals 2000', async () => {
    ...
    await webhookManager.sendWebhookMessage(mockChannel, message, mockPersonality, {
      modelIndicator,
    });
