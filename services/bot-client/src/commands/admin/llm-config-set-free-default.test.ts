/**
 * Tests for Admin LLM Config Set Free Default Handler
 */
...
describe('handleLlmConfigSetFreeDefault', () => {
  ...
  it('should defer reply with ephemeral flag', async () => {
    ...
    await handleLlmConfigSetFreeDefault(mockInteraction);
  ...
  it('should call API with correct endpoint and method', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      'http://localhost:3000/admin/llm-config/config-id-123/set-free-default',
  ...
  it('should display success embed on successful update', async () => {
    vi.mocked(mockInteraction.options.getString).mockReturnValue('config-id');
    vi.mocked(fetch).mockResolvedValue(
      new Response(JSON.stringify({ configName: 'Free Tier Config' }), { status: 200 })
    );
