...
describe('buildShapeDetailEmbed', () => {
  ...
  it('should show slug in title and footer', async () => {
    stub.listShapesImportJobs.mockResolvedValue(makeOk({ jobs: [] }));
    stub.listShapesExportJobs.mockResolvedValue(makeOk({ jobs: [] }));

    const { embed } = await buildShapeDetailEmbed(asUserClient(stub), 'user-123', 'my-shape');
  ...
  it('should pass slug query param to gateway for server-side filtering', async () => {
    ...
    await buildShapeDetailEmbed(asUserClient(stub), 'user-123', 'my-shape');

    expect(stub.listShapesImportJobs).toHaveBeenCalledWith({ slug: 'my-shape' });
    expect(stub.listShapesExportJobs).toHaveBeenCalledWith({ slug: 'my-shape' });
  });
  ...
  it('should include action buttons in two rows', async () => {
    ...
    const { components } = await buildShapeDetailEmbed(asUserClient(stub), 'user-123', 'my-shape');
