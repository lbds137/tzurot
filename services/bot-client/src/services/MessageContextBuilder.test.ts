...
describe('MessageContextBuilder', () => {
  ...
  describe('buildContextFromInteraction', () => {
    it('should build context from interaction parameters', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue('user-uuid-123');
      vi.mocked(mockUserService.getUserTimezone).mockResolvedValue('America/New_York');
      vi.mocked(mockHistoryService.getRecentHistory).mockResolvedValue([
        {
          id: 'history-1',
          role: MessageRole.User,
      ...
      const result = await builder.buildContextFromInteraction(
        interactionParams,
        mockPersonality,
      ...
      expect(mockPersonaResolver.resolve).toHaveBeenCalledWith('user-123', 'personality-123');
      expect(mockUserService.getUserTimezone).toHaveBeenCalledWith('user-uuid-123');

      // Verify context structure
      expect(result.context).toMatchObject({
        userId: 'user-123',
        userInternalId: 'user-uuid-123',
        userName: 'testuser',
        discordUsername: 'testuser',
        userTimezone: 'America/New_York',
        channelId: 'channel-123',
        serverId: 'guild-123',
        messageContent: 'Hello from slash command',
        activePersonaId: 'persona-123',
        activePersonaName: 'Test Persona',
      });

      // Verify environment
      expect(result.context.environment).toMatchObject({
        type: 'guild',
        guild: { id: 'guild-123', name: 'Test Guild' },
        channel: { id: 'channel-123', name: 'general', type: 'GUILD_TEXT' },
      });
      ...
      expect(result.personaId).toBe('persona-123');
      expect(result.personaName).toBe('Test Persona');
      expect(result.messageContent).toBe('Hello from slash command');
      expect(result.referencedMessages).toEqual([]);
    });

    it('should throw error for bot users', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue(null);
      ...
      await expect(
        builder.buildContextFromInteraction(interactionParams, mockPersonality, 'Hello')
      ).rejects.toThrow('Cannot process messages from bots');
    });
    ...
    it('should handle DM context (no guild)', async () => {
      ...
      expect(result.context.environment).toMatchObject({
        type: 'dm',
        guild: undefined,
        channel: { id: 'dm-channel-123', name: 'DM', type: 'DM' },
      });
    });

    it('should apply context epoch filter when user has cleared history', async () => {
      const contextEpoch = new Date('2025-01-15T12:00:00Z');
      ...
      await builder.buildContextFromInteraction(interactionParams, mockPersonality, 'Hello');
    ...
    it('should use channel history when extended context is enabled', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue('user-uuid-123');
      vi.mocked(mockUserService.getUserTimezone).mockResolvedValue(null);
      vi.mocked(mockHistoryService.getChannelHistory).mockResolvedValue([
        {
      ...
      // Should use getChannelHistory instead of getRecentHistory
      expect(mockHistoryService.getChannelHistory).toHaveBeenCalledWith(
        'channel-123',
        30,
        undefined
      );
      expect(mockHistoryService.getRecentHistory).not.toHaveBeenCalled();

      expect(result.conversationHistory).toHaveLength(1);
      expect(result.conversationHistory[0].content).toBe('From other user');
    });

    it('should extract guild member info when member is provided', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue('user-uuid-123');
      vi.mocked(mockUserService.getUserTimezone).mockResolvedValue(null);
      vi.mocked(mockHistoryService.getRecentHistory).mockResolvedValue([]);
      ...
      const mockGuildMember = {
        ...
        roles: {
          cache: new Map([
            ['role-1', { id: 'role-1', name: 'Moderator', position: 10 }],
            ['role-2', { id: 'role-2', name: 'Member', position: 5 }],
            ['guild-123', { id: 'guild-123', name: '@everyone', position: 0 }], // Should be filtered
          ]),
        },
      ...
      expect(result.context.activePersonaGuildInfo).toMatchObject({
        roles: ['Moderator', 'Member'], // @everyone filtered, sorted by position
        displayColor: '#FF5500',
    ...
    it('should convert conversation history to API format correctly', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue('user-uuid-123');
      vi.mocked(mockUserService.getUserTimezone).mockResolvedValue(null);
      vi.mocked(mockHistoryService.getRecentHistory).mockResolvedValue([
        {
      ...
      expect(result.context.conversationHistory[1]).toMatchObject({
        id: 'history-2',
        role: MessageRole.Assistant,
        isForwarded: true,
      });
    ...
    it('should handle empty guild name gracefully', async () => {
      vi.mocked(mockUserService.getOrCreateUser).mockResolvedValue('user-uuid-123');
      vi.mocked(mockUserService.getUserTimezone).mockResolvedValue(null);
      vi.mocked(mockHistoryService.getRecentHistory).mockResolvedValue([]);
      ...
      // Should be treated as guild context but with no guild info
      expect(result.context.environment).toMatchObject({
        type: 'guild',
        guild: undefined, // Empty name treated as no guild info
        channel: { id: 'channel-123', name: 'general', type: 'GUILD_TEXT' },
      });
    });
  });
