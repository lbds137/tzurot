...
function makeRedis(cooldownValue: string | null): Redis & {
  ...
  } as unknown as Redis & { get: ReturnType<typeof vi.fn>; setex: ReturnType<typeof vi.fn> };
}
...
describe('runRetentionTick — production, kill switch ON (live mode)', () => {
  ...
  it('cooldown present: only redis.get is called', async () => {
