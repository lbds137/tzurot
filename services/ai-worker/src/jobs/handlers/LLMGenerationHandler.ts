...
export class LLMGenerationHandler {
  ...
  async processJob(job: Job<LLMGenerationJobData>): Promise<LLMGenerationResult> {
    ...
    try {
      ...
      // Success-path diagnostic store runs here, after TTSStep, so TTS attribution
      // lands in the saved log (error paths store inside GenerationStep).
      persistSuccessDiagnostic(this.prisma, context);
