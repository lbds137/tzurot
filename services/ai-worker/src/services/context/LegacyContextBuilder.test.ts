/**
 * Tests for LegacyContextBuilder
 *
 * These tests validate token budget fundamentals extracted from ContextWindowManager.
 * The buildContext flow is no longer used in production (ContentBudgetManager replaced it)
 * but the tests ensure token counting and history selection logic remains correct.
 */

import { describe, it, expect, vi } from 'vitest';
import { HumanMessage, AIMessage, SystemMessage } from '@langchain/core/messages';
import { buildContext, type ContextWindowInput } from './LegacyContextBuilder.js';
