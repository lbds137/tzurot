...
async def test_tts_returns_wav(client: httpx.AsyncClient, mock_tts: MagicMock) -> None:
    """Happy path: voice-engine returns raw WAV.

    Opus encoding is performed downstream in ai-worker's audioNormalizer
    (single ffmpeg pass: loudnorm + libopus + ogg muxer). Voice-engine is
    purely a synthesis service.
    """
