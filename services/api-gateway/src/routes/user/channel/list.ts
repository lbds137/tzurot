  ...
  const handler = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const response = {
      settings: settingsList.map(s => ({
        ...
        extendedContextMaxMessages: s.extendedContextMaxMessages,
        extendedContextMaxAge: s.extendedContextMaxAge,
        extendedContextMaxImages: s.extendedContextMaxImages,
