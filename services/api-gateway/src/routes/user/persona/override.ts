 ...
 *
 * Atomicity is via `prisma.$transaction`: if the upsert of the override
 * fails (concurrent write, etc.), the persona create rolls back too. The
 * prior bot-client implementation called a non-existent endpoint and would
 * have left no persona at all on failure — the transactional handler is
 * what makes the call shape honest.
 */
