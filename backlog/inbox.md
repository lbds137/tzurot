...
### `[FEAT]` Railway ops tooling — sanctioned token access + GraphQL ops the CLI lacks

**Surfaced 2026-06-17** during the beta.133 probe cleanup. Railway CLI v4.11.2's `variables` command only supports `--set` (merge) — **there is no unset/delete** (v4 dropped CLI-side variable deletion). The agent needed to remove 3 dead probe env vars from dev services and had no in-band path: the GraphQL API's `variableDelete` mutation can do it, but it needs the Railway auth token, and reading that token out of `~/.railway/config.json` was (correctly) blocked by the safety classifier as credential exploration. End result: the cleanup had to be hand-done in the dashboard. This will recur for any var deletion and for other CLI-missing operations.

**Two-part scope:**

1. **Sanctioned, auditable token access.** Define an explicit path for the ops tooling to obtain a Railway token that is obviously *intentional tooling*, not ad-hoc credential scraping — e.g. a dedicated `RAILWAY_API_TOKEN` (or project token) read from the environment that the user sets once, documented in `RAILWAY_CLI_REFERENCE.md`, never logged, used transiently per-call. The point is to make "the tool uses the token" legible to both the classifier and a human reviewer, vs. the tool grepping a credential file. (Open question: project-scoped token vs. account token — project tokens are narrower-blast-radius and a better fit if they can drive `variableDelete`.)
2. **Wrap the missing GraphQL operations as `pnpm ops` commands.** Per the "No Standalone Scripts" rule, add them to the tooling package (likely extending `packages/tooling/src/commands/deploy.ts` + a `dev/` impl module with colocated tests, mocking the HTTP layer). Start with `variableDelete` (the immediate gap); consider `variable:list`/`variable:set`/`variable:delete` for a complete CRUD surface, and survey what else the CLI can't do that we'd want (e.g. programmatic redeploy triggers, deployment status queries). Endpoint: `https://backboard.railway.app/graphql/v2`; `VariableDeleteInput` takes `projectId`/`environmentId`/`serviceId`/`name`. IDs are discoverable via `railway status --json` (project, env, per-service IDs).

**Security guardrails (carry into the implementation):** never log or echo the token; resolve it transiently and pass via `Authorization: Bearer` header only; fail-fast with a clear message if the token env var is unset (point the user at the doc). Treat the token like any other secret per `00-critical.md`.

**Why inbox:** real recurring gap, but needs a design decision (token-acquisition mechanism + token scope) before implementation — not a drop-in quick win. **Promote when:** the next time a Railway mutation is needed in-session, or as a deliberate dev-tooling pass.
