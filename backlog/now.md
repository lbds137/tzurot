...
- **In**: #2443 (TASK-996 register telemetry on the `Generated response` line — MERGED 2026-09-17) · #2444 (doc-97 Phase 4 slice 1a: digest table + index + settings pair + clear/purge hooks — MERGED 2026-09-17; two additive migrations, applied to dev, prod at the premigrate).
- **Waiting on**: ~~the doc-97 Phase 4 design pass~~ **ACCEPTED 2026-09-16** (`docs/proposals/backlog/recent-days-digest.md`) → ~~slice 0~~ **MERGED #2442 2026-09-16** (`user-only` render mode + `crossChannelMaxMessages` + TASK-994) → ~~the owner's three-arm register probe on dev~~ **READ 2026-09-17** (four arms; table in the design doc §6: the archive render switch is the register lever, `user-only` the mass lever; Emily now listed on dev's render switch) → ~~slice 1a~~ **MERGED #2444 2026-09-17** (table, settings pair, D10 a–c) → **slice 1b** (generator + sweep + validators + store + usage row + ops command; grounding pre-staged at `docs/local/handoffs/ground-doc97p4-slice1.md`, carried items in the design doc §6 slice 1 row); slice 2 (render + Emily flip) rides if the train has room; the number to beat is still ~4.4k completion tokens / 72 s per turn · **the doc-61 economy pass** over `.claude/rules` and `CURRENT.md` (rules had 2,652 B of headroom and `CURRENT.md` 132 B before this cut's reset; procedure `/tzurot-doc-audit` § 3b, ending with `cadence:mark economy-pass`) · the medium/S drain in same-area batches of 3–5 (TASK-958, 959, 968, 969, 960–962).
