...
- 🏗️ `[LIFT]` **Retrieval eval goldens — A/B DONE, verdict = pivot to query-side (1c)** — the Phase-1a resume gate (full context: `active-epic.md` §§ 1a/1c). Corpus mined LOCAL-ONLY (800/19,169 Lila rows, dev — storage call 2026-07-12: sensitive beyond entity swaps; committed = the #1608 miner + the #1609 pooling/scoring instrument). **A/B RAN 2026-07-12** (20 pooled queries, LLM-judge delegated by owner, `poolScoring`): RRF hybrid ≈ dense (recall@10 0.583 vs 0.571) — SECOND near-null → RRF-as-implemented doesn't clear the bar. The real finding: **30% of queries (6/20) had BOTH arms retrieve nothing relevant — vague/referential/compound messages**, a query-layer recall failure ranking can't fix. **Owner calls remaining**: (a) `feat/memory-hybrid-retrieval` disposition — keep-parked-as-FTS-input vs delete (recommend keep: FTS saved 2 dense-missed queries); (b) sequence **1c (query rewrite/context-fold)** as the next retrieval build. Local `reports/goldens-mining/AB-RESULT.md` = the before-baseline for 1c. Filed 2026-07-06; A/B closed 2026-07-12.
