...
- 🏗️ `[LIFT]` **Real-scale eval goldens → unpark hybrid retrieval** — the Phase 1a resume gate (full context: `active-epic.md` § Phase-1a park — the memory theme was promoted to Active Epic 2026-07-06). Build goldens from REAL prod memory data at real corpus scale (hundreds of rows/persona, where dense dilution actually bites), re-run the dense-vs-hybrid A/B, then merge `feat/memory-hybrid-retrieval` on demonstrated lift or delete it on a second null. **Storage REVISED 2026-07-12 (owner, on real-sample evidence — supersedes the 2026-07-11 committed-goldens call)**: the corpus is **LOCAL-ONLY, never committed** — reading the mined sample showed content sensitive beyond what entity swaps launder (identifying narratives, third-party accounts, ~24% medical/mental-health rows; the 07-11 "no third-party exposure" premise was false). Committed = the deterministic miner (#1608: `memory:mine-goldens` + `memory:anonymize-goldens`) + query goldens only. STATE: corpus mined (800/19,169 rows, Lila persona, dev) → remaining: optional owner skim of `reports/goldens-mining/entity-report.md`, the query-construction session (~1hr, owner + keyboard), harness corpus-ref extension, A/B re-run → merge `feat/memory-hybrid-retrieval` on lift or delete on second null. Parked branch rots mechanically as develop moves — rebase it when resuming. Filed 2026-07-06.
