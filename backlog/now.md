...
- 🐛 `[FIX]` **Multi-tag restart recovery resurrects zombie groups → phantom in-character timeout errors + held/degraded real replies** — Runtime-traced 2026-07-13 (~12:32Z, prod): bot-client redeploy (12:13) → `MultiTagRecovery` rehydrated group `07b94bed` from Redis whose slot jobs died with the old instance → unfillable group hit the safety timeout 18 min later → "flushing with synthetic timeout errors" delivered an in-character `(error: timeout — "Response timed out")` to the channel, unattached to any live turn. Worse: the per-channel `ResponseOrderingService` HELD the real turn's completed result (`b586835b`, done 12:24) behind the zombie group until the 12:32 flush — 8-min delivery delay, and its TTS audio expired in Redis meanwhile (`TTS audio expired or not found`) so the reply arrived voiceless. User-visible as error-then-success chunks. Pre-existing class (recovery machinery predates beta.162); trigger = any redeploy leaving a multi-tag group entry in Redis (stale OR in-flight). **Fix shape**: (a) on rehydration, validate each slot's jobId against the queue/gateway — a group whose jobs no longer exist gets dropped/logged, never safety-flushed into user-facing synthetic errors; (b) bound the ordering hold a rehydrated group can impose (or exempt post-restart groups from ordering); (c) consider TTS-TTL vs max-hold coherence. Repro: redeploy bot-client while a multi-tag entry exists. Filed 2026-07-13.
