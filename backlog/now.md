...
- **🧑‍💻 Owner to-do**: (1) **the PROD `INTERNAL_SERVICE_SECRET` rotation** — the stage-1 dry run flips REFUSE→PASS once beta.225's gateway is live on prod; then stages 1→2→3 with each redeploy landed between (Deploy notes); (2) TASK-907: review the commitment facts via `/memory facts`, Sort A-Z, the `{assistant}` block at one end; note any welcomed pet name NOT extracted (TASK-950); (3) TASK-104: trim and re-upload the 8 over-cap voice references, then `pnpm ops voice-refs:audit --env prod`; (4) leisure, unchanged: card-level examples for Emily; the voice-harness blind review.
