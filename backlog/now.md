...
- **In**: #2505 (TASK-1070, voice-engine on Python 3.13) · #2506 (TASK-1073, husky hooks under dash) · #2507 (doc-72 PR A, page navigation) · #2508 (TASK-1079, the settings customId cap fix) · #2509 (doc-72 PR B, the resets + TASK-1080) · #2510 (TASK-1083, `REDIS_IP_FAMILY`; the first cloud-built unit) · #2511 (TASK-1067, README podman commands on localhost-only ports; docker-compose.yml deleted) · #2512 (the cloud dispatch mode in the orchestration skill; residue TASK-1091) · #2514 (removals gate and half-finished clauses in 06-backlog) · #2513 (council skill for council v4 and `debate`) · #2515 (TASK-676, one `sha256Hex` primitive) · #2518 (ruleset snapshots; the unattributed-approval flag off) · #2519 (TASK-1092, pre-push auto-throttle on ≤4 cores) · #2503 and #2517 (dependabot groups) · #2521 (TASK-702 + TASK-628, the glm-4.5-air OpenRouter denylist retired; TASK-1093 watches for a code-1210) · #2522 (TASK-576, a manual `workflow_dispatch` runs the paths-filter-gated steps; a dispatch on a branch cancels that branch's push run via the concurrency group) · #2523 (TASK-614, the documented single-file vitest command runs one file; CI's component coverage upload restored). · #2520 (doc-14 PR-6a, the `defineCustomIdFamily` core + Persona pilot; six rounds) · #2524 (TASK-777, the CI gate asserts the review run exists; new `CI_GATE_REVIEW_MISSING` sentinel; remedy half TASK-1095) · #2525 (tooling + website unit coverage upload; TASK-1094 residue) · #2526 (doc-14 PR-6b, the destructive confirmation preset on the family core; two rounds) · #2529 (TASK-733, cross-channel rows carry their quote and forward fields; cross-channel prompt blocks now render quoted replies and forward attribution) · #2528 (TASK-719, forwarded-snapshot embed image names scoped by snapshot ordinal, plus wrapper-embed parity on the forwarded reference; the deduped-path echo gap is TASK-1098) · #2527 (TASK-1096, `guard:repo-settings` asserts every main-required check is a context main's ci.yml produces, matrix legs exact; its WARNING prompts the `hook-posix-parse` re-add after this cut; four rounds) · #2530 (TASK-635, a mid-stream chunk failure persists the delivered chunks as the turn's one row; partially delivered multi-tag slots are marked delivered, owner ruling; the owner-alert dedup hashes the partial's cause frames; two rounds; the error-notice partial case is TASK-1100) · #2531 (TASK-1023, hyphenated `api-key` names redacted as object keys and query parameters; the reviewer's ten fragment posts are TASK-1099) · #2532 (TASK-1019, the `worktree:transfer` unpushed-remotes check has its `HEAD` positive revision and a real-git fixture; one round) · #2533 (TASK-984, tooling's build no longer emits its 229 test files into dist; the pre-push docs-only classifier gap is TASK-1101). On `main` outside the train: #2516 (claude-code-action 1.0.231 via main-cut; develop finalized onto it 2026-09-24, `9fe910778`; the main ruleset's `hook-posix-parse` requirement dropped live to unblock it — the cut step below re-adds it).
- **Waiting on**:
