...
- 💰 `[FEAT]` **Extraction engine: GLM-5.2 selected (sweep 2026-07-10) — finish the migration** — 8-model modern sweep + confirm run: `z-ai/glm-5.2` = 0.0% violation BOTH runs (vs Haiku 10.3%), recall 97.5–98.6%, supersession 100%, parse 48–49/50 (~2–4% batch-skip, fail-to-skip harmless). 25–50% cheaper than Haiku on OpenRouter. openrouter/free measured unusable (18/50 parsed). **Dev flipped to GLM-5.2 (burn-in live).** Remaining: (a) provider threading so extraction rides the z.ai flat-rate SYSTEM key (provider override + provider-labeled usage rows + X-Title portability — PR #1569 review scope) — **gates the fact backfill (owner)**; (b) **delay-not-downgrade (owner 2026-07-10)**: on z.ai unavailability (China peak), requeue extraction jobs with delay instead of falling back to OpenRouter — facts aren't time-sensitive, completions are; extraction never pays OpenRouter once on the plan; (c) optional: chase the 1-in-50 parse miss; (d) prod `EXTRACTION_MODEL=z-ai/glm-5.2` at prod-enable. Sweep data: scratchpad sweep-*.json (session 2026-07-10).
