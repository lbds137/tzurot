...
- **In already**: `show_thinking` retirement (#2110, incl. data-only JSONB strip migration — applied to dev 2026-08-16; prod at the cut via premigrate, safe either order) · TASK-625 premigrate comment-blind detector fix (#2111 — grew into a full quote/dollar-aware SQL walker over 4 review rounds; residuals in TASK-627) · **doc-77 PR B (#2112, 2026-08-16)** — z.ai `thinking`/`reasoning_effort` translation + denylist→allowlist flip; step-0 probe ran as direct API calls (Discord-config route abandoned: `llm_configs` is dev↔prod sync-tracked), results in doc-77; the GLM presets' advertised levels are now real.
- **Waiting on**: doc-77 PR C (#2114, in review) · GLM-5.3 catalog add (probe done 2026-08-16: live, `disabled` best-effort 70rt, levels honored; entry pre-staged) · riders below.
- **Riders (owner-settled 2026-08-16)**: TASK-620 (guest-mode free-model substitution invisible in footer — owner directive) · TASK-626 (durability hardening — owner: "worthwhile") · TASK-629 (image-only reference stubs contentless) · TASK-630 (cache observability logging). TASK-628 explicitly NOT riding — routed to the drain campaign (owner call).
- **Deploy notes**: owner dev smoke for PR B ✅ PASSED 2026-08-16 (requestId `06dd8b51`, glm-5.2 via Z.AI Coding Plan): `/inspect` shows `thinking: medium` in allParams, trace extracted and present. Nothing else outstanding.
