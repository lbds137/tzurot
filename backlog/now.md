...
- 🐛 `[FIX]` **GLM 5.3 rate-limit fallback dead-ends on OpenRouter 400 — owner-reported 2026-08-16 ~21:53 EDT** — during z.ai China-peak hours, a GLM 5.3 generation hits the z.ai rate limit and the auto-promotion falls back to OpenRouter **with the same model id** — but GLM 5.3's OpenRouter release is staggered (weeks behind z.ai, owner-confirmed vendor timing), so OpenRouter 400s (`z-ai/glm-5.3 is not a valid model ID`, ref `mswky34dbpd`) and the user gets an in-character error instead of a reply. Runtime evidence: owner screenshot, footer chain `glm-5.3 → z-ai/glm-5.3 (rate limited) • via OpenRouter` — the fallback-transparency footer (#2118-adjacent) worked; the fallback itself dead-ended. Owner worked around by switching their default preset to GLM 5.2. **Fix shape (code-read candidates, not yet grounded)**: the z.ai→OpenRouter promotion should consult the OpenRouter model catalog (`OpenRouterModelCache` already holds it) before promoting to a model id the target provider doesn't carry, and a 400-on-fallback should try the next tier rather than surfacing; alternatively a catalog flag for known z.ai-only models gates the promotion. Natural beta.204 rider (touches the ProviderRouter promotion path). Filed 2026-08-16 (night).
