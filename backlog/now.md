...
- **In already**: nothing (fresh range).
- **Waiting on**: doc-77 PR B (z.ai `thinking`/`reasoning_effort` translation + denylist→allowlist flip; step 0 = the owner's BYOK dev probe, which also verifies `thinking:'max'` and `effort:'none'` on the OpenRouter wire) · doc-77 PR C (config-time validation, consumes B's probe) · `show_thinking` retirement (unblocked, spec pre-grounded in scratchpad).
- **Candidate riders, not gating**: durability hardening small PR (withGatewayRetry + chunk-skip + webhook/DM id asymmetry) · TASK-625 (premigrate comment-blind detector).
