...
- **Waiting on**: TASK-626 (PR #2119, in CI) · **TASK-634 (owner directive 2026-08-16: "fix both of those issues before we cut")** — gate the dev retention nag to prod + extend the purge confirmation to dev (a dev purge's tombstones would sync deletions to prod); worker in flight.
