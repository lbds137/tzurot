...
- 💰 `[FEAT]` **Extraction cost controls + insight (owner spend concern 2026-07-09)** — Haiku extraction burned ~$2/2days on the system OpenRouter key (owner's residual OR spend is ~$15-20/mo post-z.ai; this pace would double it). Findings: `MODEL_DEFAULTS.FACT_EXTRACTION` hardcoded (anthropic/claude-haiku-4.5); `FACT_EXTRACTION_DAILY_LIMIT=100`/personality/day hardcoded (tripwire, not minimizer); **extraction calls write NO usage_logs rows** (main completions log via AIJobProcessor; extraction's direct createChatModel path bypasses it — zero in-system cost visibility); local eval runs untagged (missing OPENROUTER_APP_URL in .env → "unknown" app). **One small PR**: (a) `EXTRACTION_MODEL` env knob, (b) `EXTRACTION_DAILY_LIMIT` env knob, (c) usage_logs rows for extraction calls, (d) distinct `X-Title` ("Tzurot Extraction"). **Then**: run `pnpm eval:extraction` against z.ai GLM (+ openrouter/free for data) — if GLM passes the gate, extraction moves to the flat-rate z.ai plan and this OpenRouter line →$0. Market note: shapes.inc ships a separate "LTM Engine Model" selector — a memory-engine model knob is the market bar. MUST land before prod-enable. Per-user extraction-model override deliberately out (un-gateable quality). Stopgap available now: flip `EXTRACTION_ENABLED` off on dev between memory sessions.
