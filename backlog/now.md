...
- 🐛 **[INVESTIGATE — symptom confirmed, mechanism unknown] Preset (llm-config) PUTs time out — gateway >10s on the update path** — Preset edit (section modal), global-toggle, and set-default commands fail with `"Request timeout (gateway slow or unavailable)"`; the user retries and it keeps failing. **Confirmed from prod logs (2026-06-12 ~01:36–01:54 UTC, admin + user paths)**: bot-client calls `PUT /api/admin/llm-config/{id}` and `PUT /api/user/llm-config/{id}` (+ `.../set-free-default`); the gateway takes **>10s** so bot-client aborts. Gateway logs the matching `request aborted … responseTime=10005/9999/9990… statusCode:null` — pinned at the abort, request never completed. So it's **genuine gateway slowness on the llm-config update path**, not merely a too-short timeout (a config write should be sub-second). **Mechanism NOT yet known**: during those seconds the gateway logged NOTHING — prod `prisma:query` logging is off, no processing/error lines, no pool/lock/deadlock signals. Intermittent (some config ops in the same window succeeded), which *suggests* resource contention (DB connection-pool wait / lock) rather than a constant slow query — **hypothesis, unconfirmed**. **NOTE: not dev-reproducible** — it's load-correlated (prod has concurrent users; dev is a single user with no load), so it must be observed in PROD. Path: ship **targeted timing instrumentation to the prod llm-config PUT** (breakdown log: validate / DB write / cache-invalidation / total), OR stand up real perf observability first (see the Production Observability theme in `cold/queue.md`). **Three tracks**: (a) root-cause + fix the gateway slowness (real fix, OPEN — **PRIORITIZED 2026-06-17**, user: "keeps biting me"; now a Current Focus item — ship the prod timing instrumentation); (b) the bot-client timeout — **MITIGATED 2026-06-16 (#1228)**: llm-config CRUD writes now use a 20s `GATEWAY_TIMEOUTS.WRITE` budget (was 10s); the transport defaults all write methods to WRITE, so a PUT taking >10s-but-<20s no longer false-aborts client-side; (c) the UX false-negative — **MITIGATED 2026-06-17 (this session)**: on a status-0 client abort the preset dashboard now shows an honest "⏳ may still be applying — tap Refresh to confirm" notice instead of "❌ Failed to update preset. Please try again." (which was wrong — the write usually commits server-side, and "try again" risked a duplicate write). **Root cause (why the gateway PUT exceeds even 20s) is NOT fixed.** **Recurred 2026-06-17** (vision-model save on a preset, prod) — still biting post-#1228, so the gateway PUT genuinely exceeds the 20s budget sometimes. Surfaced 2026-06-11 (user prod session); track (a) stays open and is now prioritized.

---

### 🎯 Current Focus (max 3)

1. **Prod timing instrumentation for the llm-config PUT slowness** (🚨 production root-cause, escalated 2026-06-17 — "keeps biting me") — add phase-breakdown timing (`debug`-type instrumentation) to the gateway `PUT /api/{user,admin}/llm-config/{id}` (+ `set-free-default`) handlers: log `validate / DB write / cache-invalidation / total` ms with the requestId. It's load-correlated and **not dev-reproducible**, so it must reach **prod** to be useful (merge → dev auto-deploy is necessary but not sufficient; needs a release to prod, OR a temporary prod-targeted deploy). Once prod logs show which phase blows the budget, fix that phase and remove the instrumentation. See the 🚨 Production Issue entry, track (a).
