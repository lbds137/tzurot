...
- 💰 `[FEAT]` **Extraction engine: GLM-5.2 — finish the migration** — sweep data + selection rationale in git (this entry's history). ~~(a) provider threading~~ + ~~(b) delay-not-downgrade~~ **SHIPPED in PR #1572** (2026-07-10: `ZAI_CODING_API_KEY` + `EXTRACTION_PROVIDER`, busy→delay via `Worker.RateLimitError`, budget-neutral refunds, requeue shrinks to unfinished groups, sustained-busy escalation). ~~Owner dev env vars~~ + ~~z.ai confirmation eval~~ **DONE 2026-07-10**: dev extraction is LIVE on the plan (boot coherence clean; eval via z.ai endpoint: 50/50 parse — better than OpenRouter's 48–49 — recall 98.0%, supersession 100%, 0/52 effective fabrication; the sole scored "violation" was a golden-annotation gap, filed in follow-ups). Remaining: dev burn-in validates the delay path at the next real peak window (info-log observable); (c) optional: chase parse misses using the backfill run's failures as the sample set; (d) prod `EXTRACTION_MODEL=z-ai/glm-5.2` + provider vars at prod-enable. **Fact backfill is UNBLOCKED** (cold/ideas.md).
- 🏗️ `[LIFT]` **Real-scale eval goldens → unpark hybrid retrieval** — the Phase 1a resume gate (full context: `active-epic.md` § Phase-1a park — the memory theme was promoted to Active Epic 2026-07-06). Build goldens from REAL prod memory data at real corpus scale (hundreds of rows/persona, where dense dilution actually bites), re-run the dense-vs-hybrid A/B, then merge `feat/memory-hybrid-retrieval` on demonstrated lift or delete it on a second null. **Design DECIDED (owner, 2026-07-11)**: committed anonymized goldens (semi-permanent per the artifact's every-phase eval gate), mined from the OWNER'S OWN persona memories (dogfooder-in-chief, never edited — no third-party privacy exposure). Anonymization = light-touch entity swaps (paraphrase would shift embeddings and invalidate the measurement) + owner review gate before commit. Owner involvement wanted for realistic query construction. ~2hr + the owner session. Parked branch rots mechanically as develop moves — rebase it when resuming. Filed 2026-07-06.
