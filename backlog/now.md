...
1. **Test-Pyramid Taxonomy + Coverage Audit — active epic** (see [`active-epic.md`](active-epic.md)). Shipped: Phase 1 (#1284), Phase 1.5 (#1285), **suffix rename (#1339)**, **golden-fixture envelope contract + topology skeleton (#1340)**, **Phase 2a — code-derived topology generator (#1341)**, and **Phase 2b — mechanism-PRESENCE verification + `topology:check` lockfile-diff CI gate (#1342)** (probes each surface's mechanism on disk; committed `coverage-topology.json` byte-compared in `pnpm quality` + CI; 154 surfaces, 0 gaps). **Phase 2 COMPLETE.** **Enforcement RESOLVED** (council 2026-06-25): no standalone `test:tier-audit` ratchet — it would duplicate `test:audit` + `topology:check` / be a noisy `knownGaps` dump; `test:tiers` stays the report-only dashboard. Shipped the one endorsed extension — `test:audit` now covers the extracted packages' Prisma services (#1344). **Phase 3 gap-fill DONE (#1345).** **Phases 1–3 COMPLETE.** A 2026-06-25 deep audit + 3-model council reframing then opened **Phase 4 — Seam Contract Coverage**: the contract tier was hollow exactly where prod bugs cluster (1 real golden-fixture contract mis-tiered as component; 2 circular BullMQ "contract" tests; none running in CI; a CI/local Redis mock-split hiding bugs). Contract tests are mid-pyramid → populate by behavioral shape. **PR1 SHIPPED (#1346)** — Redis keystone (real Redis everywhere; 2 latent bugs fixed). **PR2 SHIPPED (#1347)** — tier honesty + CI cleanup (reclassified the real contract test, deleted infra-smoke, ran the contract tier in CI, dropped dead Postgres, renamed the job). **Next: PR3 (BullMQ contract) → PR4 (envelope scenarios) → PR6 (voice-engine schema-first) → PR7 (high-value long-tail seams: HTTP and/or Redis pub/sub) → execution ratchet LAST** (locks the complete contract set). PR7 added + ratchet moved last per user 2026-06-25. Roadmap + council detail in [`active-epic.md`](active-epic.md); full plan in `~/.claude/plans/floofy-rolling-crane.md`. The old grab-bag nits fold into the relevant Phase-4 PRs.
