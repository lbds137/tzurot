...
**🏗️ `[LIFT]` CI wall-time cut: matrix-split the test job — BEFORE 2026-07-07** — User directive 2026-07-03: the `test` job paces all of CI at 5–6min. Measured breakdown (run 85062215428): ~48s setup + **267s unit-tests-with-coverage** (all packages contending on one 4-vCPU runner) + ~45s uploads/typecheck. Design: matrix per package group (bot-client / ai-worker / api-gateway / tooling / packages-rest), each job = setup + solo package tests + its codecov flag upload (codecov merges flagged uploads; per-package files already produced today). **Method (user question 2026-07-03 answered with CI history)**: baseline from the last 10 CI runs = test job p50 ≈5.8min, range 4.9–6.0 (one 3.2 outlier) — judge the split against p50, not one run. Historical PER-PACKAGE times can't size cells (they measure shared-runner contention that the split removes) → do an in-PR CALIBRATION round: first matrix push produces real solo-cell CI times, rebalance cell membership once against those, then merge. Expected: CI floor p50 5.8min → ~3.5min (then co-paced by mutation-tests ~3m25s; lint ~3m30s is the next split candidate if wanted). **Bin-packing principles (user directive 2026-07-03 — squeeze the 20-concurrent-job free-plan budget, both directions):** (a) size matrix cells by RUNTIME, not one-per-package purism — group small packages into shared cells so every cell runs ≈ as long as the pacing cell; (b) consolidate confetti jobs where their status signal isn't load-bearing (voice-engine-test 7s is a fold-in candidate; fixup-check 7s is NOT — its independent red-on-fixup-branches status is deliberate signal per 08-review-response); (c) budget for TWO branches overlapping: ci.yml already has per-ref `cancel-in-progress` (verified — stacked pushes don't duplicate), but different branches' runs coexist, so keep single-run peak ≈16 jobs (current ~12: 7 ci.yml + 4 CodeQL + claude-review) so 2× overlap only queues briefly. Public-repo minutes are free; setup ≈48s/job is the only real per-job cost. `guard:gate-parity` note: the test job isn't the lint job — no parity impact.
