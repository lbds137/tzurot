...
- 🐛 `[FIX]` **DEV: system-settings bag mis-seeded → extraction burning haiku on the paid OpenRouter key (ACTIVE) + PROD RELEASE GATE** — Root cause (runtime-confirmed 2026-07-13): the PR-1 seed runs IN THE GATEWAY from the gateway's env, but the five extraction/zai vars lived only on ai-worker (Railway env is per-service) → both bags seeded wrong: `extractionEnabled=false, factsInPromptEnabled=false, extractionModel=haiku, extractionProvider=openrouter, zaiFreeTierEnabled=false`. Pre-PR-3 consumers read their own env → worked; post-PR-3 (dev, 2026-07-13) they read the bag. **Dev impact NOW**: backfill queue still draining on haiku via the paid OpenRouter system key — 956 calls / 4.19M in-tokens in one hour (10:29–11:28Z), ALL fail-to-skip (parser is GLM-5.2-tuned) = ~$5-10/hr for zero facts; organic extraction+facts also off. **Fix (owner, ~2 min)**: `/admin settings edit` → System pages → set the five values back (extractionEnabled ✅, factsInPromptEnabled ✅, extractionModel `z-ai/glm-5.2`, extractionProvider `zai-coding`, zaiFreeTierEnabled ✅) — or approve the SQL patch in the session scratchpad (`fix-bag.ts`, classifier-denied for agent). **PROD (release blocker, verified)**: prod gateway has NONE of the five vars; prod ai-worker has all five (incl. facts ON) → prod's bag is identically wrong. Harmless while prod runs beta.161 (env-read), but the next release (PR 3) silently kills prod extraction+facts+zai and re-points extraction to paid haiku. **Prod bag must be corrected BEFORE the next release merge** (dashboard on prod, or SQL). Structural note: the env-derived seed is GONE post-PR-3 (fallback-seeded), so this is a one-time repair class, not recurring — but the release checklist gets a bag-verification step. Filed 2026-07-13.
