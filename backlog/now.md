...
- **🧑‍💻 Owner to-do**: (1) the prod flips (Recent Days switch + Emily listed; Cross-Channel Content → Your messages only) once prod boots on `0f1e9cfbc`; (2) TASK-907: review the commitment facts via `/memory facts`, Sort A-Z, the `{assistant}` block at one end; note any welcomed pet name NOT extracted (TASK-950); (3) TASK-104: trim and re-upload the 8 over-cap voice references, then `pnpm ops voice-refs:audit --env prod`; (4) say whether to file the archive/digest consolidation idea doc (the 2026-09-18 musing; the classifier blocked the agent's create); (5) leisure, unchanged: card-level examples for Emily; the voice-harness blind review.
...
- **🗺️ Horizon (rolling three releases, re-touched at every cut)**:
  - **beta.227** — this block.
  - **beta.228** — caching Phase 3 if the doc-17 reading arrives; the remaining characters' archive flips as their gates read READY; slice C's dead-row report; the UX-epic slice (`doc-14` waves 4–6); the archive/digest consolidation pass if its idea doc is filed and the digest shape has settled.
