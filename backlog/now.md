...
- **In**: #2534 (TASK-979, the compact deferred-refs reminder the hooks print; two rounds, `20c33ce8c`) · #2536 (the `.github/rulesets/branch-protection.json` snapshot refresh for the `hook-posix-parse` re-add plus the README counts; two rounds, `14c276f5c`) · #2538 (TASK-1102, `buildSettingsCustomId` throws past Discord's custom-id cap; one round, `306e6d5eb`) · #2537 (TASK-1002, `guard:prompt-tags` scans source with one string-aware scanner, both workarounds removed; six rounds at the cap, the round-6 edges are TASK-1107; `beaf5f123`) · #2539 (TASK-1107, `}` dropped from the scanner's regex-preceder set, four heuristic edges pinned; one round, no findings, `68bcc6118`) · #2540 (TASK-1081, `shellcheck -s sh` over the lifecycle hooks in `hook-posix-parse`, hooks made shellcheck-clean; three rounds, the cloud unit's rounds landed via the driver; `05891f352`) · #2541 (the owner-approved rules-economy pass: 39 rule cuts to pointers at harness `core.md`, BACKLOG.md and the skills, twelve memory promotions landed as text, rules 129,970 → 114,213 B; four rounds, two owner rulings (cloud specs carry core.md § Safety; two local fallback sentences); `004b4f108`) · #2542 (TASK-1069 follow-up, the Vencord WebM transcode: the stream-copy remux carried jittered granule positions libsndfile rejected, so the no-key voice path failed on prod; one round, one Low comment hedge; `bdfe4b5b3`).
- **Waiting on**:
