...
- 🧹 `[CHORE]` **db-sync output UX — paginate, don't truncate (owner 2026-07-10)** — sync results currently truncate heavily; owner: "every time I run the sync, it just occurs to me the UI for this kinda sucks." Rework the output to the house browse/pagination patterns used by other commands (`04-discord.md` § Shared Utilities). Small, independent, wanted "soon." Where the output renders + which surfaces truncate = first 15 min of the task.
