...
- 💰 `[FEAT]` **Extraction engine: GLM-5.2 — finish the migration** — sweep data + selection rationale in git (this entry's history). ~~(a) provider threading~~ + ~~(b) delay-not-downgrade~~ **SHIPPED in PR #1572** (2026-07-10: `ZAI_CODING_API_KEY` + `EXTRACTION_PROVIDER`, busy→delay via `Worker.RateLimitError`, budget-neutral refunds, requeue shrinks to unfinished groups, sustained-busy escalation). Remaining: **(next) OWNER: set `ZAI_CODING_API_KEY` + `EXTRACTION_PROVIDER=zai-coding` on dev ai-worker** → one free confirmation eval via the z.ai endpoint (same weights, guards against serving-config drift) → dev burn-in on the plan validates the delay path during a real peak window; (c) optional: chase the 1-in-50 parse miss; (d) prod `EXTRACTION_MODEL=z-ai/glm-5.2` + provider vars at prod-enable. **Unblocks the fact backfill** (cold/ideas.md) once dev vars are live.
