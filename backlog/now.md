...
**🔎 `[FEAT]` Spacebar spike — `TASK-1108` (owner directive 2026-09-25: "I'd like to do the spike soon"; `doc-83` un-parked, `doc-79` is the sovereignty frame)** — part 1 is the read-only map: the discord.js surface `bot-client` actually uses, row by row against the Spacebar server source (implemented / stub / missing, with cites), a federation paragraph per candidate (federation is a new owner requirement), and whether discord.js can be pointed at a non-Discord host. Part 2, the hands-on instance probe on the Deck, waits on part 1's host answer. No new repo for the spike; a fork is the decision the spike informs, and it is the owner's. Runs after the two in-flight drain units (TASK-1107 local, TASK-1081 cloud) land; part 1 is cloud-lane eligible.
