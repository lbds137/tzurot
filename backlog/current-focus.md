...
1. **PR-2n epic — Phase 2 (services extraction)** — see [`active-epic.md`](backlog/active-epic.md). Sequencing: `PR-2o ✅ → Phase 2.5 (a ✅ → b ✅ → c-i ✅ → c-ii ✅ → c-iii → d) → PR-2p → PR-2q`. Shipped 2026-06-04: **2.5a** (#1153, shadow hydration), **2.5b** (#1154, 3 gateway endpoints + dual-write), **2.5c-i** (#1155, write cutover behind `CONTEXT_MODE`), **2.5c-ii** (#1156, routing-read cutover: `HttpPersonalityLoader` + positive/negative cache). **2.5c-iii scoped (council 2026-06-04, verdict in active-epic.md), sliced iii-0 ✅ (#1157, user-message endpoint) → iii-a → iii-b. Next: iii-a** — worker-side full context assembler (persona resolution, upserts, HistoryMerger, mention/reference rewrite) + extended shadow mode (raw envelope fields alongside legacy payload, real-DB hydration, match-rate telemetry with go/no-go threshold). Then iii-b (discriminated-union envelope cutover) → 2.5d (deletion) → PR-2p.
