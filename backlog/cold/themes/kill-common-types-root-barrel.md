...
### Scripts-hardening PR (pending user triage) — lands before Phase 3

Motivated by the `deploy-commands.ts` blind-spot finding (PR #1474 review). The `scripts/` ecosystem has drifted from `05-tooling.md`'s "No Standalone Scripts" rule (persistent tooling → `packages/tooling/`; `scripts/` = one-off, deleted after use). Scope:

- **Typecheck/lint coverage for the gaps**: `services/bot-client/scripts/**` (outside bot-client's `src/**` include — typechecked by nothing) and `scripts/` root-level files (`generate-command-types.ts`, `audit-*.ts` — not in any subdir `@tzurot/scripts` includes). Expanding `scripts/tsconfig.json` include to root `*.ts` surfaced **12 latent errors** (unused vars in `audit-*`, implicit-`any` in `generate-command-types`, a `vitest.config.ts` rootDir issue) — real, must be fixed as part of adding coverage.
- **Reorganize per the rule** (needs user's per-file staleness call): move persistent tooling → `packages/tooling/` (`generate-command-types`, `utils/update-deps`, likely `src/openrouter-gen`, the `audit-*` if periodic); delete confirmed-stale one-offs (`migrations/{verify,fix}-migration-checksums`, `migrations/migrate-error-messages`, `debug/*`, `src/db/fix-phantom-migration`, `src/inspect-leaked-request`, `utils/{verify-llm-default-constraint,set-default-llm-config}`). `deploy-commands.ts` stays with bot-client (service-specific; tooling would be a backwards dep) — just gets coverage. `vitest.config.ts` is legit.
- **Migrate the remaining bare-barrel refs** in the root migration scripts (or delete them) so Phase 3's `"."`-drop doesn't break them.
- [ ] **Restore `import type` precision in PR-1's `packages/*`** (folded in here per user, 2026-07-03). The codemod's mutate-in-place path dropped whole-import `isTypeOnly` before the PR-2 fix, so PR 1's merged packages carry `import type { X }` → `import { X }` on a split import's first group. Compiles + lints (`@typescript-eslint/consistent-type-imports` not enforced) and no boundary breach in those packages, but imprecise — for value-classes (`PrismaClient`) it turns an elided type-import into a runtime import. **Fix shape**: either grep `packages/*` for deep `import { … } from '@tzurot/common-types/…'` where the symbol is type-only-used and restore `import type`, OR enable `@typescript-eslint/consistent-type-imports` (auto-fixable) as the structural guard that catches this class repo-wide (pairs naturally with the `no-restricted-imports` guard above). Surfaced 2026-07-03 (barrel-kill PR 2 test gate).
