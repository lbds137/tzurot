...
- **S2a — route kind-threading (✅ SHIPPED, PR #1378).** Threaded request kind through the CRUD routes. **Key design realization:** kind is intrinsic to a config's unique id, so `?kind=` belongs only on **LIST** (no id — "configs of which kind"), via `parseConfigKindQuery` (Zod, default text, 400 on invalid, `query ?? {}` so an absent object isn't a 400). **By-id routes are kind-agnostic** — except the **admin** writes (edit/set-default/free-default/delete), where the Phase-1 `requireKind:'text'` gate was a deliberate protection: it's parameterized to `requireKind: kind` (defaults text → bare route 404s a vision row; `?kind=vision` unlocks vision management, incl. the **global vision default** via admin set-default since `setAsDefault` is kind-scoped). **User by-id** kept fully kind-agnostic (never had a gate; the update name-collision derives kind from the immutable row via `toConfigKind(config.kind)`). `requireKind` typed `ConfigKind`. The admin/user asymmetry is principled: thread the gate where it existed, don't bolt one onto routes that never had it.
- **S2b — vision-aware override writes (NEXT).** `routes/user/model-override.ts` branches on the config's kind so set/clear of a user's default / per-personality override writes the vision FK columns (`users.defaultVisionConfigId`, `user_personality_configs.visionConfigId`) when kind=vision, else the text FK; read paths (GET default/list) gain `?kind=`. Decision: branch-on-kind within the existing routes (kind from the config row on set, `?kind=` on clear), NOT parallel `/user/vision-override` routes. Confirm against the `VisionConfigResolver` read cascade (`visionConfigId` → `defaultVisionConfigId` → personality vision default → global vision default → fallback). Plus the deferred #1378 notes: the **`getById` double-trip** on model-update (pass the route's already-fetched row kind into `validateLlmConfigModelFields`), the **`NameExistsChecker` options-object** refactor only if the 5-param signature grows. (Global vision default already works via S2a; the personality-level default tables are a separate pre-existing gap — see `cold/follow-ups.md`.)
- **S2c — commands.** `kind` choice option (`text | vision`, default text) on the four default-setters (`/preset global default|free-default`, `/settings preset set-default|set`) + `/preset create` (the `kind` must survive the modal round-trip via the custom-ID prefix); autocomplete reads `kind` and filters via `/api/internal/models?inputModality=image` for vision. Drives S2a's `?kind=` + S2b's writes end-to-end. Modal model entry is plain text (no autocomplete inside modals), so S1b's save-time capability validation is the safety net for modal-created configs.
