...
- **S1 — backend.** Parametrize the Phase-1 hardcoded `kind:'text'` filters in `LlmConfigService` (`list`, `checkNameExists`, `resolveNonCollidingName`, `create`) by a requested kind; add `kind` to the create/update Zod schemas + the LIST/DETAIL select constants. **`kind` is IMMUTABLE** — omit it from the update schema; delete+recreate to "convert" (mutable kind orphans per-kind default flags + mis-routes resolvers). Build a **`ModelCapabilityService`** that unifies OpenRouter + the z.ai catalog, and validate capability at create AND update: **vision = fail-closed** (unknown or non-vision model → reject), text does NOT reject vision models (vision models are text-capable), and the requirement is **derived from the row's `kind`**, not a separate caller-passed flag (callers can lie; the row can't). Absorb the deferred **#4 `getById` kind-gate** here — the editing surface makes the read-path gap matter and justifies the `findUnique`→`findFirst` test churn.
- **S2 — commands.** Add a `kind` choice option (`text | vision`, default `text`) to the four default-setters (`/preset global default|free-default`, `/settings preset set-default|set`) + `/preset create` (the `kind` must survive the modal round-trip via the custom-ID prefix); autocomplete reads `kind` and filters via `/api/internal/models`; **vision-aware WRITE paths** (set-default / set-override write the vision FK columns — `defaultVisionConfigId` / `visionConfigId` — when kind=vision). Modal model entry is plain text (no autocomplete inside modals), so S1's save-time capability validation is the safety net for modal-created configs.
