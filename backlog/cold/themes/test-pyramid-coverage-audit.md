...
**Bulk (big epic) — ⚠️ SUPERSEDED (council 2026-06-25):** the original plan was a `pnpm ops test:tier-audit` registered audit-class tool (per-area tier-coverage gap matrix with a ratchet). A 3-model council (GLM-5.2 / Kimi-K2.7 / Qwen-3.7) **unanimously rejected** it: it would duplicate `test:audit` (artifact→tier) + the Phase-2 `topology:check` (cross-service surfaces), and with integration=1 / e2e=0 a broad "every area needs every tier" gate baselines almost the whole repo as a meaningless `knownGaps` dump. **Resolution**: enforcement = `test:audit` + `topology:check`; `test:tiers` stays the report-only dashboard. Shipped the one endorsed extension (#1344: `test:audit` now scans the extracted packages). A flow-level integration/e2e gate is deferred until Phase 3 produces coverage to lock. (The original premise that `test:audit` "measures colocation, not tier coverage" was also wrong — it already ratchets Prisma-service→component + schema→contract.) See `active-epic.md` › Enforcement.
