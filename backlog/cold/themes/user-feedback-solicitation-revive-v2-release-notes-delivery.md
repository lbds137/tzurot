...
**PR slicing** (each gets plan-mode at build): **PR-1** schema foundation (users columns + release_announcements + delivery_log + user_feedback in ONE migration — no mid-epic schema tax) + `/notifications` user command surface · **PR-2** DM-blast worker + `/admin broadcast` with dry-run (the test vehicle) · **PR-3** release pipeline (webhook endpoint + reconcile job + changelog classifier/formatter) · **PR-4** `/feedback` intake + owner channel posts · **PR-5** hardening (ops report, retention, docs). Feedback half (PR-4) after the release-notes half per owner priority.
