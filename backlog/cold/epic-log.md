...
_PR-3b ride-along (from #1701 reviews, reviewer-deferred "worth tightening before PR-3b"): the alias path runs two identical `user.findUnique` round-trips (`resolveOwnerUuid` + `resolveAliasUserUuid`) for every non-bot-owner alias resolution — resolve the user row ONCE in `loadFromDatabase` and derive both the access-filter uuid and the personal-alias uuid from it (only the bot-owner bypass branch needs distinct handling). Hot path: personality resolution runs per message. While in there: collapse `findPersonalityViaAlias`'s `{ userId } | 'global'` scope union to plain `userId: string | null` (r2 readability nit — same call sites the single-lookup refactor reshapes)._
