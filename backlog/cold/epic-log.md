...
| PR-2b | Remaining families onto the registry (memory, voice, settings, models, deny, channel, inspect, shapes, help, admin) + §2.3 BLURPLE fixes (voice + apikey browse) + 3 bespoke autocompletes onto the shared formatter + **ride-along from #1743 r2**: stale 📖 in personalityAutocomplete.ts:32 JSDoc → 👥 | queued |
