...
- **Provider Prompt Caching Phase 2** (`doc-17`, operational record in `active-epic.md`) — history extraction, the measured ~58% of the request; the biggest pure-savings item on the books. MUST land before memory re-entry (same prompt/history seams).
- **Platform-Portable UX Layer waves 4–6** (`doc-14`) — PR-6a factory core+pilot → 6b destructive preset → 7 `/deny` redesign (doc-87's naming-axes question folds in here) → 8 picker hygiene → 9/10 factory sweep + router adoption. Finishing this un-pauses v2 parity builds and gives doc-86 a surface standard. Phase 4 (adapter) stays trigger-gated on a real second platform.
- **Memory System Overhaul re-entry** (`doc-8`) — PARKED mid-epic; phases 0/1c/2 shipped + prod-enabled; three honest nulls (RRF, fold, composite) per the eval gate. **Trigger: the owner's felt-repetition re-measure (~15 min, pending since July).** Re-entry format: LID pilot. Phases 3–6 evidence-gated (Phase C boundary).
