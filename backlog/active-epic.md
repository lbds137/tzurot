...
- ⏳ **PR-2h…j**: per-area bot-client migrations sequenced by domain (memory / character / voice / wallet / preset / etc.). Burn-down gate enforces strict-monotonic decrease per category. Three backlog items still tracked under this trigger:
...
- ⏳ **PR-2h…i** (~2 PRs): per-area bot-client migrations at 15–25 callsites each. Burn-down gate enforces monotonic decrease per category.
