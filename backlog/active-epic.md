...
3. **2.5c** — cutover behind `CONTEXT_MODE`, sliced 2026-06-04 (user-approved): **(i)** ✅ SHIPPED 2026-06-04 (#1155) — `CONTEXT_MODE` born (`contextWritePath.ts`, legacy default = zero change); service mode flips assistant persist (throws on failure, legacy-identical semantics) + edit/delete sync (never throws) to the 2.5b gateway endpoints; shared payload builders keep authoritative/mirror wire shapes drift-proof; **(ii)** routing-read cutover — HTTP-backed `IPersonalityLoader` + `(nameOrId, userId)` cache with positive (5 min) AND negative (30-60s) entries — REQUIRED because `PersonalityService` skips its cache whenever `userId` is present, so every routing probe is a DB query today and would become an HTTP hop; **(iii)** hydration cutover — thin envelopes, ContextStep hydrates in-job (persona resolution + `getOrCreateUser` move here), user-message persist + vision update → ai-worker, cross-channel type unification, context-hash diffing (needs its own scoping pass). `CONTEXT_MODE=service` flips whatever has shipped so far — one toggle, incremental meaning. **Perf budget note (PR #1154 review, corrected):** `runSync` makes two DB round-trips (`getMessagesByDiscordIds` + `getMessagesInTimeWindow`) — same count as the pre-relocation SyncExecutor (the reviewer's "shared one fetch" claim was wrong; verified against the old code). Include sync-pass cost in the 2.5c perf budget anyway since the endpoint adds an HTTP hop; `Promise.all` over the two passes is an available micro-win if needed. **Fold-forward nits from #1154 final reviews (apply when touching these files):** (a) `conversationAssistantMessage.ts` — compute `chunkIdsMatch` once into a local + add a comment that order-sensitivity is intentional (DB column preserves send order); (b) `PersistAssistantMessageResponseSchema` → discriminated union on `created` so `matched` is required iff `created: false`.
