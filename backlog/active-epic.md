...
| 3 — scoping matrix activation | pool blend + community consent + encapsulation toggle + fiction flag. **Owner design input (2026-07-10)**: (a) classify facts at extraction time as OBJECTIVE (persona-global — "has a cat named Miso", true with every character) vs RELATIONAL (relationship-experience — "confided X to this character") so the blend policy can widen them differently; the §2.2 matrix covers where a fact is visible but not this distinction. (b) Interim asymmetry to decide before/at Phase 3: `shareLtmAcrossPersonalities` widens EPISODE retrieval but deliberately not FACT retrieval (`ConversationalRAGService` comment) — owner uses the flag; honoring it in `FactRetriever` is a small coherent change if the asymmetry bites first. | evidence-gated |
