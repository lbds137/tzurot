...
- ⏳ **PR-2j**: per-area bot-client migrations sequenced by domain (voice / wallet / preset / remaining 15-25 callsites each). Burn-down gate enforces strict-monotonic decrease per category. Three backlog items still tracked under this trigger:
...
- ⏳ **PR-2j** (~1 PR): voice/wallet/preset + remaining misc bot-client migrations at 15–25 callsites each. Burn-down gate enforces monotonic decrease per category.
- ⏳ **PR-2k**: legacy deletion final pass. Delete `adminApiClient.ts`, `userGatewayClient.ts`, the legacy `/admin /user /internal` mounts in `index.ts`, the `legacy:count` gate, and the baseline file. Counts must be zero at this point.
