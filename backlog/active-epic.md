...
| 1c — retrieval-QUERY sophistication (candidate) | Production embeds the RAW user message verbatim (`MemoryRetriever.ts` → `queryMemories(userMessage,…)`) — naive for chat: short reactive messages carry no retrievable meaning, pronouns don't resolve, the topic lives turns back. Owner instinct 2026-07-12 ("our memory query probably kinda sucks"). Candidate ladder: fold recent turns into the embedded text → LLM query condensation/rewrite → multi-query. Sequenced AFTER the goldens exist precisely so it's a before/after number on realistic messy-message goldens, not vibes; may outrank the hybrid ranking work in payoff. | candidate — gate on goldens |
