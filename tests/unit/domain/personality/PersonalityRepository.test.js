...
describe('PersonalityRepository', () => {
  ...
  describe('unimplemented methods', () => {
    it('should throw error for save', async () => {
      ...
      await expect(repository.save(personality))
        .rejects.toThrow('PersonalityRepository.save() must be implemented');
    });
    ...
    it('should throw error for findById', async () => {
      ...
      await expect(repository.findById(id))
        .rejects.toThrow('PersonalityRepository.findById() must be implemented');
    });
    
    it('should throw error for findByOwner', async () => {
      const ownerId = new UserId('123456789012345678');
      
      await expect(repository.findByOwner(ownerId))
        .rejects.toThrow('PersonalityRepository.findByOwner() must be implemented');
    });
    
    it('should throw error for findAll', async () => {
      await expect(repository.findAll())
        .rejects.toThrow('PersonalityRepository.findAll() must be implemented');
    });
    
    it('should throw error for exists', async () => {
      const id = new PersonalityId('test');
      
      await expect(repository.exists(id))
        .rejects.toThrow('PersonalityRepository.exists() must be implemented');
    });
    ...
    it('should throw error for delete', async () => {
      ...
      await expect(repository.delete(id))
        .rejects.toThrow('PersonalityRepository.delete() must be implemented');
    });
    
    it('should throw error for nextId', async () => {
      await expect(repository.nextId())
        .rejects.toThrow('PersonalityRepository.nextId() must be implemented');
    });
  });
    ...
    class MockPersonalityRepository extends PersonalityRepository {
      ...
      async findByOwner(ownerId) {
        return Array.from(this.personalities.values())
          .filter(p => p.ownerId.equals(ownerId));
      }
