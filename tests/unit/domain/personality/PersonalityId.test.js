...
describe('PersonalityId', () => {
  describe('constructor', () => {
    ...
    it('should reject undefined', () => {
      expect(() => new PersonalityId(undefined)).toThrow('PersonalityId must be a non-empty string');
    });
  ...
  describe('length validation', () => {
    it('should reject IDs shorter than 2 characters', () => {
      expect(() => new PersonalityId('a')).toThrow('PersonalityId must be between 2 and 100 characters');
    });
    
    it('should reject IDs longer than 100 characters', () => {
      const longId = 'a'.repeat(101);
      expect(() => new PersonalityId(longId)).toThrow('PersonalityId must be between 2 and 100 characters');
    });
  ...
  describe('character validation', () => {
    ...
    it('should reject special characters', () => {
      expect(() => new PersonalityId('claude@opus')).toThrow('PersonalityId contains invalid characters');
      expect(() => new PersonalityId('claude#3')).toThrow('PersonalityId contains invalid characters');
      expect(() => new PersonalityId('claude$')).toThrow('PersonalityId contains invalid characters');
      expect(() => new PersonalityId('claude!')).toThrow('PersonalityId contains invalid characters');
    });
  });
  ...
  describe('reserved names', () => {
    ...
    reservedNames.forEach(name => {
      it(`should reject reserved name: ${name}`, () => {
        expect(() => new PersonalityId(name))
          .toThrow(`"${name}" is a reserved personality name`);
      });
      
      it(`should reject reserved name case-insensitive: ${name.toUpperCase()}`, () => {
        expect(() => new PersonalityId(name.toUpperCase()))
          .toThrow(`"${name.toUpperCase()}" is a reserved personality name`);
      });
    });
