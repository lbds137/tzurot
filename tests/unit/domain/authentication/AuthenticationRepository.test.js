...
describe('AuthenticationRepository', () => {
  ...
  describe('unimplemented methods', () => {
    it('should throw error for save', async () => {
      ...
      await expect(repository.save(userAuth))
        .rejects.toThrow('AuthenticationRepository.save() must be implemented');
    });
    ...
    it('should throw error for findByUserId', async () => {
      ...
      await expect(repository.findByUserId(userId))
        .rejects.toThrow('AuthenticationRepository.findByUserId() must be implemented');
    });
    
    it('should throw error for findBlacklisted', async () => {
      await expect(repository.findBlacklisted())
        .rejects.toThrow('AuthenticationRepository.findBlacklisted() must be implemented');
    });
    
    it('should throw error for findExpiredTokens', async () => {
      const expiryDate = new Date();
      
      await expect(repository.findExpiredTokens(expiryDate))
        .rejects.toThrow('AuthenticationRepository.findExpiredTokens() must be implemented');
    });
    ...
    it('should throw error for delete', async () => {
      ...
      await expect(repository.delete(userId))
        .rejects.toThrow('AuthenticationRepository.delete() must be implemented');
    });
    
    it('should throw error for countAuthenticated', async () => {
      await expect(repository.countAuthenticated())
        .rejects.toThrow('AuthenticationRepository.countAuthenticated() must be implemented');
    });
  });
    ...
    class MockAuthenticationRepository extends AuthenticationRepository {
      ...
      async findBlacklisted() {
        return Array.from(this.users.values())
          .filter(userAuth => userAuth.blacklisted);
      }
      ...
      async countAuthenticated() {
        return Array.from(this.users.values())
          .filter(userAuth => userAuth.token && !userAuth.token.isExpired())
          .length;
      }
    }
