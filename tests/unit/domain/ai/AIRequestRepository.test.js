...
describe('AIRequestRepository', () => {
  ...
  describe('unimplemented methods', () => {
    it('should throw error for save', async () => {
      ...
      await expect(repository.save(request))
        .rejects.toThrow('AIRequestRepository.save() must be implemented');
    });
    
    it('should throw error for findById', async () => {
      const requestId = AIRequestId.create();
      
      await expect(repository.findById(requestId))
        .rejects.toThrow('AIRequestRepository.findById() must be implemented');
    });
    
    it('should throw error for findByUser', async () => {
      const userId = new UserId('123456789012345678');
      
      await expect(repository.findByUser(userId))
        .rejects.toThrow('AIRequestRepository.findByUser() must be implemented');
    });
    ...
    it('should throw error for findByPersonality', async () => {
      ...
      await expect(repository.findByPersonality(personalityId))
        .rejects.toThrow('AIRequestRepository.findByPersonality() must be implemented');
    });
    
    it('should throw error for findPending', async () => {
      await expect(repository.findPending())
        .rejects.toThrow('AIRequestRepository.findPending() must be implemented');
    });
    
    it('should throw error for findRetryable', async () => {
      await expect(repository.findRetryable())
        .rejects.toThrow('AIRequestRepository.findRetryable() must be implemented');
    });
    
    it('should throw error for getStatistics', async () => {
      const since = new Date(Date.now() - 24 * 60 * 60 * 1000);
      
      await expect(repository.getStatistics(since))
        .rejects.toThrow('AIRequestRepository.getStatistics() must be implemented');
    });
    
    it('should throw error for cleanup', async () => {
      const before = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      
      await expect(repository.cleanup(before))
        .rejects.toThrow('AIRequestRepository.cleanup() must be implemented');
    });
  });
    ...
    class MockAIRequestRepository extends AIRequestRepository {
      ...
      async findByUser(userId, options = {}) {
        const results = Array.from(this.requests.values())
          .filter(r => r.userId && r.userId.equals(userId));
      ...
      async findByPersonality(personalityId, options = {}) {
        const results = Array.from(this.requests.values())
          .filter(r => r.personalityId && r.personalityId.equals(personalityId));
      ...
      async findRetryable() {
        return Array.from(this.requests.values())
          .filter(r => r.status === 'failed' && r.canRetry());
      }
      
      async getStatistics(since) {
        const requests = Array.from(this.requests.values())
          .filter(r => r.createdAt && new Date(r.createdAt) >= since);
