...
describe('AIRequest', () => {
  ...
  describe('constructor', () => {
    it('should require AIRequestId', () => {
      expect(() => new AIRequest('string-id')).toThrow('AIRequest must be created with AIRequestId');
    });
  ...
  describe('recordResponse', () => {
    ...
    it('should require sent status', () => {
      ...
      expect(() => pendingRequest.recordResponse(AIContent.fromText('Response')))
        .toThrow('Can only record response for sent requests');
    });
  });
  ...
  describe('recordFailure', () => {
    ...
    it('should reject if already completed', () => {
      ...
      expect(() => request.recordFailure(new Error('Late error')))
        .toThrow('Cannot fail completed or failed request');
    });
    
    it('should reject if already failed', () => {
      request.recordFailure(new Error('First error'));
      
      expect(() => request.recordFailure(new Error('Second error')))
        .toThrow('Cannot fail completed or failed request');
    });
  });
