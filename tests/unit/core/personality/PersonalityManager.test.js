...
describe('PersonalityManager Integration Tests', () => {
  ...
  describe('initialize', () => {
    ...
    it('should handle deferred owner personality seeding', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Deferring owner personality seeding to background'
      );
    });
    ...
    it('should handle synchronous owner personality seeding', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Loading owner personalities synchronously'
      );
    ...
    it('should skip seeding when requested', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Skipping owner personality seeding'
      );
    });
  ...
  describe('registerPersonality', () => {
    ...
    it('should register a new personality successfully', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Successfully registered personality: test-personality'
      );
    ...
    it('should fetch and set profile info', async () => {
      ...
      getProfileErrorMessage.mockResolvedValue(
        '*sighs* Something went wrong! ||*(an error has occurred)*||'
      );
    ...
    it('should handle profile fetch errors gracefully', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        '[PersonalityManager] Could not fetch profile info for test-personality: Fetch failed'
      );
  ...
  describe('setPersonalityAlias', () => {
    ...
    it('should skip save when requested', async () => {
      ...
      const result = await personalityManager.setPersonalityAlias(
        'test-alias',
        'test-personality',
        true
      );
  ...
  describe('seedOwnerPersonalities', () => {
    ...
    it('should seed owner personalities from constants', async () => {
      ...
      const hasOwnerMessage = allInfoCalls.some(
        msg => msg && (msg.includes('Owner has') || msg.includes('Seeding complete'))
      );
    ...
    it('should skip seeding if owner has all expected personalities', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Owner has all 2 expected personalities'
      );
    ...
    it('should add only missing personalities when some already exist', async () => {
      ...
      const registerSpy = jest
        .spyOn(personalityManager, 'registerPersonality')
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Owner has 1 personalities, missing 1: test-personality2'
      );
    ...
    it('should skip seeding if no owner configured', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] No bot owner ID configured, skipping seeding'
      );
  ...
  describe('save', () => {
    ...
    it('should handle save errors', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        '[PersonalityPersistence] Failed to save personalities'
      );
    });
  });
  ...
  describe('_setDisplayNameAlias', () => {
    ...
    it('should create smart alias for collisions', async () => {
      ...
      expect(personalityManager.registry.aliases.get('lilith-second')).toBe('lilith-second-part');
      expect(logger.info).toHaveBeenCalledWith(
        '[PersonalityManager] Created alternate alias Lilith-second for lilith-second-part (Lilith was taken)'
      );
    });
    ...
    it('should fall back to random suffix if smart alias is taken', async () => {
      ...
      const anotherAlias = aliasEntries.find(
        ([alias, name]) => name === 'another' && alias.startsWith('test-')
      );
      expect(anotherAlias).toBeDefined();
      expect(anotherAlias[0]).toMatch(/^test-[a-z]{6}$/);
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('[PersonalityManager] Created alternate alias Test-')
      );
    });
  });
});
