...
describe('ConversationTracker', () => {
  ...
  describe('getActivePersonality', () => {
    ...
    it('should return null for mention-only conversation in guild channel', () => {
      ...
      const personality = tracker.getActivePersonality('mentionuser', 'mentionchannel', false, true);
      expect(personality).toBeNull();
    });
    ...
    it('should return null for stale conversation', () => {
      ...
      Date.now = jest.fn()
        .mockReturnValueOnce(Date.now() + 31 * 60 * 1000); // 31 minutes later
    ...
    it('should use extended timeout for DM conversations', () => {
      ...
      Date.now = jest.fn()
        .mockReturnValueOnce(currentTime + 90 * 60 * 1000);
      ...
      Date.now = jest.fn()
        .mockReturnValueOnce(baseTime + 121 * 60 * 1000);
  ...
  describe('loadFromData', () => {
    it('should load conversations and message mappings from data', () => {
      ...
      expect(tracker.activeConversations.get('user1-channel1').personalityName).toBe('Personality1');
    });
