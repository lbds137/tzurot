...
describe('UserTokenManager', () => {
  ...
  describe('exchangeCodeForToken', () => {
    ...
    it('should handle failed exchange with error message', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('[UserTokenManager] Failed to exchange code for token: 400 Bad Request');
    });
    ...
    it('should handle network errors', async () => {
      ...
      expect(token).toBeNull();
      expect(logger.error).toHaveBeenCalledWith('[UserTokenManager] Error exchanging code for token:', error);
    });
  });
  ...
  describe('storeUserToken', () => {
    it('should store token with expiration date', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(`[UserTokenManager] Stored token for user ${userId}`);
    });
  });
  ...
  describe('cleanupExpiredTokens', () => {
    ...
    it('should handle no expired tokens', () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith('[UserTokenManager] No expired tokens found during cleanup');
    });
  });
  ...
  describe('setAllTokens', () => {
    it('should set all tokens and migrate old format', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('[UserTokenManager] Updated 1 tokens with expiration dates');
    });
