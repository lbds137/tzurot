...
describe('CommandLoader Comprehensive Tests', () => {
  ...
  describe('processCommand', () => {
    it('should process a valid command successfully', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'test', ['arg1', 'arg2']);

      expect(logger.info).toHaveBeenCalledWith(
        '[CommandLoader] Processing command: test with args: arg1 arg2 from user: TestUser#1234'
      );
      expect(newCommandSystem.processCommand).toHaveBeenCalledWith(mockMessage, 'test', [
        'arg1',
        'arg2',
      ]);
    ...
    it('should handle command with no arguments', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'ping', []);

      expect(logger.info).toHaveBeenCalledWith(
        '[CommandLoader] Processing command: ping with args:  from user: TestUser#1234'
      );
      expect(newCommandSystem.processCommand).toHaveBeenCalledWith(mockMessage, 'ping', []);
      expect(result).toBe(mockResult);
    });
    ...
    it('should log when command is not found', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'nonexistent', []);
    ...
    it('should handle command system returning undefined', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'undefined-command', []);
    ...
    it('should handle errors and send error message', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'error-command', ['arg']);
    ...
    it('should handle errors when error message fails to send', async () => {
      ...
      await expect(commandLoader.processCommand(mockMessage, 'double-error', [])).rejects.toThrow(
        sendError
      );
    ...
    it('should handle commands with many arguments', async () => {
      ...
      await commandLoader.processCommand(mockMessage, 'multi-arg', manyArgs);
    ...
    it('should handle special characters in arguments', async () => {
      ...
      await commandLoader.processCommand(mockMessage, 'special', specialArgs);
    ...
    it('should handle empty command name', async () => {
      ...
      await commandLoader.processCommand(mockMessage, '', ['arg']);

      expect(logger.info).toHaveBeenCalledWith(
        '[CommandLoader] Processing command:  with args: arg from user: TestUser#1234'
      );
      expect(newCommandSystem.processCommand).toHaveBeenCalledWith(mockMessage, '', ['arg']);
    });

    it('should handle null result from command system', async () => {
      newCommandSystem.processCommand.mockResolvedValueOnce(null);

      const result = await commandLoader.processCommand(mockMessage, 'null-command', []);
    ...
    it('should handle command system throwing string error', async () => {
      ...
      const result = await commandLoader.processCommand(mockMessage, 'string-error', []);
    ...
    it('should preserve original message reference', async () => {
      ...
      await commandLoader.processCommand(mockMessage, 'test', []);
