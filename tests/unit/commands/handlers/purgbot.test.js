...
describe('PurgBot Command', () => {
  ...
  it('should purge system messages by default', async () => {
    ...
    expect(mockDirectSendFunction).toHaveBeenCalledWith(
      '🧹 Purging system and command messages...'
    );
    ...
    expect(mockEmbed.setDescription).toHaveBeenCalledWith(
      expect.stringContaining('Completed purging system and command messages')
    );
    expect(mockEmbed.setFooter).toHaveBeenCalledWith({
      text: expect.stringContaining('self-destruct'),
    });
  ...
  it('should handle message deletion failures', async () => {
    ...
    mockCollection
      .get('auth-msg-1')
      .delete.mockRejectedValueOnce(new Error('Could not delete message'));
