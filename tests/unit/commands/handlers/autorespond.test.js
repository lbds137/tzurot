...
describe('Autorespond Command', () => {
  ...
  it('should show current status when no subcommand is provided', async () => {
    ...
    expect(mockMessage.channel.send).toHaveBeenCalledWith(expect.stringContaining('OFF'));
  });
  ...
  it('should show status with "status" subcommand', async () => {
    ...
    expect(mockMessage.channel.send).toHaveBeenCalledWith(expect.stringContaining('ON'));
  });
