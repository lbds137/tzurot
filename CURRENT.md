...
> **Version**: v3.0.0-beta.128 (released 2026-06-07, #1168) — **hotfix-style release cut from main directly**; develop's in-flight Phase 2.5 work was deliberately excluded, then develop was rebased onto main post-release. Fixes the beta.127 prod breakage (character edit/import failing on `canEdit` validation — `updatePersonality`/`setPersonalityVisibility` handlers drifted from their declared output schemas) and ships the **full contract-enforcement package**: a manifest-conformance harness replaying ALL 145 routes through the real generated mounts over PGLite (found 3 more latent drifts: `getUserChannel`, `getSttDefaultProvider`, `createGlobalPersonality` — fixed manifest-side, none had live callers), a `registry.test.ts` completeness gate (new manifest route without a conformance fixture/skip fails the unit suite), and the typed `sendContractSuccess(res, schema, payload)` helper (compile-time payload pin; drift-class routes migrated). Plus the codecov PyPI install fix. Post-mortem: `docs/incidents/PROJECT_POSTMORTEMS.md` 2026-06-07. **No migrations.**
> **🚧 Release freeze status**: LIFTED. No release in progress. Develop rebased onto main 2026-06-07 (+1 follow-up commit: conformance fixtures for the four Phase-2.5 internal routes the completeness gate caught — the gate working on day one). PR #1167 (develop-targeted twin of the hotfix) closed as superseded.

---

## Next Session Goal

**iii-b-2 SHIPPED (#1183) + thin burn-in DONE on dev — found & fixed one regression (#1184).** The wire contract landed: bot drops the 4 re-derivable fields (`conversationHistory`/`referencedMessages`/`mentionedPersonas`/`referencedChannels`) behind a `kind: 'legacy'|'envelope'` discriminant (enum field + `superRefine`, NOT a `z.discriminatedUnion` — that would throw on in-flight absent-`kind` jobs). `participantGuildInfo` + `extendedContextAttachments` still ship (not re-derivable yet → **iii-b-3**). Also fixed the `isWeighIn` schema drift, added a promoted-path observability log, and folded in the weigh-in STM-reset epoch-cutoff fix (both assembler + legacy `MessageContextBuilder`).

**Thin burn-in on dev (`CONTEXT_THIN_PAYLOAD=true`, still on):** confirmed `kind=envelope` assembling. The smoke testing surfaced a real cross-service regression — **`#1184` (CI-green + LGTM, awaiting merge)**: thin dropped `context.referencedMessages`, but the gateway's `createJobChain` read only that field to spawn vision/transcription jobs for attachments in **replied-to / linked** messages, so a reply/link to an image went undescribed. Fix = `createJobChain` falls back to `rawAssemblyInputs.rawReferencedMessages`. **Lesson (filed to inbox as a `[RESEARCH]` spike):** three green unit suites, one broken seam — `jobChainOrchestrator` HAD a referenced-attachment test but only for the fat shape. Want deterministic test-quality gates (mutation testing + a job-payload contract layer), not attention-dependent rules.

**Remaining in the context-relocation epic (2.5):** **iii-b-3** (grow the envelope's guild-info raw form → drop `participantGuildInfo` + `extendedContextAttachments`; possibly split 3a/3b) → **iii-cleanup** (5 carried #1156 fold-forwards) → **2.5d** (delete legacy + `MessageContextBuilder` + bot-client Prisma + all `CONTEXT_*` flags; tighten depcruise) → unblocks PR-2p.
