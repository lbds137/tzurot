...
**Next: Phase 2 PR 2.1** (`StructuredHistoryEntry` typed IR — spec derives from §9c; design settled). TASK-718 SHIPPED (#2178, 6 review rounds, cap hit on the last, residual polish filed as TASK-722) — nested dispatch is now the codified Fable-driver standard. **PR 2.1 spec decisions, settled pre-compaction**: (1) rename `RawHistoryEntry` → `StructuredHistoryEntry` in place in `conversationTypes.ts` (16 importers, mechanical; no relocation, no alias); (2) `ConversationContext.rawConversationHistory` becomes `StructuredHistoryEntry[]`, migrating the inline copy's load-bearing field docs onto the canonical type; (3) `PromptHistorySource` (ContextStep) becomes the type itself per TASK-683's fix shape; (4) byte-parity proof = behavior-inert diff (types/imports/comments only, zero runtime-expression changes) + full suites — the serializer (`ContextWindowManager.selectAndSerializeHistory`) and the memory-dedup pre-pass (`ContentBudgetManager.preselectHistory`) land on the IR via the rename, satisfying §9c's first-consumer clause; (5) `ConversationMessage` (common-types, Date-typed, producer `mapToConversationMessage` in conversation-history) stays separate — the IR is the post-normalization prompt-side boundary, stated in its docstring.
