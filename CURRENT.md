...
_None. PR #927 (z.ai auto-promotion + URL encoding fix) merged 2026-04-27/28 after 5 review rounds + council pre-review (Opus 4.6). User reported on dev that with key set + preset configured `provider: 'openrouter'` + `model: 'z-ai/glm-5.1'`, the request still routed via OpenRouter (not their z.ai quota). Plus a URL encoding regression from PR #926 (`encodeURIComponent('/')` → `%2F` produced 404 footer links). Fixes: ProviderRouter now auto-promotes any OpenRouter `z-ai/<model>` request to z.ai-direct when the user has a zai-coding key (single-preset UX, no duplicate presets needed); URL encoder now uses per-segment encoding to preserve path slashes; path-traversal defense escapes `.`/`..` segments per `00-critical.md`. Whitelist guards against z.ai catalog drift; `wasAutoPromoted` field plumbed for the upcoming retry-with-fallback (C3 — see `backlog/inbox.md`). **Next**: manual end-to-end verification on dev that the promotion fires (logs should show `wasAutoPromoted=true`, footer should point to `z.ai/blog/glm-5`, z.ai dashboard quota should consume), then C3 (retry-with-fallback) before next release per user's preference._
