...
**Unreleased since beta.140** (7 PRs to develop, no migrations):
...
- **#1372 — build(api-gateway): pre-deploy migration command** (the breaking-migration deploy-window fix): Railway `preDeployCommand: npx -y prisma@7 migrate deploy` + Dockerfile `COPY prisma.config.ts` + `PREDEPLOY_MIGRATIONS.md`. **⚠️ Merged to develop to VALIDATE on the dev deploy** — the pre-deploy command only fires on a real Railway deploy, so merge-to-dev IS the validation path (no local dry-run exists). Watch the dev api-gateway deploy logs for the `prisma migrate deploy` step; a failed pre-deploy BLOCKS the deploy (recoverable by pushing a fix/revert to develop). Post-validation: reflect the auto-migrate behavior across the manual-migrate docs/rules/skill (tracked in `cold/follow-ups.md`).

**➡️ Next: vision-epic follow-up release** — remaining open items (all `backlog/cold/`): **#4 `getById` `kind:'text'` gate** → Phase 2 (the only release-review finding still open — #2 resolved in #1371; `_merge_overlap` / 404 / phantom all shipped above); the **post-validation doc/rule drift** (reflect api-gateway auto-migrate once #1372 is confirmed on dev) and the **multi-service expand/contract** caveat (both `cold/follow-ups.md`). Then **Phase 2** (vision editing surface). `active-epic.md` holds the Model Configuration Overhaul epic (`cold/themes/model-configuration-overhaul.md`).
