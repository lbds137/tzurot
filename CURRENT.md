...
**Active epic: "Slim common-types" (PR-2p) — extraction arc DONE, in close-out sweep.** All 4 packages extracted (config-resolver #1295, identity #1299, conversation-history #1300, cache-invalidation #1302); **Cluster A COMPLETE** (#1304 + #1305); **Cluster B triaged + batched B1–B6**; **B6-#18 SHIPPED via #1306** (transport `kind` discriminant); **B6-#11 SHIPPED via #1314** (extended-context voice transcripts — DB-first/STT-fallback worker re-derive). **➡️ Immediate next: the cache-duration audit + voice-cache-key fix** (user-approved scope 2026-06-23) — fix the `VoiceTranscriptCache` volatile signed-URL key (stable key like `VisionDescriptionCache` + TTL bump), bound the unbounded `BaseConfigResolver` cache, and write a `docs/reference/caching/` duration-rationale doc. Then the remaining epic close-out: **B1–B5** (mechanical context-relocation), **Cluster C** (3 test-infra), the **tooling-coverage** follow-up — full list in [`backlog/cold/epic-log.md`](backlog/cold/epic-log.md). After the clusters: the queued **dependency PRs**.
