...
**Unreleased since beta.141** (8 PRs, no migrations): **#1376 — S1** (kind-aware backend) + **#1377 — S1b** (capability validation: `ModelCapabilityService` + unified shape + fail-closed vision gate) + **#1378 — S2a** (route kind-threading: `?kind=` LIST scoping; admin by-id `requireKind: kind` → global vision default settable; user by-id kind-agnostic) + **#1379 — S2b** (vision-aware override writes: `model-override.ts` branches on kind → per-personality / default vision overrides write `defaultVisionConfigId` / `visionConfigId`) + **#1380 — S2c** (set commands: `kind: Text|Vision` on `/preset create`, `/preset global default|free-default`, `/settings preset set|set-default`) + **#1381 — S2d** (clear commands: `kind` on `/settings preset clear` + `clear-default`) + **#1382 — S2e** (browse vision-aware: `/preset browse` + `/settings preset browse` surface vision configs/overrides 👁-badged — initial two-call merge) + **#1383 — S2f** (single-call browse: `kind` first-class in the list contract + `?kind=all` sentinel → one fetch not two; orthogonal `kind` browse option; pagination followUp; **ack-first fix** for stale pre-deploy customIds). **Vision config management is fully reachable from Discord** — create + set + clear + **browse**, text AND vision, admin AND user. **Phase 2's user-facing work is COMPLETE** (S3 admin dashboard deferred).

**➡️ Next: the user wants the Model-Config epic RELEASED** (develop→main; needs explicit user approval — the one hard exception to standing merge perms). **8 unreleased PRs on develop, no migrations.** Before the release, verify the warm-Redis-cache deploy note (`cold/follow-ups.md` — `kind`-required schema bump). **Then pending: structural enforcement of the deferUpdate-ack-first class** (S2f's squash-review caught a real 3s-rule regression 3 rounds missed; council split — ESLint rule vs Declarative-Ack dispatcher — user decision pending). Open follow-ups (`cold/follow-ups.md`): **manifest `'all'` sentinel** + **take:100 truncation** + **warm-cache deploy check** (all S2f review) · **`DEFAULT_CONFIG_KIND` sweep (8 sites)** · **`CONFIG_KIND_OPTION_DESCRIPTION` dedup** + **`as ConfigKind` runtime guard** · **`MAINTENANCE_MODE` for destructive migrations** · **vision cache-invalidation**. New idea (`cold/ideas.md`): **in-character error delivery** (cross-ref the gateway-hang prod issue in `now.md`).
