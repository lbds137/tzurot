...
2. **TASK-748 NEXT** (owner directive): key-aware vision fallback tier order — grounding in the task file (`composeVisionTiers` stable-sort, `authOptions` carries the key signal, guest path self-corrects via broad-free-fallback). Small; decide inline-vs-dispatch at build.
