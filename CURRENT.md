...
> **Version**: v3.0.0-beta.142 (released 2026-06-30) — **additive migration** (`AdminSettings` default pointers; premigrated to prod _before_ the release merge per the additive-window rule). Headliners: **capability-driven model config** — vision is now a model **capability** (`supportsVision`, OpenRouter-authoritative → z.ai catalog → fail-closed), not a `kind` discriminator, so any vision-capable model works for image input; **full vision-preset management from Discord** (create / set / clear / **browse**, text + vision, admin + user, 👁 badges on vision-capable models); the preset `kind` option became `slot` (Chat/Vision) on setters + `capability` on browse, and `/preset create` dropped it (capability comes from the model); global/free defaults moved to four `AdminSettings` per-slot FK pointers (`{global,free}Default{Llm,Vision}ConfigId`, `onDelete:SetNull`). Also: forwarded-message-text fix (#1391/#1393), slot-correct setters (#1396), list badges/ordering from live pointers not stale booleans (#1394), TTS 300s budget for serverless cold-start (#1389), sharp 0.35.2 type adapt (#1386). **Mechanics**: premigrate-before-merge (additive) → rebase-merge (64 commits) → `release:finalize` SHA-aligned develop; holistic review re-ran each cycle, final verdict clean (every finding tracked/non-blocking). Three dev-smoke fixes folded in pre-merge: clear-both-slots (#1399), kind-scoped edit collision + verified-kind validation (#1400), clear-default fallback reads the pointer not the stale boolean (#1401). _Prior: v3.0.0-beta.141 (2026-06-28) — weigh-in prod-bug fix (#1368) + free-default kind-scope fix (#1374) + `release:premigrate` tooling (#1373)._
