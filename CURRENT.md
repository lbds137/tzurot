...
**Unreleased since beta.141** (4 PRs, no migrations): **#1376 — S1** (kind-aware backend) + **#1377 — S1b** (capability validation: `ModelCapabilityService` + unified shape + fail-closed vision gate) + **#1378 — S2a** (route kind-threading: `?kind=` LIST scoping; admin by-id `requireKind: kind` → global vision default settable; user by-id kind-agnostic) + **#1379 — S2b** (vision-aware override writes: `model-override.ts` branches on kind → user default / per-personality vision overrides write `defaultVisionConfigId` / `visionConfigId`). **Backend is now feature-complete for vision config management** via the internal API; no Discord-facing change yet (commands land in S2c).

**➡️ Next: Model Configuration Overhaul — Phase 2 / S2c (commands)** (see [`active-epic.md`](backlog/active-epic.md)). S0 (beta.141) + S1 + S1b + S2a + S2b done. **S2c** = the `kind` command option on the four default-setters + `/preset create` + autocomplete/browse kind-filtering — exposing the now-complete backend to users; then **S3** dashboard (deferred). Open follow-ups (`cold/follow-ups.md`): **`MAINTENANCE_MODE` for destructive migrations**; **personality-level default config write-routes**; **vision cache-invalidation** (when `VisionConfigResolver` gets a cache); **STT cross-boundary repetition** (runtime-confirmed Voxtral-side, not our code). _(The TTL "nit" from the beta.141 review was a verify-before-accepting false positive — dropped.)_
