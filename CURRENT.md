...
**beta.206 follow-up scoping (agent recommendation 2026-08-21, one item AWAITING OWNER):** TASK-710 IN — size:S, in-theme, and TASK-708 opens the same file next. TASK-711 OUT of the release gate — `area:repo` process tooling, non-runtime, rides the next `.claude` PR. **TASK-712 — OWNER DECISION PENDING.** Recommendation: include and port the gate, because after #2167 the two paths disagree about the same data reaching the same consumer and "the bot-cache gate is enough" has never been argued. Not done unilaterally: tightening it changes what the model sees on flows that work today. Range at the time of writing: 2 runtime PRs / 40 files — far from both cut triggers. Watch item: 708(×2)+43+667+563+700+doc-17 Phase 2 (+710/712) is 10–12 runtime PRs, so the ~10 backstop likely fires BEFORE Phase 2 lands; Phase 2 may want its own train.
