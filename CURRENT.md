...
## ▶️ NEXT — beta.208 theme work

**doc-12 P1** (discoverability report SQL over `command_events` — rows accumulating in prod since beta.207) is the cycle's build. Owner-taste queue: TASK-764 (/deny add UX redesign) · TASK-765 (thread-only denials). Usage note: owner reported 40% Fable at ~2.3 days into the week (2026-08-25); ledger seeded; the flip-to-Opus-driver lever stays available on their word.
