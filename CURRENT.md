...
**Unreleased since beta.141** (5 PRs, no migrations): **#1376 — S1** (kind-aware backend) + **#1377 — S1b** (capability validation: `ModelCapabilityService` + unified shape + fail-closed vision gate) + **#1378 — S2a** (route kind-threading: `?kind=` LIST scoping; admin by-id `requireKind: kind` → global vision default settable; user by-id kind-agnostic) + **#1379 — S2b** (vision-aware override writes: `model-override.ts` branches on kind → user default / per-personality vision overrides write `defaultVisionConfigId` / `visionConfigId`) + **#1380 — S2c** (commands: `kind: Text|Vision` option on `/preset create`, `/preset global default|free-default`, `/settings preset set|set-default` — scopes the autocomplete + the write; kind threads the create modal via the custom-ID's 3rd segment). **Vision config management is now reachable from Discord** (admin global vision default + user vision default/per-personality overrides + vision preset create), exposing the S1–S2b backend end-to-end.

**➡️ Next: Model Configuration Overhaul — Phase 2 / S2d (clear commands)** (see [`active-epic.md`](backlog/active-epic.md)). S0 (beta.141) + S1 + S1b + S2a + S2b + S2c done. **S2d** = `kind` option on `/settings preset clear` + `clear-default` so a vision override/default can be cleared (bot-client-only: `?kind=` on `deleteModelOverride`/`clearDefaultModelConfig` client methods + handler threading; backend clear routes already accept `?kind=` from S2b); then **S3** dashboard (deferred). Open follow-ups (`cold/follow-ups.md`): **`CONFIG_KIND_OPTION_DESCRIPTION` dedup** + **`as ConfigKind` runtime guard** (both from S2c review) · **`MAINTENANCE_MODE` for destructive migrations** · **personality-level default config write-routes** · **vision cache-invalidation** (when `VisionConfigResolver` gets a cache) · **STT cross-boundary repetition** (runtime-confirmed Voxtral-side, not our code).
