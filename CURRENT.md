...
**iii-b-1 SHIPPED (#1182, 2026-06-09) — now the dev burn-in for it.** The worker now drives the prompt from `assembleCore()` behind `CONTEXT_ASSEMBLY_PROMOTE` (reversible; bot unchanged). Weigh-in guard landed (null output persona, keep persona-scoped epoch — corrected the earlier backwards burn-in note). The earlier shadow burn-in (2026-06-08, 8/11 all-12-surfaces green; results in `active-epic.md` § "Burn-in results") validated the _surfaces_; **next is the b-1 burn-in of the promoted path**: flip `CONTEXT_ASSEMBLY_PROMOTE=true` on dev, run the smoke matrix, compare flag-off vs flag-on _generations_ (esp. Fork-C vision-timing — now load-bearing — and weigh-in). Then **iii-b-2** = discriminated-union thin payload (bot stops shipping legacy fields; resolve participantGuildInfo as envelope metadata), carrying 3 fold-forwards from the #1182 review (contextFlags.ts move; cross-channel promote test; drop the stray `async`). Then **2.5d** deletion → PR-2p.
