...
**Active epic**: **Route Manifest Scaffold + Typed-Client Codegen** ([active-epic.md](backlog/active-epic.md)) — Phase 3 effectively complete. **PR-1.5f shipped to develop** (PR #1100, merged 2026-05-26) — declared the 13 memory routes in the manifest: handler-export refactor for all `/user/memory/*` to `(deps: RouteDeps) => RequestHandler` shape; new `MemoryActionTokenService.peek*` companions to enable peek-validate-consume; `memoryBatchHelpers.ts` extracted to keep memoryBatch.ts under the line cap; `asyncHandler` returns `Promise<void>` so multi-await tests can actually await internal microtasks; 12 new response schemas + 37 contract tests; 13 new manifest entries; 13 new `PATH_MAP` entries. Manifest 128 → 141 routes (104 user, +13). 4 commits, 2 review rounds; round 1's two items applied inline, round 2 LGTM with date-schema nits filed to quick-wins. (PR-1.5e shipped PR #1099 / 2026-05-26: preview-token handshake + idempotent PUT lock + peek-validate-consume.)

**Next under the epic**:

1. **PR-1.5g: admin/diagnostic audience lift + absorb three open quick-wins** — 5 admin-tier diagnostic routes lift to user audience with `acceptsSubject: true` (4 of 9 already lifted in PR-1.5c; these 5 are the more sensitive ones). Bundle in: purge phrase case-sensitivity unify (~1 LOC), batchDelete `_` → `::` customIds (~6 LOC), and the date-schema `.datetime()` tightening from PR-1.5f round-2 review (~8 LOC + codegen regen). ~165 LOC total. Branch: `feat/pr-1.5g-diagnostic-lift`.
2. **PR-2 (future): route-prefix cutover** — atomic switch from legacy `/admin /user /internal` mounts to generated `mountAdminRoutes/mountUserRoutes/mountInternalRoutes` + migrate all 310 bot-client call sites (103 `adminFetch` + 207 `callGatewayApi`) + delete `adminApiClient.ts`/`userGatewayClient.ts`. Single deploy-gap PR. Three backlog items already filed under this trigger (wallet rate-limiter middleware, common-types export-count audit, /wallet/set typed-client migration).
