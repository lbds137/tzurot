...
**Dev-smoke follow-up (filed to `cold/follow-ups.md`):** the user-disambiguation surfaces dev-bot-relayed input under the bot's webhook name (`Lila (@Rotzot · תשב)`) — those relayed messages carry no `personaId` and the bot's webhook name as `discordUsername`, so when a persona name collides with a character name the same human fragments into two identities. Pre-existing (not from the beta.138 PRs); surfaced because #1349's name-based collision check now fires where the old displayName-based one didn't. Not a release blocker; the user's rename removed the collision.

_Nothing else unreleased on develop yet._
