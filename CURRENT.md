...
**Unreleased since beta.141** (3 PRs, no migrations): **#1376 — S1** (kind-aware LLM config backend) + **#1377 — S1b** (capability validation: `ModelCapabilityService` + unified `ModelCapabilities` shape + fail-closed vision gate) + **#1378 — S2a** (route kind-threading: `?kind=` LIST scoping via `parseConfigKindQuery`; admin by-id `requireKind: kind` — `?kind=vision` now sets the global vision default; user by-id kept kind-agnostic with collision-from-row). No user-facing change yet (routes default text; commands land in S2c); but an admin can now set a global vision default via the internal API.

**➡️ Next: Model Configuration Overhaul — Phase 2 / S2b** (see [`active-epic.md`](backlog/active-epic.md)). S0 (beta.141) + S1 (#1376) + S1b (#1377) + S2a (#1378) done. **S2b** = vision-aware override writes in `model-override.ts` (branch on config kind → write `defaultVisionConfigId` / `visionConfigId` when kind=vision; read paths gain `?kind=`); then **S2c** commands (`kind` option + autocomplete filtering), **S3** dashboard (deferred). Open follow-ups (`cold/follow-ups.md`): **`MAINTENANCE_MODE` for destructive migrations**; **personality-level default config write-routes** (no write route for text OR vision today). _(The TTL "nit" from the beta.141 review was a verify-before-accepting false positive — dropped.)_
