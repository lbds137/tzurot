...
> **Version**: v3.0.0-beta.139 (released 2026-06-26) — **no migrations.** Headliners: **dead-image latency fix** — an expired Discord CDN image sitting in a channel's context no longer re-storms across OpenRouter providers every turn (~100s/turn eliminated); the reference path now honors attachment-bound cached failures while still re-attempting transient ones (#1359). **Relay-echo identity recovery** — a persona-name/character-name collision no longer fragments one human into two chat-log identities (#1352). **Voice-engine response validation** — the unsafe `as` casts in `VoiceEngineClient` are replaced with Zod schemas, so a Python-side drift throws clearly instead of leaking `undefined` (#1358). Plus the **Test-Pyramid Phase 4 contract tier** — BullMQ job-chain contract, bot→worker envelope scenarios, the voice-engine cross-language (Python↔TS) contract, a code-derived coverage topology + execution-check ratchet + CI drift gate, and the epic close-out (#1353/#1354/#1356/#1357/#1358). **Mechanics**: 19-commit release; clean rebase-merge + `release:finalize` SHA-aligned develop; 1 holistic-review round (no blockers, 4 follow-ups → backlog). _Prior: v3.0.0-beta.138 (2026-06-26) — z.ai fast-fallback, chat-log fixes, footer attribution, conversation-persist fast pool, Test-Pyramid Phase 1–4 (PR1–2)._
> **🚧 Release freeze status**: LIFTED. No release in progress.

---

## Unreleased on Develop (since beta.139)
