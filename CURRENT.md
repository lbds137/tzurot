...
- [ ] **1. Telemetry row lands** (needs-smoke: first runtime exercise of the new insert path): run any slash command on dev → one row in `command_events` with the right dotted command name, outcome `ok`, sane latency. Check: `pnpm ops run --env dev npx prisma studio` or a psql query on occurred_at DESC LIMIT 5. Masking: none (fresh table).
- [ ] **2. Error embed posts** (needs-smoke: first runtime post through the reporter): force any command error on dev (e.g. a character command against a nonexistent slug that throws, or temporarily break a config) → one 🚨 embed in the FEEDBACK_CHANNEL_ID channel with source/stack-hash fields and NO message text. Failure shape: nothing posts (check bot-client logs for 'Error-channel reporter not initialized' or 'Failed to post').
- [ ] **3. Dedup suppresses** (needs-smoke: window logic under real timers): repeat the SAME error within a few minutes → no second embed. A DIFFERENT error still posts. Report: pass/fail per item is enough.

Cut when: smoke passes + owner approves the release PR (always explicit). Prod migration is additive → standard `release:premigrate` order. Release notes: 15 PRs (13 runtime) per `release:range`; 207 files (under both backstops, cut is plan-triggered).
