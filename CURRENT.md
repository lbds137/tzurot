...
- **tooling:** triaged `commands:audit`'s first-run warnings 13 → 4 — fixed the stub-description regex false-positive (legit "Test …" descriptions) and whitelisted 6 deliberate domain verbs (`get`/`set-default`/`clear-default`/`forget`/`auth`/`logout`). (#1210)
- **bot-client + api-gateway:** `/models` browse v2 part 3 — curation. Pins models used by Tzurot's global presets to the top of the list (📌), with the active sort applied independently within both the pinned and non-pinned tiers; preset set fetched via a 3-way concurrent read that degrades to no-pinning on failure. Folds in two carried nits: a 🔀 meta-router badge (plumbed `isRouter` end-to-end after fixing `top_provider.context_length` to `number | null`), and the `both`-source card footer now naming both routes. Completes the v2 arc (card → sort → curation). (#1218)
- **bot-client + api-gateway:** `/models` browse v2 part 2 — sort modes. Toggle cycles default (usable-first) → price (cheapest, no-$ entries last) → recent (newest by OpenRouter `created`, undated entries last). Sort is encoded in the customId so pagination/select rebuild it statelessly, and the active sort shows in the embed. Plumbed `created` (Unix seconds) through `OpenRouterModelCache.toAutocompleteOption` → `ModelAutocompleteOption` (+ Zod schema) → `CatalogModel`. (#1217)
- **bot-client:** `/models` card redesign (v2 part 1) — council-reviewed "spec sheet": usability traffic-light color (green=runnable / orange=needs key), provider author line, hyperlinked title, `slug` + capability-emoji + bold-status description, short inline Context·Price·Access fields, masked links, source/unit footer. No thumbnail. (#1214)
- **api-gateway + bot-client:** fixed `/models` showing only the 6 static z.ai models on dev — `fetchModels` was a raw `fetch()` missing `X-Service-Auth`, rejected by `requireServiceAuth`. Migrated `/models` into the typed-client system (`GET /api/internal/models`, reuses `OpenRouterModelCache`) so auth is automatic; deleted the root `/models` router (dead `/refresh` dropped). Added `gatewayAccessGuard.test.ts` — fails CI if any bot-client file reads `GATEWAY_URL` outside the transport/infra allow-list, enforcing "typed clients only." (#1213)
- **bot-client + api-gateway:** `/models` command — user-aware model browser. `/models browse` (paginated, capability/search-filtered, usable-first, select → card) + `/models view <model>` (autocomplete → card). Each card shows capabilities/context/pricing/z.ai-support and **whether the requesting user can run it** (free / usable / needs OpenRouter key / needs z.ai key / needs-either), computed client-side from their wallet. Merges the OpenRouter list with the static z.ai catalog (z.ai-only models like glm-5.2 discoverable; on-OR ones deduped). Raised the gateway `/models` limit 100→1000 so all ~337 models show (z.ai names sort last alphabetically, were cut by the old cap). New `Models` /help category. (#1212)
- **bot-client:** renamed the `/preset global default` + `free-default` option `config`→`preset` (it selects a preset) — resolves the option-name-drift warning; restructured the autocomplete to discriminate user-vs-global presets by subcommand group instead of option name. `commands:audit` now at 3 standing warnings (2 `list`→`browse` backlogged, 1 accepted `type` drift). (#1211)
