...
**Unreleased since beta.141** (6 PRs, no migrations): **#1376 — S1** (kind-aware backend) + **#1377 — S1b** (capability validation: `ModelCapabilityService` + unified shape + fail-closed vision gate) + **#1378 — S2a** (route kind-threading: `?kind=` LIST scoping; admin by-id `requireKind: kind` → global vision default settable; user by-id kind-agnostic) + **#1379 — S2b** (vision-aware override writes: `model-override.ts` branches on kind → user default / per-personality vision overrides write `defaultVisionConfigId` / `visionConfigId`) + **#1380 — S2c** (set commands: `kind: Text|Vision` on `/preset create`, `/preset global default|free-default`, `/settings preset set|set-default`) + **#1381 — S2d** (clear commands: `kind` on `/settings preset clear` + `clear-default` — handlers read+send kind since clearing is kind-specific). **Vision config management is fully reachable from Discord** — set AND clear, text AND vision, admin AND user — exposing the S1–S2b backend end-to-end. **Phase 2's user-facing work is complete** (S3 admin dashboard deferred).

**➡️ Next: a release of the stack, or promote a new theme** (see [`cold/queue.md`](backlog/cold/queue.md)). The Model Config epic's shippable work is done (S0 beta.141 + S1/S1b/S2a/S2b/S2c/S2d on develop; **S3 dashboard deferred** — Discord modals fight a model-picker UI). **6 unreleased PRs on develop, no migrations** — a release is due when the user calls it. Open follow-ups (`cold/follow-ups.md`): **`DEFAULT_CONFIG_KIND` sweep (8 sites)** + **browse list/delete kind-coupling** (both S2d review) · **`CONFIG_KIND_OPTION_DESCRIPTION` dedup** + **`as ConfigKind` runtime guard** (S2c review) · **`MAINTENANCE_MODE` for destructive migrations** · **personality-level default config write-routes** · **vision cache-invalidation** (when `VisionConfigResolver` gets a cache) · **STT cross-boundary repetition** (Voxtral-side). New idea (`cold/ideas.md`): **in-character error delivery** (user directive — errors should come from the character, not system notices; cross-ref the gateway-hang prod issue in `now.md`).
