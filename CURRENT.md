...
**Merged**: **#2289** TASK-847+846 (single-flight subscribe + error listener; five rounds — round 2 found a third shutdown race, round 4 moved the listener before the handshake; residue TASK-858) · **#2290** rules economy pass (owner call: trim before the drain batches; −10.3k bytes, five rounds restored ~1.3k of over-cuts). **#2291** Batch 1 hooks (four rounds; each of the first three closed a real hole in the new tracker-write guard — quoted wrappers, `cd`-fallthrough ordering, escaped separators — the probe suite grew 38 → 52). **Open**: **#2292** Batch 2 tooling/release (round 2 pushed: the dev gate's unreachable split, including the rejected-promise path). **Archived**: TASK-699 (duplicate of 818), TASK-595 (option (b) had shipped under TASK-459 before it was filed; prose remainder → TASK-859). **No PR**: auto-memory 77k → 19.6k bytes (26 → 10 files).
