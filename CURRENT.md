...
**Unreleased since beta.141** (2 PRs, no migrations): **#1376 — model-config Phase 2 / S1** (kind-aware LLM config backend: service + schema kind-parametrized, `getById` kind-gate, clone-name walk extracted) + **#1377 — Phase 2 / S1b** (capability validation: `ModelCapabilityService` over a unified, provider-agnostic `ModelCapabilities` shape — OpenRouter authoritative → z.ai catalog → null; fail-closed vision gate in `validateLlmConfigModelFields`; create-path name-collision now kind-scoped). No user-facing change yet (routes still default text); the vision editing surface lands in S2.

**➡️ Next: Model Configuration Overhaul — Phase 2 / S2** (see [`active-epic.md`](backlog/active-epic.md)). S0 (beta.141) + S1 (#1376) + S1b (#1377) done. **S2** = the deferred route-layer kind threading (`list` / `getById` / `requireKind` / update-path `ensureNoNameCollision` via a `?kind=` param — required before any vision route goes live) **+ commands** (`kind` option on default-setters + `/preset create` + autocomplete/browse kind-filtering + vision-aware write paths); then **S3** dashboard (deferred). Open follow-up (`cold/follow-ups.md`): **`MAINTENANCE_MODE` for destructive migrations** (promote when one's planned). _(The TTL "nit" from the beta.141 review was a verify-before-accepting false positive — dropped.)_
