...
**Done:** TASK-706 (#2166), TASK-668 (#2167) and TASK-708 PR 1 (#2168), all merged 2026-08-21. Forwards now carry attribution, their origin channel, and no longer leak our own `-#` footers into the prompt. All three tasks stay OPEN — each has one acceptance clause that is runtime-unverified; see the smoke queue below.

**Next, in order:** TASK-710 (spec drafted — extract the private-thread membership half ONLY; probe the discord.js channel type with a positive control rather than hand-rolling a structural type) → TASK-712 (owner-approved option (a); the async ripple is measured on the task — four sync methods, stopping at an already-async caller, so build the full gate) → TASK-43 (**re-scope first**: the "MessageSnapshot strips mentions" blocker is false at the type level; the recorded next step is a runtime probe, not an implementation) → TASK-667 (binding decision recorded: derive the synthetic name from embed index + slot) → TASK-563 (parked branch is on origin and safe; 1 ahead / ~468 behind, so rebase).

**TASK-708 PR 2 is an OWNER DECISION, not queued work.** Agent recommends DECLINING it on merit (argued on the task): the prescribed authorship gate cannot separate the two paths that emit a bold `**Name:** ` prefix — we wrote both, and for the `/chat` relay echo the name is the USER's, so stripping deletes the only attribution the text carries. Build nothing for 708 until this is ruled.

**beta.206 follow-up scoping (agent recommendation 2026-08-21, one item AWAITING OWNER):** TASK-710 IN — size:S, in-theme, and TASK-708 opens the same file next. TASK-711 OUT of the release gate — `area:repo` process tooling, non-runtime, rides the next `.claude` PR. **TASK-712 IN — owner approved 2026-08-21, resolving to option (a): port the gate** so `buildForwardMarker` agrees with `resolveOriginChannelName` on fail-closed. Options (b)/(c) are closed. **Sequence it AFTER TASK-710** — 710 extracts the shared membership helper, and doing 712 first writes a third copy for 710 to collapse. One thing to settle at build time, recorded on the task: `buildForwardMarker` is synchronous and the private-thread check is not. Range re-derived after #2168 via `pnpm ops release:range`: **3 runtime PRs / 61 files** — far from both cut triggers (~10 runtime PRs, ~250 files). Watch item: 708(×2)+43+667+563+700+doc-17 Phase 2 (+710/712) is 10–12 runtime PRs, so the ~10 backstop likely fires BEFORE Phase 2 lands; Phase 2 may want its own train.
