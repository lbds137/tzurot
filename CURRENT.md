...
**Phase 2 PR 2.1 SHIPPED** (#2179, merged `78beaae11`, 3 review rounds, zero blocking findings; built via nested dispatch): `StructuredHistoryEntry` is now the single prompt-side history-row declaration — TASK-683 closed (all three enumerated copies folded, plus RAGUtils' local copy and `extractRecentHistoryWindow`'s inline shape found in review; surviving narrows are `Pick<>`-derived) — and the false "unique name" claim class was swept repo-wide (only the mapper's debunk line survives, by design). TASK-718 SHIPPED earlier (#2178) — nested dispatch is the codified Fable-driver standard. **PR 2.2 SKIPPED** (decided 2026-08-21, recorded in §9c row 2.2): channel identity already lives at the group level (`crossChannelHistoryGroupSchema.channelEnvironment`, Zod-gated), the cross-channel row shape is a structural subset of the IR, and the own-user-role-message move is inseparable from the flagged message-array reshape — it ships as 2.3's first bullet. **Next: PR 2.3 BUILD** (probe precondition cleared 2026-08-21 — both providers accept consecutive user messages, recorded beside §9c Q1). **Spec settled, 8 decisions:** (1) flag `realMessagesEnabled`, systemSettings default false, `getSystemSetting` live-read (rosterBlurbEnabled pattern); (2) branch in `ContentBudgetManager.allocate` — flag-on additionally returns `historyMessages: BaseMessage[]` + `crossChannelMessage?: HumanMessage`, system message renders with empty serializedHistory (chat_log section omits naturally), `ConversationalRAGService.invokeModelAndClean` composes `[system, crossChannel?, ...history, current]`; flag-off byte-identical, pinned by test; (3) rendering — assistant = responder rows (`resolveAssistantRowRole` id-match) with raw content + minimal timestamp, everyone else user-role with `[Name — <promptTime absolute>]` header, kwargs `{speakerId, isAi, discordMessageId}`, NO same-role merging, time gaps as a line in the next message's header zone, entry BODY (quotes/dedup stubs, image descs, embeds, voice, reactions, forwards) via ONE shared renderer extracted from the XML entry serializer (XML keeps envelope, real path takes body); (4) cross-channel `<prior_conversations>` XML unchanged as its own HumanMessage before current-channel; (5) §2.4 in 2.3 = existing dedup-stub semantics carried through the shared renderer + flag-gated stub wording (no "chat log" pointer when the section is absent), contextual_references verified out-of-window-only; (6) S0 header-leakage guard + participants roster reframing, both flag-gated; (7) measurement reuses `measureHistoryEntryTokens` (XML-form over-measure, safe direction; revisit in 2.4); (8) plumbing — `selectAndSerializeHistory` return gains `crossChannelXml`, `PreselectedHistory` gains `selectedEntries` + `crossChannelXml` (flag-neutral). NOT in 2.3: fetch hysteresis (carries), §2.5 eviction (2.4), flag flip (2.5), persistence changes, `ConversationHistoryEntry` fold (carried board item). Round-3 reviewer observation to carry into whichever Phase 2 slice next touches the wire shape: `ConversationHistoryEntry` (pipeline/types.ts) is a natural later fold onto Pick/Omit of the IR. Then PR 2.3 (flagged real-messages consumer) — requires the consecutive-user re-probe on z.ai/OpenRouter FIRST (§9c council Q1: one-shot live call during the build; council memory is not a probe).
