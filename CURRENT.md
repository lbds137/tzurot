...
| 2   | **`/admin db-sync`** (dev first, then prod)               | First sync under the new tombstone regime. Doubles as a data point on the open silent-death issue — transport probes still live.                                                                                | 🟡 **RAN** (owner, prod) — outcome not yet recorded. The load-bearing detail is whether the **reply landed** or hung on "thinking"; a hang would give the probe trail we've been waiting for.                                  |
