...
- **➡️ Epic remaining**: **B6-#11** (voice transcripts — council-verified flat-list design, ready to build) + **B1–B5** (~13 mechanical context-relocation items) + **Cluster C** (3 test-infra) + the **tooling-coverage** follow-up. All in the [`epic-log.md`](backlog/cold/epic-log.md) close-out tracker. Then: the queued dependency PRs.
...
**Active epic: "Slim common-types" (PR-2p) — extraction arc DONE, in close-out sweep.** All 4 packages extracted (config-resolver #1295, identity #1299, conversation-history #1300, cache-invalidation #1302); **Cluster A COMPLETE** (#1304 + #1305); **Cluster B triaged + batched B1–B6**; **B6-#18 SHIPPED via #1306** (transport `kind` discriminant — 10 review rounds; substance done by round 4, rest cosmetic). **➡️ Immediate next: B6-#11** (extended-context voice transcripts — council-verified flat-list/STT-re-derive design; ship voice attachment refs in the envelope, worker re-derives via shared Redis cache → STT, NOT DB lookup — the worker can't reach live-Discord-fetched extended-context messages in its DB). Then **B1–B5** (mechanical context-relocation), **Cluster C** (3 test-infra), the **tooling-coverage** follow-up. Full list in [`backlog/cold/epic-log.md`](backlog/cold/epic-log.md). After the clusters: the queued **dependency PRs**.
