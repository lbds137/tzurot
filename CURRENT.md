...
**Next, in order:** TASK-667 (owner decision recorded: embed index + slot naming; call sites re-enumerated 2026-08-21 — four naming sites plus one deliberate non-member, and the counter is internal to `extractEmbedImages`, so the naming half is likely one edit) → TASK-708 PR 2 (now defined — see below) → TASK-563 (parked branch verified intact at `0f7d16d00`; rebase, then fix the two fixtures) → TASK-700.
