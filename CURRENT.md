...
**Done:** TASK-706 (#2166), TASK-668 (#2167), TASK-708 PR 1 (#2168) and TASK-710 (#2169), all merged 2026-08-21. TASK-710 is CLOSED (no smoke needed — a behaviour-preserving extraction, canaried at both call sites). Forwards now carry attribution, their origin channel, and no longer leak our own `-#` footers into the prompt. The other three (706, 668, 708) stay OPEN — each has one acceptance clause that is runtime-unverified; see the smoke queue below.

**Next, in order:** TASK-712 (owner-approved option (a); the async ripple is measured on the task — four sync methods, stopping at an already-async caller, so build the full gate) → TASK-43 (**re-scope first**: the "MessageSnapshot strips mentions" blocker is false at the type level; the recorded next step is a runtime probe, not an implementation) → TASK-667 (binding decision recorded: derive the synthetic name from embed index + slot) → TASK-563 (parked branch is on origin and safe; 1 ahead / ~468 behind, so rebase).
