...
> **Version**: v3.0.0-beta.135 (released 2026-06-20, #1275) — **no migrations.** Headliners: the **Phase 2.5d context-assembly cutover finale** — worker-side assembly is now unconditional; deleted the legacy promote-path + shadow instrumentation (1300+ lines), `CONTEXT_MODE`, and the `RAW_ENVELOPE`/`THIN_PAYLOAD` flag-gating (#1267/#1268/#1269/#1270). **Verified safe against a ~14h clean prod soak** (40 `Context assembled via promoted path` info logs, 0 assembly errors) before giving up the env-flip rollback. Plus two prod bug fixes the user reported: the **referenced-image vision 403** — the worker now downloads image bytes and sends them inline instead of handing OpenRouter a Discord external-proxy URL it can't fetch (#1274), and the **trigger-message duplication** — the most recent user message no longer appears twice (assembled chat-log + live turn) (#1272). Also: LlmConfigResolver convergence (#1258), gateway read-timeout default flip to DEFERRED (#1262), character-edit empty-text round-trip fix (#1259), summon-anonymity discriminated union (#1260), and a slash-command consistency sweep (#1263/#1264/#1265). **Mechanics**: clean rebase-merge; `release:finalize` SHA-aligned develop; holistic review LGTM, no blocking findings. _Prior: v3.0.0-beta.134 (#1256) — incognito mode + DB connection-pool fix._
...
- **Cluster B — triaged + batched (B1–B6)** this session; ~17 valid items (2 agent-flagged "obsolete" were false-negatives — verified). **B6 (the 2 design-decision items) got a council pass + verification**: **#18 ✅ SHIPPED via #1306** (council `kind` discriminant; the council's #11 "DB re-derive" design was _refuted_ by verification — extended context is live-Discord-fetched, not in the worker DB — so #11 keeps the original flat-list/STT-re-derive shape). #1306 also fixed a real `isSaveTimeout` status-0 conflation + surfaced an **error-message-honesty audit** (filed to `cold/ideas.md`).
- **B6-#11 ✅ SHIPPED via #1314** — extended-context voice transcripts re-derived worker-side (DB-first / STT-fallback). bot-client ships `rawExtendedContextVoiceMessages` refs only for aged-out (empty-transcript) voice; the worker re-resolves via `getMessageByDiscordId` → `transcribeAudio` fallback (cap 10, newest-tail). Design corrected from the council's STT-only after verifying the transcript already lives durably in `conversation_history` — common case is a free DB pass-through, STT only for genuinely-ambient voice the bot never persisted. 3 rounds (round-2 caught a real cap-ordering bug: `slice(-cap)` not `slice(0,cap)` on the oldest-first list).
- **➡️ Epic remaining**: **B1–B5** (~13 mechanical context-relocation items) + **Cluster C** (3 test-infra) + the **tooling-coverage** follow-up. All in the [`epic-log.md`](backlog/cold/epic-log.md) close-out tracker. Then: the queued dependency PRs.

**Emergent prod fix this session — voice-gate video false-positive ✅ SHIPPED via #1309**: a directly-uploaded MP4 (carrying a `duration`) tripped the voice transcriber → bot apologized for failing to transcribe a "voice message". Root cause was operator drift: the "audio content-type AND duration" predicate was inlined across **8** bot-client sites and 3 had drifted to `|| duration` (a video carries a duration). Fix extracted a single `isVoiceAttachment` into a dependency-free leaf module (`utils/voiceAttachment.ts`) and routed every site through it. 5 review rounds — each surfaced a real missed site or a genuine improvement (incl. `hasForwardedVoiceAttachment` now reading the precomputed `isVoiceMessage` flag, which also closed an omitted-content-type detection gap). Two non-blocking follow-ups filed (parallel `hasVoiceAttachment` snapshot path; `isVoiceMessage` `optional()` schema trap).

**Emergent prod fix this session — bot's own TTS audio leaking into reply-target references ✅ SHIPPED via #1312**: when a user replies to a personality's voice message, the AI saw its OWN TTS audio file as an attachment (`[audio/ogg: <personalityId>-<slug>.ogg]` in a `<contextual_references>` quote) and reasoned about "an audio message I sent". Root cause: a personality reply is delivered via webhook with TTS as an `audio/*` FILE attachment (no Discord voice `duration` → `isVoiceMessage: false`), so the existing voice filter missed it and the reference renderer folded it in. Fix: shared `stripBotVoiceAttachments` kernel (common-types) drops `audio/*` attachments from bot/webhook-authored refs (`authorIsBot || webhookId`), applied once in the worker's `enrichRawReferences` (covers dedup-stub + full-quote paths). LGTM, 1 round.

**Cluster B close-out (PR-2p) — Batches A+B+C shipped this session**: **#1310** (Batch A — ai-worker `ContextStep`: `assertEnvelopeJob` returns the narrowed assembler, worker-overwrite invariant doc, `resolveSummonAnonymity` audit) + **#1311** (Batch B — `contextWritePath`→`gatewayWriteHelpers` rename + PR #1283 weigh-in nits) + **#1313** (Batch C — vestigial `conversationHistory` wire-field removal across 3 common-types defs + bot-client `MessageContext`/ship-site + the dead ai-worker `normalizeConversationHistory` path; −306 net lines). **All mechanical/ready Cluster B items now done** (incl. **B6-#11**, the extended-context voice-transcript re-derive, ✅ #1314). Remaining: **7 trigger-gated items** stay parked (perf-gated dedup/eviction, "3rd consumer" stub, etc.).

**Cache-duration audit + voice-cache-key fix (user directive) ✅ SHIPPED via #1315 + #1316**: `VoiceTranscriptCache` keyed on the full _signed_ Discord CDN URL (re-signed every re-fetch → near-total cache miss → needless re-transcription). **#1315** added a shared `deriveAttachmentCacheKey` (prefer attachment id, else query-stripped URL hash — the proven `VisionDescriptionCache` pattern, now shared by both caches), bumped `VOICE_TRANSCRIPT_TTL` 5min→1h (key is now content-stable), and converged vision onto the helper. **#1316** bounded the two genuinely-unbounded in-memory caches — identity `BaseConfigResolver` (raw Map + a `setInterval` cleanup flagged in-code as a scaling-blocker) and `ModelCapabilityChecker` (raw Map) — onto the shared `TTLCache`, and removed the now-dead `enableCleanup`/`stopCleanup` API from the identity base (verified zero callers; #1316 also fixed a latent `cacheTtlMs:0`-no-longer-disables regression). Full per-cache rationale (all ~34 caches, why each TTL) → [`docs/reference/caching/cache-duration-rationale.md`](docs/reference/caching/cache-duration-rationale.md); the 10s config-resolver TTL was verified justified (eager pub/sub-invalidation backstop) and kept. One `disableCache`-option follow-up filed.
...
**Active epic: "Slim common-types" (PR-2p) — extraction arc DONE, in close-out sweep.** All 4 packages extracted (config-resolver #1295, identity #1299, conversation-history #1300, cache-invalidation #1302); **Cluster A COMPLETE** (#1304 + #1305); **Cluster B triaged + batched B1–B6**; **B6-#18 SHIPPED via #1306** (transport `kind` discriminant); **B6-#11 SHIPPED via #1314** (extended-context voice transcripts — DB-first/STT-fallback worker re-derive). **Cache-duration audit + voice-cache-key fix ✅ DONE** (#1315 stable key + 1h TTL; #1316 bounded the unbounded caches; `docs/reference/caching/cache-duration-rationale.md` written). **Epic close-out swept (2026-06-23)** — Cluster B + Cluster C re-audited and confirmed entirely trigger-gated/parked (no fired triggers; see `epic-log.md` sweep note); the only non-gated item, the **tooling-coverage** PR, is **deferred post-release** (dev-experience, doesn't ship). **Deps cleared**: #1296 (checkout 6→7) + #1303 (dev deps) merged; #1297 (prod-dep group) auto-closed by dependabot as redundant (develop already current); the 4 undici security alerts are already fixed on develop (`undici ^6.27.0` override) and clear on the release. **➡️ Immediate next: cut `v3.0.0-beta.136`** (no migrations; ~25 PRs since beta.135 — epic completion + voice fixes). Then promote the next `cold/queue.md` theme.
