...
- **Memory Phase 1a still PARKED** on `feat/memory-hybrid-retrieval` — resume gate now = the owner QUERY session (~1hr: realistic messy chat messages + recall expectations; swap-map review now optional since nothing content-bearing gets committed) → harness corpus-ref extension → A/B re-run. New epic phase candidate **1c: retrieval-query sophistication** (prod embeds the raw user message verbatim — measure rewrite ladder on these goldens).

## Next Session Goal

**Next:** (1) **admin-runtime-settings PR 2 — dashboards** (groups/pagination mechanism, Defaults + System page groups, ride-alongs incl. the two #1605 polish follow-ups; then PR 3 consumer swaps + env deletion, PR 4 runtime text fallback descent — artifact amended 2026-07-12 with O8 free-floors-configurable + O9/D12); (2) **owner goldens QUERY session** (~1hr, keyboard) → harness corpus-ref → hybrid A/B re-run. beta.160 smoke: vision coalescing ✅ runtime-confirmed in prod logs (2026-07-12); remaining items (GLM 5.2 /inspect 500K, role="character" sanity, beta.159 holdovers) still open. Watches: dev backfill (~59%), first guest z.ai traffic (`/admin usage`), Saturday weekly-audit run, prod lock-storm recurrence (probe DURING the window — see board entry). Fable access through July 19.
