...
> **Version**: v3.0.0-beta.138 (released 2026-06-26) — **no migrations.** Headliners: **z.ai overload resilience** — a transient z.ai 429 now fast-falls-back to OpenRouter after **one** attempt (~2min vs the old ~5.5min full-retry-budget burn), with the footer model-link corrected to the _effective_ provider after the swap (#1348). **Two chat-log fixes** — the bot's own voice output no longer appears **twice** in context (its TTS transcript duplicated the message body), and two personalities sharing a display name are now disambiguated by their unique `name` on both the stored-history and extended-context paths (#1349). **Explicit footer provider attribution** — every model footer names the serving provider ("via OpenRouter" / "via Z.AI Coding Plan") instead of leaving it to the `z-ai/` vendor-prefix (#1350). Plus a **conversation-persist DB-hang fast pool** (#1343). And the **Test-Pyramid Phase 4 (seam contract coverage) epic** — real Redis everywhere in component tests, an honest contract tier running in CI, code-derived coverage topology + drift gate, the golden-fixture envelope contract, and tier-accurate test suffixes (#1339–#1347). **Mechanics**: 26-commit release; clean rebase-merge + `release:finalize` SHA-aligned develop; 1 holistic-review round (LGTM, follow-ups → backlog); dev-smoke-verified (voice dedup 7/7 user-only, footer, post-rename attribution). _Prior: v3.0.0-beta.137 (2026-06-25) — infra-vs-negative bug-class fix + dead stop-sequence removal._
> **🚧 Release freeze status**: LIFTED. No release in progress.

---

## Unreleased on Develop (since beta.138)

**Released v3.0.0-beta.138 on 2026-06-26** (see version header; notes: [tag v3.0.0-beta.138](https://github.com/lbds137/tzurot/releases/tag/v3.0.0-beta.138)). The z.ai fast-fallback + footer model-link, the two chat-log fixes (voice dedup + same-display-name disambiguation), the explicit footer provider attribution, the conversation-persist fast pool, and the whole Test-Pyramid Phase 4 epic shipped — full detail in git history and the release notes. `release:finalize` SHA-aligned develop with main; prod auto-deploying.

**Unreleased since beta.138:**

| PR    | Title                                                                                 | Outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| #1359 | `fix(ai-worker): stop re-storming dead image URLs on the reference path`              | Pre-release squeeze-ins (user-requested before cutting the release). **High-value fix**: a permanently-dead image (expired Discord CDN URL sitting in extended context) no longer re-storms across OpenRouter providers every turn (~100s/turn latency eliminated) — `checkNegativeCache` gained an `attachmentBoundOnly` mode so the reference path honors attachment-bound cached failures while still re-attempting transient ones. Plus two hygiene items: `PROVIDER_FOOTER_LABEL` constrained to LLM providers (structural guard — a voice provider can't leak into the LLM footer) + `/models browse` internal `search`→`query` rename. 2 review rounds, all non-blocking (round-2 nit dismissed — the positive-case footer test already exists).                                                                         |
| #1358 | `chore: close out the test-pyramid Phase-4 epic (deferred nits)`                      | Test-Pyramid Phase 4 PR7 — the epic CLOSE-OUT grab-bag (see `active-epic.md`). Cleared the deferred Phase-4 nits: dead `.component.test.ts` audit guards + e2e scan branch removed; `contractFixtures` path-traversal guard + direct tests; topology execution-check doc/anchor polish + `contentImportsSymbol` un-export; envelope test-hardening (`resetAllMocks` + voice-history assertion); `ConversationHistoryService` UUID-collision flake fixed (seeded timestamps); voice-engine error-path schema validation (`errorDetailSchema` + ZodError observability); temporal-marker hook extended to Python. **Epic COMPLETE.** 3 review rounds, all non-blocking (round-3 nit dismissed on a verified-false Zod-strict premise). Backlog swept: flow-gate parked, smoke-check filed to ideas, compile-time-harness dropped. |
| #1352 | `fix(ai-worker): recover the human behind relay-echoes in extended context`           | The first post-release task (Option 2: resolve the human at the source). Relayed dev-bot input carries no `personaId` + the bot's webhook name as `discordUsername`; `recoverRelayEchoIdentities` re-resolves those rows to the real human via the persisted user identity, so a persona-name/character-name collision no longer fragments one human into two chat-log identities. **Closes the beta.138 dev-smoke follow-up** (the `Lila (@Rotzot)` fragmentation surfaced post-rename).                                                                                                                                                                                                                                                                                                                                       |
| #1353 | `test: replace circular BullMQ contract tests with a real-producer golden fixture`    | Test-Pyramid Phase 4 PR3 — see `active-epic.md`. Real-`createJobChain` producer → committed fixtures in `@tzurot/test-utils` → consumer schema validation; non-circular. Centralized `stableFixtureJson` into test-utils.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| #1354 | `test: extend the envelope contract to cross-channel, voice, and mention seams`       | Test-Pyramid Phase 4 PR4 — see `active-epic.md`. Parameterized the bot-client→ai-worker golden-fixture envelope contract with 3 new fixtures (voice ground-truth split, cross-channel env decoration, mention rewriting). **Process catch**: `.contract.test.ts` only runs via `pnpm test:integration` — the unit `pnpm test` config excludes it (CI caught a test that never ran locally). 4 review rounds, all non-blocking.                                                                                                                                                                                                                                                                                                                                                                                                  |
| #1355 | `docs(skills): name the verify command per test tier in the testing skill`            | Test-Pyramid Phase 4 support — documents the per-tier verify command in the `/tzurot-testing` skill (`pnpm test:integration` for contract/integration tests; the unit `pnpm test` config excludes them), closing the gap that caused #1354's local CI miss.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| #1356 | `feat(tooling): upgrade coverage topology from presence to execution check`           | Test-Pyramid Phase 4 PR5 — see `active-epic.md`. The topology now requires a contract test to IMPORT the real producer/consumer symbol (ts-morph), not just exist; a circular test gaps and `topology:check` catches it. Council-unanimous Option A. 6 review rounds (1 real bug — a type-import bypass — fixed; the rest doc-precision).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| #1357 | `feat(voice-engine): cross-language JSON-response contract (golden-fixture + TS Zod)` | Test-Pyramid Phase 4 PR6 (the last cross-service seam) — see `active-epic.md`. Locks the Python↔TS voice-engine boundary: committed fixtures the Python producer test asserts against + TS Zod schemas replacing the unsafe `as` casts in `VoiceEngineClient`. Council-unanimous Option A (no Pydantic refactor). Drift gate is structural + free (both CI jobs run every PR). 3 review rounds.                                                                                                                                                                                                                                                                                                                                                                                                                                 |

---

## Next Session Goal

**The Test-Pyramid Taxonomy + Coverage Audit epic is COMPLETE** (Phases 1–4, PR1–7; PR7 = #1358 close-out grab-bag, merged 2026-06-26). The contract tier is populated and CI-enforced (code-derived coverage topology + execution-check ratchet + byte-compared drift gate); the close-out swept the backlog — flow-level gate **parked** with a sharp trigger (`cold/follow-ups.md`), post-deploy smoke check **filed** to `cold/ideas.md`, compile-time contract-test harness **dropped** (council: gold-plating). The `active-epic.md` slot is now OPEN.

**➡️ Immediate next: promote the next theme from [`cold/queue.md`](backlog/cold/queue.md)** — each substantial pick earns a council pass (GLM-5.2 / Kimi-K2.7 / Qwen-3.7) before plan-mode. **Unreleased on develop since beta.138**: the Phase-4 PR3–7 work (#1353/#1354/#1356/#1357/#1358) + #1352 — a release is due when the user calls it.
