...
**Done:** TASK-706 (#2166), TASK-668 (#2167), TASK-708 PR 1 (#2168), TASK-710 (#2169), TASK-712 (#2170), TASK-43 probe (#2171) — all merged 2026-08-21. **TASK-710 and TASK-712 are CLOSED** (710 a behaviour-preserving extraction; 712 a tightening whose changed behaviour is only visible to a forwarder LACKING access, which a smoke cannot stage). 706, 668 and 708 stay OPEN pending the smoke queue below.

**Next, in order:** TASK-716 (gate is shared by convention, not mechanism — fix shape revised to "export `resolveOriginChannelName` and call it", import direction verified, carries a docstring-precision ride-along) → TASK-667 (owner decision recorded: embed index + slot naming; call sites re-enumerated 2026-08-21, four naming sites plus one deliberate non-member) → TASK-563 (parked branch verified intact at `0f7d16d00`; rebase, then fix the two fixtures) → TASK-700.

**TASK-708 PR 2 is an OWNER DECISION, not queued work.** Agent recommends DECLINING on merit: the prescribed authorship gate cannot separate the two paths emitting a bold `**Name:** ` prefix — we wrote both, and for the `/chat` relay echo the name is the USER's, so stripping deletes the only attribution the text carries. Build nothing for 708 until ruled.
