...
> **Version**: v3.0.0-beta.140 (released 2026-06-27) — **contains a migration** (`20260627040007_vision_config_kind`, applied to dev + prod, vision globals seeded). Headliner: **vision as a first-class config** — image description resolves through a cascade-driven global vision default (`qwen/qwen3.7-plus`) instead of the per-preset `visionModel` field, fixing the incident where a vision-incapable main model fell back to a slow 397B model and timed out; the vision model is now changeable in one place (#1364). Also: **long-audio STT chunking** — long voice messages chunk for transcription (up to ~12 min) with clear timeout/too-long messaging (#1363); UUID-collision test-flake fix (#1361). **Mechanics**: rebase-merge + `release:finalize` SHA-aligned develop; 1 holistic release-review round (2 bugs + 2 correctness gaps found, all low-impact → backlog for the follow-up). **⚠️ Post-deploy incident**: prod hit a ~3-min breaking-migration window (new code on the pre-migration schema → `column llm_configs.kind does not exist` → personality-load failures → user-visible generic "No response received") between deploy-live (20:38Z) and the manual prod migration (~20:41Z); recovered on migrate (20:42Z). Backlogged: eliminate the breaking-migration deploy window structurally. _Prior: v3.0.0-beta.139 (2026-06-26) — dead-image latency fix (#1359), relay-echo identity recovery (#1352), voice-engine response validation (#1358), Test-Pyramid Phase 4 contract tier._
> **🚧 Release freeze status**: LIFTED. No release in progress.

---

## Unreleased on Develop (since beta.140)
...
**Unreleased since beta.140** (8 PRs to develop, no migrations):
...
- **#1368 — weigh-in prod-bug fix** (the bug the user reported): `/character random` / `/character chime-in` with no message no longer feeds the latest channel message — including a _different character's_ reply and a TTS→STT round-trip of it — back as the user turn. It now anchors on the synthetic `WEIGH_IN_MESSAGE` (the latest message stays in history). Bonus: restored guild/channel-scoped denylist checks the synthetic anchor was silently skipping.
- **#1369 — voice `_merge_overlap` punctuation false-match fix** (release-review finding): STT chunk-stitching matches on non-empty content subsequences with index-mapping, so punctuation tokens inside a real overlap no longer duplicate the overlapped words.
- **#1370 — api-gateway: 404-not-500 + phantom-`EXCLUDED_TABLES` detection + TTS set-default existence assert** (two round-3 nits + the symmetric TTS gap): `resolveTargetKindOrThrow` / TTS `setAsDefault` / `setAsFreeDefault` throw a typed `NotFoundError` → HTTP 404 (was 500 with the internal op string); `validateSyncConfig` flags individually-stale EXCLUDED_TABLES entries (removed the now-dropped `image_description_cache`).
- **#1371 — ai-worker: delete dead fail-fast `storeFailure` writes + fix misleading docs** (release-review #2): the fail-fast / auth-failure paths never reached a cache reader, so the model-less `storeFailure` writes were dead — deleted + docs corrected (both builders made sync).
- **#1372 + #1373 — breaking-migration deploy-window fix** (council-reframed): #1372 first tried a Railway `preDeployCommand`, but a dev-deploy validation (SSH dry-run in the live runner) proved it doubly broken — `prisma` is a devDep unresolvable in the `--prod` runner, and the per-service `railway.json` isn't auto-detected by location, so it never ran — and a single-service pre-deploy is false safety anyway (ai-worker/bot-client race the schema). A 3-model council (GLM-5.2 / Kimi-K2.7 / Qwen-3.7-max, unanimous) reframed the root cause; **#1373 reverted #1372** and shipped `pnpm ops release:premigrate`: migrate the prod schema BEFORE the release merge (while old code still runs), so auto-deploy lands into a ready schema and the window closes for all four services at once. Detects destructive shapes (DROP/RENAME/SET NOT NULL/DROP CONSTRAINT/ALTER COLUMN TYPE) and refuses without `--allow-destructive`. Net effect on develop: the premigrate command + migrate-before-merge docs/rules/skills; no `railway.json`/Dockerfile change.

**➡️ Next: vision-epic follow-up release** — remaining open item: **#4 `getById` `kind:'text'` gate** → Phase 2 (the only release-review finding still open — #2 resolved in #1371; `_merge_overlap` / 404 / phantom all shipped above). The breaking-migration deploy window is **resolved** (#1373 — migrate-before-merge); only its destructive-migration carve-out remains (the `MAINTENANCE_MODE` follow-up in `cold/follow-ups.md`, promote when a destructive migration is planned). Then **Phase 2** (vision editing surface). `active-epic.md` holds the Model Configuration Overhaul epic (`cold/themes/model-configuration-overhaul.md`).
