...
**iii-b-1 burn-in DONE (dev, promoted path verified) + iii-b-2 IN PR.** The b-1 dev burn-in confirmed the promoted path drives generation (10 jobs, 0 errors, weigh-in null-persona + LTM skip clean, Fork-C vision ordering holds; proven via the per-generation epoch-query fingerprint since the promoted path was silent). Two weigh-in fixes fell out and rode forward: the **STM-reset epoch cutoff is now skipped for weigh-in** (anonymous channel summon must not be bounded by the invoking user's `/conversation reset`) — fixed in both the assembler and the legacy `MessageContextBuilder`.

**iii-b-2 (`feat/context-iii-b-2`) — thin context payload.** Bot drops the 4 re-derivable fields (`conversationHistory`/`referencedMessages`/`mentionedPersonas`/`referencedChannels`) behind a `kind: 'legacy'|'envelope'` discriminant (NOT a `z.discriminatedUnion` — absent-discriminant would throw on in-flight jobs; it's an enum field + `superRefine`). `participantGuildInfo` + `extendedContextAttachments` stay (not re-derivable yet → **iii-b-3**). New bot flag `CONTEXT_THIN_PAYLOAD`; worker assembles envelope-kind unconditionally + fail-loud. Also fixes the `isWeighIn` schema drift + adds a promoted-path observability log. **Deploy worker before bot.** Carries the 3 #1182 fold-forwards (contextFlags.ts move; cross-channel promote test; stray `async`). The 5 #1156 fold-forwards re-homed to a standalone **iii-cleanup** slice. Then **iii-b-3** (envelope guild-info growth) → **2.5d** deletion → PR-2p.
