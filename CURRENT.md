...
- [x] **2. Error embed posts** — **PASS on retest (2026-08-24, post-#2207)**: one 🚨 bad_request embed in the log channel (source: job, stack hash 60d69712, request ID present, NO message text). First attempt had failed and found the multiTagDeliveryFlow reporter bypass (fixed + swept in PR #2207).
- [x] **3. Dedup suppresses** — **PASS (2026-08-24)**: repeat of the same error 11 min later delivered in-channel but posted NO second embed. **Cleanup remaining**: owner flips COLD back to its real model (agent must not guess the prior override), THEN agent deletes the 'SMOKE broken model (delete me)' llm_config row from dev (sync-tracked table — must not ride a dev→prod sync).
