...
**iii-b-2 SHIPPED (#1183, 2026-06-11) — next is the dev burn-in of the THIN payload.** The wire contract landed: bot drops the 4 re-derivable fields (`conversationHistory`/`referencedMessages`/`mentionedPersonas`/`referencedChannels`) behind a `kind: 'legacy'|'envelope'` discriminant (enum field + `superRefine`, NOT a `z.discriminatedUnion` — that would throw on in-flight absent-`kind` jobs). `participantGuildInfo` + `extendedContextAttachments` still ship (not re-derivable yet → **iii-b-3**). Also fixed the `isWeighIn` schema drift, added a promoted-path observability log, and folded in the weigh-in STM-reset epoch-cutoff fix (both assembler + legacy `MessageContextBuilder`). On dev now via Railway auto-deploy with `CONTEXT_THIN_PAYLOAD` **off** (bot still fat). **Next: flip `CONTEXT_THIN_PAYLOAD=true` on dev** (worker's already kind-aware — deploy ordering is moot with the flag off), confirm via the `'Context assembled via promoted path'` log that envelope-kind jobs assemble, and re-run the smoke matrix (esp. the kept fields `participantGuildInfo`/`extendedContextAttachments` still populate, weigh-in, Fork-C). Then **iii-b-3** (envelope guild-info growth → drop the last 2 fields) and the standalone **iii-cleanup** (5 carried #1156 fold-forwards) → **2.5d** deletion → PR-2p.

_b-1 burn-in (done earlier): promoted path verified — 10 jobs, 0 errors, weigh-in null-persona + LTM skip clean, Fork-C vision ordering holds (proven via the per-generation epoch-query fingerprint since the path was silent)._
