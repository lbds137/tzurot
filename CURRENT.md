...
**Cluster B close-out (PR-2p) — Batches A+B shipped this session**: **#1310** (Batch A — ai-worker `ContextStep`: `assertEnvelopeJob` returns the narrowed assembler, worker-overwrite invariant doc, `resolveSummonAnonymity` audit) + **#1311** (Batch B — `contextWritePath`→`gatewayWriteHelpers` rename + PR #1283 weigh-in nits). 7 trigger-gated Cluster B items stay parked (perf-gated dedup/eviction, "3rd consumer" stub, etc.). Remaining ready: **Batch C** = `MessageContext.conversationHistory` vestigial wire-field removal (scoped: worker-telemetry caveat debunked — the reads are on `ConversationContext`, not the wire field). Then B6-#11 (voice transcripts, plan-mode).
