...
- **NOT in v1**: guild-tier *preset* pointers (System B stays untouched; named trigger: a server community actually asks for a server default model) and **policy clamps** (a guild *cap* that binds even user tiers). Clamps are a second AXIS (bounds, not precedence) requiring a post-resolution clamp pass — council consensus: when the trigger fires (first genuine moderation/cost-control need), it's a **design session of its own**, not a small follow-up; noting that here prevents under-scoping later.
