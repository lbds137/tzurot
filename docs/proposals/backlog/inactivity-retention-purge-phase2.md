...
>
> 1. **`PersonalityOwner` already carries a `role` column and it is dormant.** `prisma/schema.prisma` declares `role String @default("owner") @db.VarChar(50)`. The sole writer (`ai-worker/src/jobs/ShapesImportHelpers.ts:47`) hardcodes `'owner'` with `userId === ownerId` — a self-duplicate of `personality.ownerId`, so no genuine co-owner row exists anywhere. All readers test row **existence** and ignore `role`: `routes/user/personality/helpers.ts:69` (`canUserEditPersonality`) and `:115` (`canUserViewPersonality`), `personality/list.ts:97`, `personality/get.ts:44`, and — see finding 4 — `ai-worker/src/jobs/AccountExportAssembler.ts:236`. So "a `PersonalityOwner` grant over-grants edit" is true only because the reads ignore a column that already exists — the primitive may be *make the readers role-aware*, not *invent a permission model*.
> 2. **The DTO permission shape has no `canView`.** `common-types/src/utils/permissions.ts` exports `EntityPermissions = { canEdit, canDelete }`; its docstring already anticipates the extension ("Adding new roles (e.g., moderators) only requires backend changes"). There is a shape to extend rather than a greenfield decision.
> 3. **Design constraint — any row-granting fix must also fix the DTO, or ship one that lies.** `computePersonalityPermissions` derives `canEdit` from `personality.ownerId` **alone** and never consults `PersonalityOwner`, while the route guard `canUserEditPersonality` **does**. This is inert today precisely because no real co-owner rows exist — which is exactly what an accessibility fix would create. A reach-holder would be told `canEdit: false` by the DTO while the gateway accepts their edit.
>
> 4. **A fifth reader makes a `PersonalityOwner` grant leak someone else's authored content into a data export.** `AccountExportAssembler.fetchCharacters` unions co-owned IDs (swept from `PersonalityOwner`, `:236`) with directly-owned ones and exports the **full character definition** for each — its own comment reads _"Everything except binary blobs — the definition is the user's content."_ Granting a reach-holder a row would therefore put the orphaned character's system prompt and persona text into **their** export, as though they had authored it. That is the wrong direction for a data-minimization epic, and it is invisible from the permission-check call sites: the export path never asks "can this user view it," it asks "which rows name this user." **Any grant-a-row design must decide what the export does with the grant** — role-aware readers must include this one, or the grant must not be a `PersonalityOwner` row at all.
>
> **Still open (the actual design questions):** (a) who are the grantees — the `findCrossUserReachIds` set already computed at re-home time, or broader? (b) grant at re-home time (a bounded write, but needs backfill semantics) or resolve dynamically at read time (no write, but a join on every view check) — finding 4 pushes on this one, since a read-time resolve creates no row for the export sweep to find? (c) does `role` become an enum, and is the reach-widening follow-up (co-ownership grants are invisible to `partitionOwnedByReach`) decided in the same pass? Both touch the same predicate.

**Reclamation semantics — owner-directed, sharpened.** _"If somebody leaves the platform and their character ends up orphaned, but then they come back later, they should get access back to it. It should be their character like it was before, which means removing the orphaned placeholder user, so that it goes back to how things were prior to all of this."_ Reclamation is therefore a **full restore to the prior state**, not a partial grant: `owner_id` returns to the reclaiming user, the sentinel holds nothing further, and `original_owner_discord_id` clears. **This constrains what reach-holders may receive** — anything a purge hands out automatically must be something reclamation can dissolve without negotiating with third parties. A grant that amounted to real co-ownership would survive the restore and leave the returning owner sharing their character with everyone who ever chatted with it, which is not "how things were prior."

**Deliberate co-ownership is a SEPARATE feature** (owner call, same conversation): a purge-side access grant and an owner-initiated "share this character with you" are different in who initiates, what it means, and whether it survives reclamation — same junction table, different `role`. Scoped out of PR-D and filed in [`backlog/cold/ideas.md`](../../../backlog/cold/ideas.md) § Deliberate character co-ownership; PR-D ships only the machinery both need (role-aware readers + the orphan role).
...
### Phase-3 build notes (2026-07-26)

**The notify branch is the cohort-discovery mechanism, not a courtesy.** Runtime evidence (beta.177): release blasts are opt-in-gated (`notifyOptedInAt` — `sent=1`), so nothing ever DMs the inactive cohort and nothing stamps the unreachable flags; Phase 2's purge cohort is structurally 0 without Phase 3's warning DMs bouncing off the genuinely-unreachable subset. This retro-corrects the epic's "the 26 become the first real preview batch" expectation.

**The parent design's "verify `AccountExportJob` delivery (25MB DM cap, link expiry)" gate resolved by grounding: there is no delivery step.** Exports are link-only — `POST /user/account/export` mints the token synchronously and the user downloads via `GET /exports/:token` (24h expiry, 24h cooldown); no DM/attachment code exists anywhere. Consequence: the warning DM invites the user to run `/settings data export` themselves; it never mints a link (dead within 24h vs a 30-day grace) and never attaches a file (a brand-new untested failure mode). The purge-vs-export race closes itself: requesting an export IS activity (the Phase-1d stamp), which exits the user from the cohort entirely.
...
Two clarifications from the same ruling and its review rounds (#1805):
browse-only/command-only users count as bystanders — slash commands leave no
durable per-user trace, so they are data-indistinguishable from pure
bystanders, and the owner ruled both purge silently (the policy text
deliberately describes the evidence bar, not command history). And the
bystander arm is gated on never-notified, so a served notice makes the 30-day
grace floor unconditional in SQL — evidence vanishing through a non-user path
can never purge a mid-grace user early.
