...
>
> **Pre-design grounding (code-verified) — the primitive is narrower than "new permission model".** Three findings change the shape of the fix:
>
> 1. **`PersonalityOwner` already carries a `role` column and it is dormant.** `prisma/schema.prisma` declares `role String @default("owner") @db.VarChar(50)`. The sole writer (`ai-worker/src/jobs/ShapesImportHelpers.ts:47`) hardcodes `'owner'` with `userId === ownerId` — a self-duplicate of `personality.ownerId`, so no genuine co-owner row exists anywhere. All four readers test row **existence** and ignore `role`: `routes/user/personality/helpers.ts:69` (`canUserEditPersonality`) and `:115` (`canUserViewPersonality`), `personality/list.ts:97`, `personality/get.ts:44`. So "a `PersonalityOwner` grant over-grants edit" is true only because the reads ignore a column that already exists — the primitive may be *make four readers role-aware*, not *invent a permission model*.
...
>
> **Still open (the actual design questions):** (a) who are the grantees — the `findCrossUserReachIds` set already computed at re-home time, or broader? (b) grant at re-home time (a bounded write, but needs backfill semantics) or resolve dynamically at read time (no write, but a join on every view check)? (c) does `role` become an enum, and is the reach-widening follow-up (co-ownership grants are invisible to `partitionOwnedByReach`) decided in the same pass? Both touch the same predicate.
