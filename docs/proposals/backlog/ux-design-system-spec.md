...
**Page index (hub) navigation** (doc-72): the settings dashboards' navigation row is ◀ Prev · `N/M` · Next ▶ · 🗂️ Index. The page label lives in the embed title and footer (`Page 2/3 · Memory`), not on the row. Index opens a navigation-only view that lists every page and has one jump select. A dashboard of 4+ pages opens on the index; smaller ones open on page 1. Back from a setting returns to the page it was opened from. Every settings dashboard is paged: the character, character-override and channel dashboards use the cascade tiers' Memory · Context & Display · Voice pages.
**Resets** (doc-72): every page whose settings all have an Auto state renders ♻️ Reset page (Danger, its own row below the navigation row, disabled when nothing on the page is set at this tier); the index hub renders ♻️ Reset all on dashboards whose scope holds the user's own overrides (not admin). Both go through the §3.5 Tier-A confirm naming the count, and clear in one save.
