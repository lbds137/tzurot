...
| **2.5** | Flag flip: staged rollout, prefix-diff before/after (per-personality grouping, TASK-698 first), snapshot review per §6; voice-consistency harness re-arms with extraction arms. Rollout-verification caveat RESOLVED: `promptHashHistoryStable` is re-keyed to the shipped message array flag-on (§9d D5, TASK-729 SHIPPED) — the stable-prefix diagnostic now measures real cache behavior in both modes. Flip gates ALL SHIPPED: task-723 closed (§9d build units TASK-726/727/729 all merged) and task-724 (shipped-id/render desync — SHIPPED #2182). **The gate list is empty**; TASK-731 (messages inspect view) and TASK-734 (bot slash-reply visibility) build alongside before the flip proposal |
