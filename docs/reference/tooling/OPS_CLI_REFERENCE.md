...
| `pnpm ops dev:deferred-refs`        | Surface tracker tasks referencing the given (or staged) files — never fails; `--compact` (what the hooks pass) prints one line per file with the count and up to two ids |
...
| `pnpm ops memory:render-pilot --env dev --personality <slug> --answer-model <id> --judge-model <id> --summary-model <id>` | Three-arm memory-render pilot (V/F/S render + eval); LOCAL-ONLY output in gitignored `reports/render-pilot/`. `--glm-provider <openrouter\|zai-coding>` routes answer/summary/voice GLM calls (default `zai-coding`, the flat-rate coding plan); `--summary-thinking <disabled\|high>` (default `disabled`) and `--answer-thinking <disabled\|high>` (default `high`) control extended reasoning on that provider |
| `pnpm ops memory:summarize --env dev --personality <slug> --dry-run`                                                      | Report the ≥95% flip gate, fact coverage, dead rows by error class, and the token estimate — no jobs enqueued                                                                                                                                                                                                                                                                                                     |
| `pnpm ops memory:summarize --env dev --personality <slug>`                                                                | Enqueue archive-summary pre-warm jobs for the personality's hot (recently-retrieved) rows; idempotent by memory id, so re-running is safe                                                                                                                                                                                                                                                                         |
| `pnpm ops memory:summarize --env dev --personality <slug> --include-cold`                                                 | Also sweep cold (rarely/never-retrieved) rows behind the hot budget                                                                                                                                                                                                                                                                                                                                               |
...
- `release:premigrate` — run BEFORE merging the release PR so auto-deploy lands into a ready schema; destructive shapes need `--allow-destructive` inside a maintenance window, and a migration marked `-- tzurot:apply-after-deploy` needs `--allow-marked` (or, better, merge first and `db:migrate` after) (see `.claude/rules/03-database.md` § Deployment). Also cross-checks that dev has applied the release range's migrations before premigrating prod (`prisma migrate status` against dev) — refuses if dev is behind, naming the range and dev's own status; `--allow-dev-pending` overrides. Because of that cross-check, a prod premigrate needs the **dev** Railway link too; a missing link surfaces as that same unreachable refusal, with the Railway URL-fetch error quoted. An unreachable dev is reported separately from a behind dev and refuses on its own terms — `--allow-dev-pending` does not cover it, since that flag asserts dev's state is known.
...
| `pnpm ops gh:ci-gate <n> --sha <sha>`      | Wait for CI and the review run, then report checks (arm in a Monitor; see `05-tooling.md` § PR Monitoring) |
...
| `pnpm ops secrets:rotate-env --env prod --name INTERNAL_SERVICE_SECRET --stage 1` | Staged rotation for a dual-accepting name (1=stage, 2=roll, 3=finalize)      |
...
`secrets:rotate-env` rotates a plain shared secret with a brief 401 mismatch window while services redeploy on the new value at different times, UNLESS the name's verifier dual-accepts `<NAME>_PREVIOUS` (`INTERNAL_SERVICE_SECRET`, verified by api-gateway) — for those, `--stage 1|2|3` is required and there is no window: stage 1 preserves the old value and redeploys only the verifier, stage 2 rolls the presenters, stage 3 clears `_PREVIOUS`.
...
| `pnpm ops guard:readme`                      | Fail when README.md drifts from the repo it describes (CI gate, not audit-class)                                                                                                                                                                                                                                                                  |
| `pnpm ops guard:repo-settings`               | Fail when deletion of a long-lived branch (`main`, `develop`) is reachable via GitHub settings, or when the main ruleset requires a status check with no job in `origin/main`'s ci.yml (release preflight + `ops health`; NOT a CI gate — CI tokens can't read rulesets, so it would degrade to a no-op there)                                    |
...
| `pnpm ops guard:env-example`                 | Fail when `.env` and `.env.example` declare different active keys; prints key names only and skips when `.env` is absent (pre-push only, not audit-class)                                                                                                                                                                                         |
...
| `pnpm ops lines:check`                       | Always-loaded surfaces (`.claude/rules/*.md` total, `CURRENT.md`, `.claude/skills/*/SKILL.md` total) within LINE and BYTE budgets, plus a hard 147,000-char `.length` ceiling on `CLAUDE.md` + rules (audit-class, `--summary` wired; `--breakdown` ranks every file worst-first by bytes)                                                        |
| `pnpm ops lines:update-baseline`             | Make budget growth explicit on both dimensions (`--dry-run` supported; `--surface <name>` refreshes one surface only — `rules`, `current`, or `skills`; same meta contract as `cpd:update-baseline`)                                                                                                                                              |
...
`guard:readme` checks five classes against README.md: the Project Structure tree vs. on-disk `services/`/`packages/` directories, the Prerequisites Node/pnpm versions vs. `package.json` `engines`, every fenced `pnpm <script>` vs. root `package.json` scripts, the Slash Commands section vs. shipped slash and message context-menu commands, and every relative link/anchor vs. the files and headings that actually exist. The command-list class has no allowlist.
...
`guard:repo-settings` encodes one invariant: deletion of a long-lived branch must be UNREACHABLE. `delete_branch_on_merge` deletes the head branch on EVERY merge (the per-merge `--delete-branch` flag is irrelevant), and GitHub performs that delete with admin privileges — so it passes straight through a ruleset that lists `bypass_actors`. CRITICAL fires only on the combination (auto-delete on AND some long-lived branch deletion-reachable); a missing `deletion` rule is HIGH on its own; bypass actors on `main` are MEDIUM. Bypass actors on `develop` are **deliberate** (needed for `release:finalize`'s force-push and the sanctioned direct doc-commit path) and are never a finding on their own. Network-dependent: it degrades to an "unavailable" line and exits 0 when `gh` can't answer. A second invariant: every main-required context must be a job `origin/main`'s `ci.yml` actually produces — matrix legs are expanded from `strategy.matrix` (a `name:` template with `${{ matrix.<key> }}` or GitHub's default `job (values)`) and a context is satisfied only by an EXACT produced context; a job present without that leg is its own HIGH. HIGH on violation, and a report-only WARNING for a develop-only context whose exact context has reached main (`fixup-check` exempt). An unreadable `origin/main` degrades only the required-checks sub-surface (one `unavailable` line), never the deletion-safety verdict, and a missing `origin/main` ref is fetched first (`ensureRef`, shared with `guard:workflow-sync`).
