...
| `pnpm ops lines:check`                       | Always-loaded surfaces (`.claude/rules/*.md` total, `CURRENT.md`, `.claude/skills/*/SKILL.md` total) within their LINE and BYTE budgets (audit-class, `--summary` wired; `--breakdown` ranks every file worst-first by bytes — the trim order)                                                                                                    |
