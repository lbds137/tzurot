...
| Channel/admin settings, autocomplete, preset + browse pickers, voice list, model capabilities, maintenance flag, forwarded-message origins (`forwardedOriginCache`)                                                    | in-memory `TTLCache`                                 | same — read models over rows we own                   |
...
| UI/session state (dashboard `SessionManager`, `ActionTokenService`, `MemoryModeSessionManager`, verification messages, db-sync report stash, nag cooldowns)                                                            | Redis, TTL                                           | losing it ends a UI session; nothing is unrecoverable |
