...
| `OpenRouterModelCache` (Redis)     | **24h** | The OpenRouter model catalog changes ~daily; 24h bounds the upstream API budget. A 5-min in-memory L1 tier sits in front to avoid per-request Redis round-trips. **Proactively refreshed** every TTL/3 by `ModelCatalogRefresher` (api-gateway) — this entry is NOT request-driven, because ai-worker reads the key on every generation but cannot populate it, so a quiet day used to expire it permanently. |
