...
| `guard:boundaries`                                   | `packages/tooling/src/dev/check-boundaries.ts`          | Service-boundary import rules — registered but `--summary` not yet wired (tracked in `backlog/quick-wins.md`); no canary yet either (Layer 1 conditional on emitting `--summary`) |
