...
| "credentials could not be read"            | The cookie is stored but can no longer be decrypted, so it cannot be used at all                            | `/shapes auth` again to replace it; re-harvesting is required                  |
