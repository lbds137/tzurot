...
| `/notifications` | `view` `enable` `disable` `level`                             | Release-notes DM preferences — opted in by default at level `minor`; levels come from release content (breaking→major, features→minor, fixes-only→patch) |
