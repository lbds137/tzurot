...
function testValidationMiddleware() {
  ...
  testCases.forEach(test => {
    ...
    logger.info(
      `Expected: ${test.shouldPass ? 'PASS' : 'FAIL'}, Actual: ${result.success ? 'PASS' : 'FAIL'}`
    );
...
function runAllTests() {
  ...
  logger.info('======================================================');
  logger.info(
    `Validation Tests: ${validationResults.passCount} passed, ${validationResults.failCount} failed`
  );
  logger.info(
    `Args Conversion Tests: ${conversionResults.passCount} passed, ${conversionResults.failCount} failed`
  );
  logger.info(`End-to-End Tests: ${e2eResults.passCount} passed, ${e2eResults.failCount} failed`);

  const totalPassed =
    validationResults.passCount + conversionResults.passCount + e2eResults.passCount;
  const totalFailed =
    validationResults.failCount + conversionResults.failCount + e2eResults.failCount;
