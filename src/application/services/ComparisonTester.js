...
class ComparisonTester {
  ...
  async compareMultiple(operations) {
    const results = await Promise.allSettled(
      operations.map(op => 
        this.compare(op.name, op.legacy, op.new, op.options)
      )
    );
  ...
  _deepCompare(obj1, obj2, options = {}, path = '') {
    ...
    if (customComparators[path]) {
      ...
      return { match: result, discrepancies: result ? [] : [{ path, type: 'custom_mismatch', legacy: obj1, new: obj2 }] };
    }
    ...
    if (Array.isArray(obj1) && Array.isArray(obj2)) {
      if (obj1.length !== obj2.length) {
        discrepancies.push({ path, type: 'array_length_mismatch', legacy: obj1.length, new: obj2.length });
    ...
    for (const key of commonKeys) {
      ...
      if (!compareTimestamps && (key.includes('timestamp') || key.includes('createdAt') || key.includes('updatedAt'))) {
        continue;
      }
      ...
      const result = this._deepCompare(obj1[key], obj2[key], options, path ? `${path}.${key}` : key);
      if (!result.match) {
        discrepancies.push(...result.discrepancies);
      }
    }
  ...
  getStatistics() {
    ...
    stats.overallSuccessRate = stats.totalComparisons > 0 
