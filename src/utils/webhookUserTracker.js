...
function isProxySystemWebhook(message) {
  ...
  if (message.applicationId && KNOWN_PROXY_WEBHOOK_IDS.includes(message.applicationId)) {
    logger.info(`[WebhookUserTracker] Identified proxy system by application ID: ${message.applicationId}`);
  ...
  if (message.embeds?.length > 0) {
    for (const embed of message.embeds) {
      ...
      if (embed.fields?.some(field => 
        (field.name === 'System ID' || field.name === 'Member ID') ||
...
function getRealUserId(message) {
  ...
  if (cachedUserId) {
    logger.debug(`[WebhookUserTracker] Found cached user ${cachedUserId} for webhook ${message.webhookId}`);
    return cachedUserId;
  }
...
function shouldBypassNsfwVerification(message) {
  ...
  if (isProxySystemWebhook(message)) {
    logger.info(`[WebhookUserTracker] Bypassing NSFW verification for proxy system: ${message.author?.username || 'unknown'}`);
    return true;
  }
...
function isAuthenticationAllowed(message) {
  ...
  if (isProxySystemWebhook(message)) {
    logger.info(`[WebhookUserTracker] Authentication not allowed for proxy system: ${message.author?.username || 'unknown'}`);
    return false;
  }
  ...
  if (realUserId) {
    ...
    logger.info(`[WebhookUserTracker] Authentication allowed for webhook with known user: ${realUserId}`);
    return true;
  }
