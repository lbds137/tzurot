...
async function authMiddleware(message, command, args) {
  ...
  if (message.webhookId) {
    ...
    if (command === 'auth' && !webhookUserTracker.isAuthenticationAllowed(message)) {
      logger.warn(`[Auth Middleware] Auth command from proxy webhook denied: ${message.author.username}`);
      return {
        authenticated: false,
        error: 
  ...
  if (!isAuthenticated && !isAuthCommand) {
    logger.info(`[Auth Middleware] Unauthorized user ${message.author.tag} attempted to use command: ${command}`);
    ...
    try {
      ...
      logger.warn(`[Auth Middleware] Failed to send DM to user ${message.author.id}: ${dmError.message}`);
      return {
        authenticated: false,
        error: 
          `**Authentication Required**\n\n` +
