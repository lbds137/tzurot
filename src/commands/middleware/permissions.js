...
function permissionsMiddleware(message, command, commandModule) {
  ...
  if (!commandModule || !commandModule.meta.permissions || commandModule.meta.permissions.length === 0) {
    return {
      hasPermission: true
    };
  }
    ...
    switch (permission) {
      case 'ADMINISTRATOR':
        if (!validator.isAdmin(message)) {
          logger.info(`[Permissions] User ${message.author.tag} lacks ADMINISTRATOR permission for ${command}`);
          return {
            hasPermission: false,
      ...
      case 'MANAGE_MESSAGES':
        if (!validator.canManageMessages(message)) {
          logger.info(`[Permissions] User ${message.author.tag} lacks MANAGE_MESSAGES permission for ${command}`);
          return {
            hasPermission: false,
