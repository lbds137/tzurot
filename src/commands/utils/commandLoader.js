  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        logger.debug(`[CommandLoader] Loaded command module: ${commandModule.meta.name} from ${file}`);
      } catch (error) {
    ...
    logger.info(`[CommandLoader] Loaded ${results.count} command modules successfully (${results.failed.length} failed)`);
  } catch (error) {
