...
- 🧹 `[CHORE]` **Sanity-check ShapesPreflight against `/api/users/info` post-deploy** — Preflight target was swapped from `/api/auth/session` (unverified) to `/api/users/info` (known to exist via `ShapesIncUserProfile` type) in PR #874. On the first `/shapes auth` submit after the swap ships to dev: (1) run `/shapes auth` with a known-good cookie on dev; (2) `pnpm ops logs --env dev --service api-gateway --lines 1000 | grep -i preflight`; (3) expect `"Preflight valid"` with status 200 = close item; `"Inconclusive preflight"` with status 404/5xx = investigate (endpoint moved); `"shapes.inc rejected submitted cookie"` on a known-good cookie = real auth issue. Council-recommended swap (2026-04-22) makes this verification, not gamble. **Start**: `services/api-gateway/src/services/ShapesPreflight.ts`. Promoted from Inbox 2026-04-22.

- 🧹 `[CHORE]` **File OpenRouter issue for GLM-4.5-air fake-user-message reasoning leak** — Companion to PR #875 (shipped 2026-04-22). The Chain-of-Extractors post-processor is our fix, but OpenRouter's reasoning-middleware can polyfill this at the API layer so every consumer benefits. They actively polyfill similar quirks for DeepSeek/Qwen/Llama. **Payload**: attach raw API response from req `b533e288-fb07-46c0-a5e2-a0f78883e63e`, model string `z-ai/glm-4.5-air:free`, trigger `reasoning.enabled=true`. Note the pattern: model wraps CoT in `<from_id>UUID</from_id>\n<user>Name</user>\n<message>reasoning</message>` — structurally distinct from `<think>` but unambiguously a reasoning leak (UUID validation makes it safe to detect). **Exit criterion**: OpenRouter populates `message.reasoning` for this pattern → `GLM_FAKE_USER_MESSAGE_ECHO_PATTERN` and its test suite can be deleted. Surfaced 2026-04-22.
...
- 🧹 `[CHORE]` **Refactor `preset/dashboardButtons.handleDeleteButton` to defer before Redis session lookup** — The `getSessionDataOrReply` helper does a Redis session lookup then either returns data or calls `interaction.reply(SESSION_EXPIRED)`. When called from `handleDeleteButton`, the Redis lookup precedes any `defer*` / `reply`, meaning a slow round-trip would consume Discord's 3-second budget. **Blocker on simple fix**: if the caller does `deferUpdate` first, `getSessionDataOrReply`'s fallback `interaction.reply(SESSION_EXPIRED)` errors because the interaction is already acked. **Fix shape**: add a sibling helper (or refactor existing) that takes an already-deferred interaction and uses `followUp` for the SESSION_EXPIRED branch. Update `handleDeleteButton` to `deferUpdate` first. **Start**: `services/bot-client/src/utils/dashboard/sessionHelpers.ts:192` `getSessionDataOrReply`; `services/bot-client/src/commands/preset/dashboardButtons.ts:210` `handleDeleteButton`. Character's equivalent was fixed in a prior sweep — this is the remaining instance. Surfaced 2026-04-19 during PR #836 r4 review. Promoted from Inbox 2026-04-22.

- 🧹 `[CHORE]` **Add lint/test assertion that dashboard section fields declare `maxLength`** — `detectOverLengthFields` in `services/bot-client/src/commands/character/truncationWarning.ts` intentionally skips fields where `field.maxLength === undefined` because `ModalFactory` applies default caps only at modal-show time. The tradeoff: if a new field is ever added to `services/bot-client/src/commands/character/config.ts` without an explicit `maxLength`, silent-truncate silently re-opens for that field. Currently nothing enforces `maxLength` presence — protection is "discipline + code review." Flagged in PR #825 R3 (2026-04-17). **Fix options**: (a) narrow `FieldDefinition` type to make `maxLength` required on text/paragraph inputs (cleanest long-term); (b) structure.test.ts-style assertion that walks dashboard config and fails CI if any paragraph/short field lacks `maxLength` (cheaper short-term); (c) doc-only JSDoc note (not recommended). **Start**: `services/bot-client/src/commands/character/truncationWarning.ts` `detectOverLengthFields`; `services/bot-client/src/utils/dashboard/types.ts` `FieldDefinition`; `services/bot-client/src/commands/character/config.ts`. Promoted from Inbox 2026-04-22.
...
- 🧹 `[CHORE]` **Automate step 5 of release flow ("rebase develop onto main after merge")** — Git-workflow skill documents this step but nothing enforces it. Got skipped after beta.98 shipped; showed up ~24 hrs later as "conflicts with main" on the beta.99 release PR. Fix shape: (a) add `pnpm ops release:finalize` command that runs `git fetch --all && git checkout main && git pull && git checkout develop && git pull && git rebase origin/main && git push --force-with-lease` with per-step confirmation; (b) alternatively, extend `pnpm ops release:tag` to run the rebase as its final action; (c) add a pre-session-start guard that detects "main has commits develop doesn't have same-SHA" and reminds. **Start**: `packages/tooling/src/commands/release.ts` already has `release:bump`; `release:finalize` fits naturally. Promoted from Inbox 2026-04-22.
