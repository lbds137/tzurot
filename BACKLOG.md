...
- 🐛 `[FIX]` **`checkNsfwVerification` + `checkGuestModePremiumAccess` silently block verified/paid users on API errors — class-of-bug fix** — Two known instances of the same anti-pattern where a tri-state (paid/guest/unknown, verified/unverified/unknown) is collapsed into a binary, causing transient gateway failures to look identical to "user definitively lacks the entitlement." Result: intermittent "Guest Mode" / "Not Verified" lockouts for users who have the entitlement.

  **Known call sites:**
  - `services/bot-client/src/utils/nsfwVerification.ts` — returns `{ nsfwVerified: false }` on throw; flagged by claude-bot review on PR #819
  - `services/bot-client/src/commands/settings/preset/guestModeValidation.ts:97` — `hasActiveWallet = walletResult.ok && ...` treats any `/wallet/list` failure as Guest Mode, blocking premium-model preset selection for users with active paid keys. User-reported 2026-04-21 with screenshot (Kimi K2.6 blocked despite active key)

  **Fix shape**: shared tri-state result type + `ApiResult<T>` helper that callers destructure explicitly. Widening the return types forces every call site to make a fail-open / fail-closed policy choice rather than inheriting the silent fail-closed default. Downstream ai-worker `ApiKeyResolver` + `AuthStep` already enforce authoritatively, so the bot-client pre-flight checks can safely fail-open on API error (worst case: user gets a clearer downstream error instead of a wrong "you don't have access" message).

  **Start**: `services/bot-client/src/utils/nsfwVerification.ts` + `services/bot-client/src/commands/settings/preset/guestModeValidation.ts`; grep for other `*Result.ok && ...` patterns in bot-client that collapse tri-state into binary. Likely a handful more hiding. Per-callsite policy decision documented at implementation time.
