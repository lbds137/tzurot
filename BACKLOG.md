...
- 🏗️ `[LIFT]` **Extend `buildBrowseButtons` to support custom `TSort` generic** — `utils/browse/buttonBuilder.ts` hardcodes `currentSort: BrowseSortType` (`'name' | 'date'`) in both type signature and toggle logic (`currentSort === 'date' ? 'name' : 'date'`). This forces `admin/servers` to maintain a local `buildButtons` for its `'members' | 'name'` sort space — see the comment block on `buildButtons` in `services/bot-client/src/commands/admin/servers.ts`. The fix is parallel to PR #773's `createBrowseCustomIdHelpers` extension: add a `TSort` generic, default to `BrowseSortType` for backward compat, and accept caller-provided toggle logic + per-sort labels/emojis (e.g., `toggleSort: { next: (current) => ..., labelFor: (sort) => { label, emoji } }`). Touches the 7+ existing callers — they should all keep working via the default. Flagged in PR #773 round 7 review. **Start**: `services/bot-client/src/utils/browse/buttonBuilder.ts`; reference admin/servers' `buildButtons` for the custom-sort use case.
- 🏗️ `[LIFT]` **Type-level `includeSort: false` enforcement via discriminated `ParsedBrowseCustomId`** — Currently `parseCustomIdCore` returns `validSorts[0]` as a placeholder when `includeSort: false`, and the contract that callers MUST NOT read `.sort` is enforced via a file-content scan in `browse.test.ts` (see the `includeSort: false contract` describe block). The scan has acknowledged false-negative windows: a future caller storing a parse result in a variable name not in `PARSE_RESULT_VAR_NAMES` would slip past. Stronger enforcement: split `ParsedBrowseCustomId` into two variants (`WithSort<F, S>` has `sort: S`, `WithoutSort<F>` omits the field) and have `BrowseCustomIdHelpers.parse` return the right one based on `includeSort`. This makes reading `.sort` on a `WithoutSort` result a TypeScript compile error instead of a test-time string match. Flagged in PR #773 round 5 + round 7 reviews as "worth a backlog item if the caller count grows" (currently 5). **Start**: `services/bot-client/src/utils/browse/types.ts` for the type split; `customIdFactory.ts` for the conditional return type; the file-scan tests in `browse.test.ts` can then be deleted as redundant.
