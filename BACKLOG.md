...
- 🐛 `[FIX]` **SSRF: encode dynamic path segments in bot-client gateway calls** — 00-critical.md requires `encodeURIComponent` on any dynamic URL segment ("never interpolate user-provided values into URLs without encodeURIComponent"). Several bot-client → gateway calls violate this. Known instances (non-exhaustive): `services/bot-client/src/commands/preset/dashboardButtons.ts` `handleConfirmDeleteButton` — `callGatewayApi(\`/user/llm-config/${entityId}\`, ...)`. `entityId`flows from Discord button customIds (which were built from API-returned UUIDs), so the practical attack surface is low — but the rule says encode as defense-in-depth. **Fix shape**: grep`callGatewayApi\(\``across`services/bot-client/src/`to enumerate every template-literal URL, wrap every interpolated segment in`encodeURIComponent`. Consider adding an ESLint `no-restricted-syntax`rule flagging template-literal URLs in`callGatewayApi`calls that don't pass through`encodeURIComponent`. **Start**: `services/bot-client/src/commands/\*_/_.ts` — expect to touch multiple dashboards. Surfaced 2026-04-19 during PR #836 r4 review (pre-existing — reviewer correctly flagged as "worth a follow-up rather than a blocker").

- 🧹 `[CHORE]` **Audit dashboard button handlers for 3-second rule compliance (un-deferred `reply` + pre-reply async work)** — `handleDeleteButton` in `preset/dashboardButtons.ts` uses `interaction.reply()` (not `deferReply`) for the no-permission path, preceded by a synchronous `getSessionDataOrReply` session lookup. The session lookup is synchronous-ish today (session held in local map), so the 3-second budget isn't threatened, but the pattern is replicated across the four browse-capable commands. Each one should be audited: any sync-or-async work before the first `defer*`/`reply` risks the 3-second rule under load. Related to 04-discord.md "defer first, then process". **Fix shape**: grep `interaction.reply\(` across `services/bot-client/src/commands/**/dashboardButtons.ts` and `dashboardDeleteHandlers.ts`. For each callsite, verify nothing async precedes it. Where async work does precede, switch to `deferReply({ flags: Ephemeral })` + `editReply`. **Start**: `preset/dashboardButtons.ts:214` (known instance). Surfaced 2026-04-19 during PR #836 r4 review (pre-existing — reviewer flagged as "worth a follow-up when the pattern is reviewed across all dashboard handlers").
...
- 🧹 `[CHORE]` **Skip `configsResult` fetch in `checkGuestModePremiumAccess` when wallet confirms paid user** — `services/bot-client/src/commands/settings/preset/guestModeValidation.ts` uses `Promise.all` to fire `/wallet/list` and `/user/llm-config` in parallel, but the configs result is only consumed in the guest-mode path. After the PR #857 refactor, the common paid-user flow through `/settings preset default` and `/settings preset set` returns early with `reason: 'paid'` before ever reading `configsResult` — the configs fetch is pure bandwidth waste. **Fix shape**: switch to serial — await wallet first, return early on paid, then fetch configs lazily only when we need to decide free-vs-premium. Trade-off: adds ~50-200ms latency for guest users (serial round-trip instead of parallel), in exchange for eliminating one round-trip per interaction for paid users. Pre-existing behavior; flagged as a non-blocking nit by the PR #857 r3 review. Low urgency — blast radius is pure bandwidth, not correctness. **Start**: `guestModeValidation.ts` `checkGuestModePremiumAccess`. Surfaced 2026-04-21.
...
- 🐛 `[FIX]` **Kimi K2.6 reasoning leaks as plain text — recommend reasoning-off on K2.x presets + add detection telemetry** — Kimi K2.6 via OpenRouter (`moonshotai/kimi-k2.6`) emits its chain-of-thought as plain narration mingled with the final response, terminated by `"Final decision:\n<actual answer>"`. No `<think>` / `</think>` tags, no orphan closing tag (unlike K2.5's documented quirk), and OpenRouter's `message.reasoning` field is not populated (`hasReasoningInKwargs: false`, `hasReasoningDetails: false`, `hasReasoningTagsInContent: false`). Our thinking-extraction utility correctly finds nothing to extract because nothing is structurally marked — a full 1,500-token reasoning dump reaches Discord as the visible "response." Example: `requestId 3876327d-5e6f-4361-a6ac-5840843584a9` (2026-04-21) — user said `@Cold test`, got 158 seconds of reasoning plus 6 words of actual answer.
  ...
  **Cheap telemetry to add (Option 3, ~5 LOC)**: warn-log in `ResponsePostProcessor` when `config.reasoning?.enabled === true` AND `thinkingExtracted === false`. Tells us per-model how often the parse-miss happens, so when K2.7 ships we detect drift immediately. **Start**: `services/ai-worker/src/services/ResponsePostProcessor.ts` — likely the right place (has both config and extraction result in scope).
