...
- 🏗️ `[LIFT]` **CI test-suite speed investigation** — CI runs spend ~4–5 minutes on the `test` step alone, and local pre-push hook runs ~4 minutes on Steam Deck (concurrency=1). The waiting cost compounds over every PR iteration and every Round-N review cycle. Investigate where the time actually goes before prescribing fixes. **Candidate areas, in priority order**: (1) Real-timer audits — grep `setTimeout|setInterval` in `*.test.ts` without nearby `vi.useFakeTimers()` or `vi.advanceTimers`. Observed 2026-04-24: `AudioProcessor.test.ts` has retry tests showing 3005ms / 3004ms duration lines (real 3s backoffs × several tests = ~12s pure wall-clock burn). Similar patterns likely elsewhere. (2) Test import cost — ai-worker suite's vitest stats showed 63s of `import` time on 2616 tests. Lazy-loading or narrower describes for heavyweight modules (Prisma, LangChain, sharp) could cut it materially. (3) CI concurrency — the pre-push hook sets `--concurrency=1` under `LOW_RESOURCE_MODE` for Steam Deck memory; if the GitHub Actions `test` step inherits the same flag, that leaves parallelism on the floor (runners have more memory than the Deck). **First step when picked up**: run `pnpm ops xray` or equivalent instrumentation to get per-file duration + import time, prioritize fixes by elapsed-seconds-saved-per-run. **Fix shape likely**: `DownloadAttachmentsStep.test.ts` pattern from PR #889 — injectable timing params default to prod values, tests pass `0` (dropped 540ms → 17ms there). Replicating across the ~5 slowest test files could meaningfully cut CI time. Surfaced 2026-04-24.

- 🐛 `[FIX]` **Cap aggregate data-URL payload per LLM-generation job** — `DownloadAttachmentsStep` base64-encodes each attachment into `job.data.context.attachments[].url`, which BullMQ JSON-serializes into a single Redis key. The per-attachment cap (`MAX_ATTACHMENT_BYTES` = 25 MiB) is enforced, but the aggregate is not. For image attachments, `resizeImageIfNeeded` shrinks them to ~1–2 MB each, so this is rarely a concern in practice. **Hazard scenario**: non-image attachments bypass resize. A job carrying, say, 10× 20 MB audio/video files would pass per-attachment checks, produce ~260 MB of base64 in `job.data`, exceed Redis's 512 MB per-key limit, and fail at BullMQ's `JSON.stringify` boundary with an opaque `DataCloneError` (or similar). **Fix shape**: after `Promise.all([downloadAll(...)...])` but before the `job.data.context` mutation, sum the final buffer sizes and throw a classified `JobPayloadTooLargeError` if the total exceeds, e.g., 50 MB. Post-resize sum means images never trip it. ~10 lines. **Why deferred**: low-probability on real Discord traffic (typical message is 0–5 image attachments, all resized), and today's failure mode is a loud BullMQ error rather than silent corruption. Surfaced 2026-04-24 by PR #889 review. Reference: PR #889 Round 3 claude-review.

- 🐛 `[FIX]` **Queue-age gate for AudioTranscriptionJob** — `DownloadAttachmentsStep`'s 12-hour queue-age gate (PR #889) only runs in the LLM-generation pipeline. Transcription jobs (`AudioTranscriptionJob`, handled by `AudioProcessor.fetchAudioBuffer`) fetch Discord CDN URLs directly and will receive an `HTTP 403: Forbidden` (now typed `HttpError` with `status: 403`) if the URL has expired. That IS surfaced to the user as an error, but it lacks the classified `ExpiredJobError` telemetry that the LLM path produces, making it harder to distinguish "CDN URL expired from queue backpressure" from "voice engine unreachable" in dashboards. **Fix shape**: extract `checkQueueAge(job, maxAgeMs)` into `services/ai-worker/src/utils/attachmentFetch.ts` (or a new `jobAgeGate.ts`), call it at the top of `AudioTranscriptionJob.processAudioTranscription`. ~15 lines including test. **Why deferred**: transcription jobs are short-lived (~seconds), so the 12-hour threshold is genuinely unlikely to trigger except in a pathological backpressure incident (which is already a P0). Accepting the inconsistent telemetry today. Surfaced 2026-04-24 by PR #889 review. Reference: PR #889 Round 3 claude-review.

- 🐛 `[FIX]` **Avoid orphaned in-flight downloads when one attachment group fails** — `DownloadAttachmentsStep.process` runs `Promise.all([downloadAll(trigger), downloadAll(extended)])`. If the trigger download rejects first (e.g., a 403 on an expired CDN URL), `Promise.all` rejects immediately but the extended-context download continues to completion in the background — the fetches finish, buffers get allocated and base64-encoded, then GC'd. Bounded waste (BullMQ retry re-queues cleanly), but non-trivial for a 12-attachment extended-context payload. **Fix shape**: switch the outer to `Promise.allSettled`, collect failures from both groups, throw once with a combined message. ~10 lines, preserves the hard-fail semantic but without the wasted work. Alternative: shared `AbortController` across both `downloadAll` calls — more complex, probably overkill. **Why deferred**: no observed user-facing symptom today, and BullMQ retry cycles through cleanly. The dedicated backlog entry for "label trigger vs. extended" and this one might be addressed together in a single error-ergonomics pass when either signal promotes. Surfaced 2026-04-24 by PR #889 Round 8 claude-review.

- 🐛 `[FIX]` **Label trigger vs. extended-context attachments in `DownloadAttachmentsStep` error messages** — When `downloadAll` fails for the extended-context array but the trigger-message array succeeds (or vice versa), the aggregated error reads `"Failed to download N attachment(s): <filename>: <reason>"` — it names the filename but gives no indication which array the failure came from. Minor observability gap: a log-grepping incident responder sees the filename but not the array context. **Fix shape**: pass a `label: 'trigger' | 'extended'` to `downloadAll` and include it in the error message prefix — one-line change + test adjustment. **Why deferred**: the reviewer explicitly called this out as "one-line fix if it ever matters" — zero signal today that incident responders need this disambiguation. Promote if we ever see an attachment-download incident where the array context would've shortened triage. Surfaced 2026-04-24 by PR #889 Round 7 claude-review.

- 🐛 `[FIX]` **Typed aggregated error for `DownloadAttachmentsStep.downloadAll`** — Individual failures inside `downloadAll` are typed (`HttpError`, `AttachmentTooLargeError`, generic `Error` on transient retries), but the step-level aggregation collapses them into an anonymous `new Error('Failed to download N attachment(s): ...')`. Callers (the outer pipeline handler, `LLMGenerationHandler.processJob`'s catch block) can't `instanceof`-classify the aggregated error — they'd have to string-match the message to distinguish CDN expiration from size-cap from transient network. **Acceptable today**: the failure path surfaces as an async error to the user regardless of classification; no retry/backoff policy keys off the aggregated type. **Fix shape when needed**: introduce `AggregateAttachmentDownloadError extends Error` carrying a `failures: Array<{ name: string; error: Error }>` field; keep message format for backwards-compat with logs. **Watch for**: first need would be if we ever want "retry the whole job if ALL failures are transient network errors" vs "fast-fail if any 403" vs "surface different user message based on failure class." Surfaced 2026-04-24 by PR #889 Round 6 claude-review.

- 🐛 `[FIX]` **Reconsider hard-fail vs soft-error for attachment download failures** — PR #889 changed attachment-download failure semantics: the old `api-gateway/AttachmentStorageService.downloadAndStore` used `Promise.allSettled` and returned the original Discord CDN URL as a fallback on per-attachment failure (soft-error; job continued with an unresolved URL), while the new `DownloadAttachmentsStep.downloadAll` throws on any failure and fails the whole job (hard-fail). The new behavior is arguably more correct — the old soft-error would hand the LLM a broken URL and produce weird responses — but it's a visible user-facing change: transient CDN hiccups that were previously silent now produce classified async errors. **Watch-item**: if users report "attachments dropped without warning" turning into "job failed visibly" complaints, reconsider a partial-failure soft-error path (e.g., continue with successful attachments, surface a non-fatal warning for failed ones). **Fix shape when picked up**: change `downloadAll` to keep partial successes and attach failure metadata to the generation context, then surface a human-readable "couldn't load N of M attachments" notice alongside the LLM response. **Why deferred today**: no observed user complaints yet; visible errors are easier to diagnose than silent corruption. Surfaced 2026-04-24 by PR #889 Round 5 claude-review.
