...
- 🏗️ `[LIFT]` **Harden preset clone collision detection against error-message drift** — `NAME_COLLISION_PATTERN` in `services/bot-client/src/commands/preset/dashboardButtons.ts` is a regex match on the gateway's error message text (`/already have a config named/i`). If `ErrorResponses.validationError` wording in `services/api-gateway/src/routes/user/llm-config.ts` ever changes, the retry loop silently stops matching and all clone attempts fail on the first collision. **Fix shape**: return a typed error code from api-gateway (e.g., `{ error: 'NAME_COLLISION', ... }` in the JSON body), and match on the code rather than the message string. Bot-client already parses the JSON via `parseErrorResponse` in `userGatewayClient.ts`, so the plumbing exists. **Why not now**: the current string match works against today's wording and is explicitly documented in the code. Hardening is worth tracking but not urgent. **Surfaced by**: claude-bot review on PR #821, round 2. **Start**: `services/api-gateway/src/utils/errorResponses.ts` (add a name-collision error variant with a stable code); `services/bot-client/src/utils/userGatewayClient.ts` (extend `parseErrorResponse` to return the error code); `services/bot-client/src/commands/preset/dashboardButtons.ts:65` (match on code instead of pattern).
