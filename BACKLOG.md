...
- 🧹 `[CHORE]` **README codecov badge shows 0% while website shows 92.73%** — Badge URL in `README.md:3` is pinned to `branch/develop`: `https://codecov.io/gh/lbds137/tzurot/branch/develop/graph/badge.svg`. Codecov's branch-level coverage aggregation reports 92.73% on `main` (confirmed via website screenshot 2026-04-22) but renders 0% on the `develop` branch URL. Most likely cause: codecov CI uploads coverage on PR runs (against develop) but the branch-aggregate coverage data for `develop` isn't getting computed/stored — possibly because develop-branch CI isn't configured to upload on direct-push commits, only PR merges. **Quick fix**: change `branch/develop` → `branch/main` in README.md:3. Matches the release-state-on-badge convention most projects use. **Deeper investigation (optional)**: verify whether `develop` branch uploads are actually running — if not, the codecov/CI config may have a gap worth closing. **Start**: `README.md:3`; also check `.github/workflows/` for codecov upload steps + `codecov.yml` branch config. Surfaced 2026-04-22 by user after checking the website post-beta.103 release.
