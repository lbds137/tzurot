...
- 🧹 `[CHORE]` **Fix `parseShapesSessionCookieInput` routing inconsistency for bare base64-padded tokens (follow-up to PR #872)** — The `looksLikeCookieString` detection heuristic in `packages/common-types/src/types/shapes-import.ts` routes any input containing `=` into the cookie-string parse path. With the newly-widened regex (PR #872), a bare token like `AAAA...A==` (standard base64 padding) is accepted by `isPlausibleShapesTokenValue` but then mis-routed: `parseShapesSessionCookieInput` sees the `=` in `==`, enters the cookie-string path, fails to find the `__Secure-better-auth.session_token=` prefix, and returns `{ok: false, reason: 'wrong-cookie'}` instead of succeeding. The user-facing error message is then misleading (says "wrong cookie name" when the real issue is the heuristic). **Current state**: the `wrong-cookie` behavior is locked into `shapes-import.test.ts` via the `'mis-routes bare base64-padded tokens to wrong-cookie (known limitation)'` test, so any future change has to update the test intentionally. **Fix (single option remaining)**: tighten `looksLikeCookieString` to key off the cookie name rather than the raw presence of `=`. Suggested by claude-bot review on PR #872 (2026-04-22): `const looksLikeCookieString = input.includes(SHAPES_SESSION_COOKIE_NAME) || input.includes(';');`. A bare `==`-padded token won't contain the cookie name string, so it falls through to the bare-token path correctly. Single `name=value` pairs and full `Cookie:` headers both contain either the cookie name or `;`, so they stay routed correctly. Update the lock-in test to flip `wrong-cookie` → `{ok: true}` when fixing. **Start**: `parseShapesSessionCookieInput` in shapes-import.ts. Flagged on PR #872 (2026-04-22) as pre-existing but newly-visible.

- 🧹 `[CHORE]` **Two tiny polish items on `release:draft-notes` tooling (follow-up to PR #871)** — claude-bot's third review cycle flagged two nits that didn't fit this PR's scope. Both are 3-5 line changes. **(a) Parameterize `--base develop`** in `listMergedPrsSince`: currently hardcoded in `packages/tooling/src/release/github-prs.ts`. If the repo ever renames `develop` or if the tool is lifted to a fork with a different default branch, the query silently returns 0 PRs. Fix: thread a `base?: string` option from the CLI flag (default `'develop'`). **(b) Parameterize the repo URL in `renderMarkdown`**: the `https://github.com/lbds137/tzurot/compare/...` compare URL is hardcoded in `packages/tooling/src/release/notes-format.ts:~163`. Threading `repoUrl?: string` through `RenderOptions` would make the tool forkable without source patches. Low priority since this is a single-repo tool. **Start**: both files named above. Surfaced by claude-bot review on PR #871 (2026-04-22).
...
- 🧹 `[CHORE]` **Verify shapes.inc preflight endpoint `/api/auth/session` post-deploy (follow-up to PR #870)** — PR #870 chose `GET /api/auth/session` as the preflight path based on Better Auth's convention, but the endpoint was not verified against shapes.inc directly before shipping. The graceful-degradation path (`inconclusive` on 404 → persist anyway) means a wrong endpoint choice won't break the auth flow, but it also means the preflight would provide **zero** validity-checking value — silently falling through on every submit. **What to do after the next beta containing PR #870 ships to dev**: (1) run `/shapes auth` with a known-good cookie on dev; (2) grep Railway logs for the `ShapesPreflight` logger output — `pnpm ops logs --env dev --filter "@api-gateway"` piped through `grep ShapesPreflight`; (3) if every entry shows "Inconclusive preflight" with status 404, the endpoint is wrong and needs to be swapped. **Candidate alternatives if /api/auth/session is wrong**: `GET /api/shapes/username/me` (if shapes.inc exposes a "current user" shape endpoint), or a HEAD request to any known GET endpoint. **Start**: `services/api-gateway/src/services/ShapesPreflight.ts` — change `PREFLIGHT_ENDPOINT`. Update the test fixture too. Surfaced by claude-bot review on PR #870 (2026-04-22) — reviewer explicitly noted "a BACKLOG entry to verify the endpoint post-deploy would make the follow-up explicit so it doesn't fall through the cracks."
