...
- 🧹 `[CHORE]` **Make Opus default explicit in `VoiceEngineClient.synthesize`** — The `format` field is omitted from the FormData when `options?.format` is undefined, with a comment saying "defaults to opus". The default actually lives server-side (voice-engine), so the client-side comment silently diverges if the server default ever changes. Low-risk while the services are co-deployed, but a ~1-line fix makes the contract explicit: `formData.append('format', options?.format ?? 'opus')`. **Start**: `services/ai-worker/src/services/VoiceEngineClient.ts` (or wherever synthesize() builds the FormData). Surfaced 2026-04-20 during PR #840 release review.
