...
- 🧹 `[CHORE]` **`auth.ts` delete + status handlers bypass `req.provisionedUserId` (Phase 5c pattern regression)** — `createDeleteHandler` and `createStatusHandler` in `services/api-gateway/src/routes/user/shapes/auth.ts:134–198` are wrapped by `requireProvisionedUser(prisma)` middleware (which attaches `req.provisionedUserId`) but both handlers are typed as `AuthenticatedRequest` (not `ProvisionedRequest`) and re-query the DB with `prisma.user.findFirst({ where: { discordId } })` to re-derive the same internal UUID the middleware already found. `createStoreHandler` in the same file does it correctly (casts to `ProvisionedRequest`, calls `resolveProvisionedUserId`). **Fix shape**: change the two handler signatures to `ProvisionedRequest`, replace the `findFirst` lookups with `resolveProvisionedUserId(req, userService)`, simplify the not-found guards (middleware already ensures user exists). Removes one DB round-trip per call; keeps the file consistent with Phase 5c's invariant. **Why this belongs in Phase 6's integration-test scope**: this is exactly the class of regression (Phase 5c pattern applied inconsistently across sibling handlers in the same file) that the Phase 6 route-level integration tests are designed to catch. Worth a test that asserts "handlers mounted behind `requireProvisionedUser` don't hit `prisma.user.findFirst` by discordId" — it'd make this slip impossible. Surfaced by claude-bot review on release PR #879 rounds 1+2. **Start**: `services/api-gateway/src/routes/user/shapes/auth.ts:134-198`.

- 🧹 `[CHORE]` **Remove unused `_parts` parameter from `processReasoningDetail`** — `services/ai-worker/src/utils/thinkingExtraction.ts:559`. The `_` prefix escape hatch is per `02-code-standards.md` for signatures we don't control (callbacks, interface implementations). `processReasoningDetail` is an internal function — we do control the signature, so the parameter should be removed outright, not prefixed. 2-line fix: remove the param from the signature, remove the call-site argument in `extractApiReasoningContent`. Surfaced by claude-bot review on release PR #879 round 2.
