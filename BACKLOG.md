...
- 🐛 `[FIX]` **Audit `character` / `persona` / `deny` browse flows for the same back-to-browse drop that hit preset** — PR #836 shipped `renderTerminalScreen` + the `browseContext` carry-forward for the preset command, but only the preset file is in `ENFORCED_FILES` of `terminalScreen.structure.test.ts`. The other three browse-capable commands (`/character browse`, `/persona browse`, `/deny browse`) almost certainly drop the back-to-browse affordance on their delete/clone/equivalent terminal actions, same class of bug we just fixed in preset. **Fix shape**: for each command, (a) walk delete/terminal handlers and migrate any raw `editReply({ components: [] })` to `renderTerminalScreen`, (b) if the command has a clone-equivalent action, copy `sourceData.browseContext` to the new session, (c) add the file to `ENFORCED_FILES` in `terminalScreen.structure.test.ts` — the test then ensures the migration stays done. Rule-of-three says do character + persona + deny in one PR each (not a mega-refactor) to keep the diffs reviewable. **Start**: `services/bot-client/src/commands/character/dashboardButtons.ts` (plus sibling `dashboardDeleteHandlers.ts` if present), ditto for persona and deny. Surfaced 2026-04-20 during preset release prep — user flagged that the preset fix only covered one of four browse-capable commands.
...
- 🧹 `[CHORE]` **Widen or eliminate the 3-line opt-out window in `terminalScreen.structure.test.ts`** — The structural test in PR #836 scans for raw `components: []` and checks the three preceding lines for `// intentionally-raw:` marker. If a future author puts the marker on the object-construction comment 4+ lines above the `components: []` line, it won't be found and the test will fail spuriously. Not an issue today with `preset/dashboardButtons.ts` but will bite as more files are added to `ENFORCED_FILES`. **Fix shape**: either (a) require the marker on the same line as `components: []` (stricter but unambiguous), or (b) widen to match any `intentionally-raw:` in the containing function body via a simple block-scope heuristic. Start simple: require same-line. **Start**: `services/bot-client/src/utils/dashboard/terminalScreen.structure.test.ts`. Surfaced 2026-04-19 during PR #836 r4 review.
...
- 🏗️ `[LIFT]` **Typed `TerminalAction<TData>` contract for dashboards opened from /browse** — PR #836 shipped `renderTerminalScreen` (Option A) + a structural test (Option B) to enforce that terminal dashboard handlers preserve the Back-to-Browse affordance. Next step: make the contract **compile-time-enforced** rather than string-match-enforced. **Fix shape**: introduce a `TerminalAction<TData>` interface in `utils/dashboard/` requiring every terminal-action handler to declare `postActionScreen(session, result): TerminalScreenOptions` as a typed field; each command's delete/clone/archive handlers implement it and are invoked via a shared dispatcher. TypeScript then refuses to compile a new terminal action that forgets to supply the post-action renderer — no more relying on grep-based tests or author vigilance. Scope: ~2-3× the size of PR #836 because it reshuffles handler signatures across all four browse-capable commands (character, persona, preset, deny). **Start**: examine PR #836's `renderTerminalScreen` and `terminalScreen.structure.test.ts` as the baseline — the `TerminalAction` interface replaces both with a type-level invariant. Migrate preset first (it's already using the helper), then character/persona/deny. Surfaced 2026-04-19 during PR #836 scoping — user asked for "future insurance" beyond A+B.
