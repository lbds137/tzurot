...
- 🧹 `[CHORE]` **Fix `parseShapesSessionCookieInput` routing inconsistency for bare base64-padded tokens (follow-up to PR #872)** — The `looksLikeCookieString` detection heuristic in `packages/common-types/src/types/shapes-import.ts` routes any input containing `=` into the cookie-string parse path. With the newly-widened regex (PR #872), a bare token like `AAAA...A==` (standard base64 padding) would be accepted by `isPlausibleShapesTokenValue` but then mis-routed: `parseShapesSessionCookieInput` sees the `=` in `==`, enters the cookie-string path, fails to find a `__Secure-better-auth.session_token=` prefix, and returns `{ok: false, reason: 'wrong-cookie'}` instead of succeeding. Previously invisible because the old narrow regex rejected `==` first. **Impact**: only affects users who paste a bare token value (not the full `name=value` string) where their token happens to end in standard-base64 padding. Since the modal instructions point users at DevTools' "Value" column, this is plausible if Better Auth ever emits padded base64 tokens in the wild. **Fix options**: (a) tighten `looksLikeCookieString` to require a recognizable `name=` prefix structure (not just the presence of `=`); or (b) add a `parseShapesSessionCookieInput` test documenting the current `wrong-cookie` behavior for bare `==`-containing tokens to lock it in intentionally. **Start**: `parseShapesSessionCookieInput` in shapes-import.ts. Flagged by claude-bot review on PR #872 (2026-04-22) as pre-existing but newly-visible.
