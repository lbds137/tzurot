...
- 🧹 `[CHORE]` **Template-literal `userId` interpolation in memory-service warn logs** — Three pre-existing warn messages still embed `context.userId` via template literal interpolation instead of structured fields: `services/ai-worker/src/services/MemoryPersistenceService.ts` (×2, "No persona found for user ${context.userId}, skipping LTM storage" and "No persona found for user ${context.userId}, cannot defer LTM"), and `services/ai-worker/src/services/MemoryRetriever.ts` (×1, "No persona found for user ${context.userId} with personality ${personality.name}, ..."). Per `.claude/rules/02-code-standards.md`: _"❌ BAD - String interpolation loses structure."_ **Fix shape**: move userId (and personality.name in the retriever case) into structured fields — `logger.warn({ userId: context.userId }, 'No persona found, skipping LTM storage')`. These also emit with `{}` as the empty first arg — fixing the interpolation also fixes that. Pre-existing, not introduced by PR #863's Pino sweep. Surfaced by claude[bot] review on PR #863. Land directly on develop.

- 🧹 `[CHORE]` **`logger.warn({}, 'message')` empty-object pattern — codebase-wide cleanup candidate** — Multiple warn/info calls pass an empty `{}` as the first argument where no structured fields exist. Functionally equivalent to `logger.warn('message')` with no perf difference (pino elides empty objects), but the `{}` is noise and a pattern-break from structured-logging discipline. **Scope**: `grep -rnE "logger\.(warn\|info\|error\|debug)\(\{\}, " services/ packages/ --include='*.ts' | grep -v '.test.ts'` for current count. **Fix shape**: drop the empty object argument — `logger.warn({}, 'msg')` → `logger.warn('msg')`. Pure syntactic cleanup, no behavior change. Could fold into a future cleanup sweep alongside the existing Pino `{error}` → `{err: error}` backlog item. Low priority. Surfaced by claude[bot] review on PR #863.

- 🧹 `[CHORE]` **PR #864 reviewer follow-up — compress multi-line ReDoS comments to one line each** — The regex fixes in PR #864 landed with 3-4 line explanatory comments on several sites (e.g. `VoiceEngineClient.ts` bounded-quantifier block, `apiErrorParser.ts` adjacent-whitespace-groups block, `AttachmentStorageService.ts` DNS-trailing-dots block, `responseArtifacts.ts` polynomial-slide block, and `errorSanitization.ts` which got an expanded JSDoc). Violates `.claude/rules/02-code-standards.md`: _"Never write multi-paragraph docstrings or multi-line comment blocks — one short line max."_ **Fix shape**: compress each to a single line retaining the WHY (ceiling rationale) but not the WHAT (the `{1,64}` bound is visible in the regex). Reviewer's template: `// ReDoS: {1,64} ceiling; 0-1 trailing slashes in practice.` **Affected files**: `services/ai-worker/src/services/voice/VoiceEngineClient.ts`, `services/ai-worker/src/utils/apiErrorParser.ts`, `services/ai-worker/src/utils/responseArtifacts.ts`, `services/api-gateway/src/services/AttachmentStorageService.ts`, `services/bot-client/src/utils/errorSanitization.ts`. Land directly on develop. Surfaced 2026-04-21 in PR #864 review.

- 🧹 `[CHORE]` **PR #864 reviewer follow-up — add regression tests for bounded ReDoS patterns** — PR #864's regex fixes passed existing tests, but no new tests explicitly exercise the boundary conditions of the new bounded quantifiers. If a future touch changes the ceiling or the pattern structure, silent regressions (e.g., stack trace no longer detected because `\s{1,16}` got narrowed) would slip through. **Fix shape**: per file, add targeted cases: (a) `responseArtifacts.test.ts`: does `\s{0,64}<reactions>` still match a reactions block with leading whitespace? (b) `errorSanitization.test.ts`: does `\s{1,16}at\s{1,4}\w` still detect `"    at SomeClass.method"`? (c) `apiErrorParser.test.ts`: does the new bounded `statusPattern` match `"HTTP status code: 429"`, `"status: 500"`, `"status=404"`? (d) `WebhookManager.test.ts`: does `\s{0,16}#\d{4}$` still strip Discord discriminators from tags? (e) `personalityMentionParser.test.ts`: does `[punct]{1,16}$` still match real-world trailing punctuation? Non-blocking nice-to-have. **Start**: existing `.test.ts` files next to each modified source file. Surfaced 2026-04-21 in PR #864 review.

- 🧹 `[CHORE]` **PR #863 reviewer follow-up — 3 missed `{ error: var }` sites where `var` is an Error instance** — The Pino `{error}` sweep in PR #863 matched ES6 shorthand `{ error }` / `{ error, ... }` but didn't match `{ error: <non-error-variable> }` (key named `error:` with a differently-named value). Reviewer on PR #863 identified 3 sites where the value IS actually an Error instance and should use `err:`: (a) `services/bot-client/src/utils/dashboard/SessionManager.ts` — two sites, `{ error: cleanupErr, ... }` inside `.catch(cleanupErr => ...)` blocks; (b) `services/ai-worker/src/services/UserReferenceResolver.ts:~293` — `{ field: fieldName, error: result.reason, personalityId: ... }` where `result` is a `Promise.allSettled()` rejected entry (compare against `AttachmentProcessor.ts` in the same PR which correctly uses `{ err: result.reason }` for the identical pattern). **Fix shape**: same rename — `error: cleanupErr` → `err: cleanupErr`, `error: result.reason` → `err: result.reason`. Three-line change, no tests needed (behavior-preserving). Also: minor cosmetic, `services/api-gateway/src/middleware/errorHandler.ts` has `'Unhandled error:'` with trailing colon that pre-existed — drop the colon while here if bundling. Surfaced 2026-04-21. Land directly on develop after PR #863 merges.

- 🧹 `[CHORE]` **PR #862 reviewer follow-up — add encoding-exercising tests** — PR #862 (SSRF admin-fetch encoding) added `encodeURIComponent` wrappers but the existing tests for `preset/api.test.ts` and `admin/usage.test.ts` use test fixtures with no special characters (`preset-123`, `7d`), so removing the encoding call wouldn't break them. Reviewer flagged this as non-blocking. **Fix shape**: add analogous tests per the gold-standard at `services/bot-client/src/commands/inspect/lookup.test.ts:323-327` (tests with `req/with/slashes` and asserts URL contains `req%2Fwith%2Fslashes`). For `preset/api.test.ts`: a `presetId` like `'preset/with/slash'` → assert URL contains `preset%2Fwith%2Fslash`. For `admin/usage.test.ts`: unlikely to matter since `timeframe` is an enum, but a `'7d with spaces'` test for consistency. **Start**: `services/bot-client/src/commands/preset/api.test.ts`; `services/bot-client/src/commands/admin/usage.test.ts`. Surfaced 2026-04-21 during PR #862 review.

- 🧹 `[CHORE]` **Codebase-wide Pino `{ error }` → `{ err: error }` sweep** — Reviewer on PR #860 flagged that `logger.error({ error }, …)` silently discards stack traces because Pino's default serializer only fires on the `err` key. Using any other key name means the Error object lands in a plain field without the `err.stack` / `err.message` / `err.type` unpacking that makes prod logs useful. **Scope**: `grep -rn "logger\.\(error\|warn\)({ error" services/ packages/ --include="*.ts"` returns 28 production-code sites (excluded test files). Every one is a lossy log. **Fix shape**: mechanical rename — `{ error }` → `{ err: error }`, `{ error, ...rest }` → `{ err: error, ...rest }`. Zero behavior change for happy paths; meaningful improvement for any investigation that needs a stack trace. Can be scripted (`perl -i -pe 's/\{ error(,|\s*\})/{ err: error$1/g'` across the 28 files) followed by review + `pnpm test`. **Not urgent** — the bug is "lossy observability" not "broken behavior" — but low-cost to clear. Could bundle into next cleanup-sweep pass. **Start**: the 28-site grep above; cross-reference against `.claude/rules/02-code-standards.md` "Pino Logger Format" section which already documents the right pattern. Surfaced 2026-04-21 during PR #860 review cycle 2 (reviewer called the incognito.ts incidental fix "a real bug, not cosmetic").
...
- 🐛 `[FIX]` **Post-`/character create` dashboard has no Delete button** — The flow for creating a new character lands on a dashboard that's missing Delete from its action set. The dashboard served from `/character browse → select` does have Delete, so the two entry points produce different dashboards with different capabilities. Users who want to immediately delete a character they just created have to go `/character browse → select → Delete` as a separate trip. **Fix shape**: trace both code paths in `services/bot-client/src/commands/character/` — likely `create.ts` builds a different component set than `browse.ts`/`dashboardButtons.ts`. Either (a) unify so all paths go through the same dashboard factory with the same action set, or (b) add Delete explicitly to the post-create dashboard. Option (a) is the right structural answer and folds into the "Preset cascade standardization → UX alignment" epic. Surfaced 2026-04-20 during beta.102 RC testing.

- ✨ `[FEAT]` **Add a Create button inside `/X browse` view for convenience** — Currently creating new entries requires a separate `/X create` invocation. Putting a Create button in the browse embed would streamline the "I want to add one more" loop. **Fix shape**: add a new button to the `buildBrowseButtons` call across the four browse-capable commands; each routes to a modal or the existing `/X create` flow. **Considerations**: modal vs. deferred-reply-to-existing-flow; where to source the initial slug/name (user types in modal vs. click-through); how to preserve the browse page+filter state so the refreshed list shows the newly-created item where it sorts. Folds into the "Preset cascade standardization → UX alignment" epic. Surfaced 2026-04-20 during beta.102 RC testing.

- 🏗️ `[LIFT]` **Persona create UX should mirror character create UX (plus edit-in-place)** — `/persona create` drops you into a different shape than `/character create` — separate invocations needed to view/edit the persona after. Users should not have to remember command-per-verb; a single create flow should land on a dashboard that supports view/edit/delete in place. This is a specific case of the broader cascade-UX-inconsistency problem. Fold into the "Preset cascade standardization" epic which already flags cross-cascade UX alignment as its Step 2. **Start**: `services/bot-client/src/commands/persona/create.ts` vs. `services/bot-client/src/commands/character/create.ts` + their respective `dashboardButtons.ts` / post-create handlers. Compare the component sets and the post-create interaction flows. Surfaced 2026-04-20 during beta.102 RC testing.

- 🏗️ `[LIFT]` **Bot-owner/admin should be able to delete any preset** — Currently preset delete is owner-only at the API-gateway level, which means a bot-owner managing the global preset set on behalf of users can't intervene when a user's config is misconfigured or abandoned. Admin override capability would be valuable for moderation/maintenance. **Fix shape**: extend the admin delete endpoint at `services/api-gateway/src/routes/admin/` to allow deleting any LlmConfig regardless of owner; add a matching bot-client affordance (either a separate `/admin preset delete <id>` slash command, or promote the existing user-facing delete button to work-when-admin on any preset with an admin confirmation). The config-override cascade should be audited for the same gap. **Start**: `services/api-gateway/src/routes/admin/llm-config.ts` (expected location), `services/api-gateway/src/services/LlmConfigService.ts` for the ownership check that will need a bypass. Surfaced 2026-04-18 during user note-taking.

- 🏗️ `[LIFT]` **Preset cascade standardization — character-tier slash command + UX alignment with config-override pattern** — The preset cascade (`LlmConfigResolver.resolveConfig`) has user-tier commands (`/settings preset default`, `/settings preset set <personality>`, `/settings preset clear-default`) but **no character-tier UX** for a personality creator to pin their character to a specific preset. Historically this was "filled" by the auto-pin bug that PR #853 removed — `setupDefaultLlmConfig` snapshotted the global default into `personality_default_configs` at creation time, fossilizing characters against stale presets and breaking user-cleared-default fallback. With auto-pin gone, personalities correctly cascade to the current global default, but the creator has no way to opt in to a deliberate pin through any slash command or dashboard.

  **Contrast with the config-override cascade** (sampling, memory, reasoning, vision): has dashboards at **every tier** — `/settings defaults edit`, `/channel settings`, `/channel context`, per-personality override via `user_personality_configs.configOverrides`. Preset commands are flat args at the user tier only. The asymmetry is probably why the auto-pin bug slipped through — someone building preset stopped at user-tier because "character tier is set at creation, done." Config-override authors were forced to think about creator/editor UX and got it right.
  ...
  **Fix shape (multi-PR epic)**:
  ...
  3. Consider whether the current resolver priority (`user-per-personality → user-default → personality-default → global-default`) is actually right, or whether users expect `user-default` to supersede `personality-default`. Council consultation 2026-04-20 flagged this as a genuine design question, not a bug.
  ...
  **Start**: `packages/common-types/src/services/LlmConfigResolver.ts:141` (cascade logic), `services/bot-client/src/commands/settings/preset/` (existing user-tier commands, use as template), `services/bot-client/src/commands/character/dashboardButtons.ts` (add section), `services/ai-worker/src/jobs/ShapesImportHelpers.ts:41` (shapes pin path to preserve). Surfaced 2026-04-20 during Kimi-K2.5-routing bug triage (see PR #853). User feedback: _"the defaults should be set by the character creator... there should be a slash command for them to do that"_ and _"this cascade pattern should be a little bit standardized."_
...
- 🏗️ `[LIFT]` **Shared PGLite test factory to DRY `extensions: { vector, citext }` across 16 int-test files** — The citext PR (#841, 2026-04-21) added the `citext` PGLite contrib extension alongside `vector` to every int-test file that constructs its own `new PGlite({ extensions: { vector, citext } })`. 16 identical call sites (15 int-test + 1 e2e-test). Each new PGLite-dependent Postgres extension means another 16-site sed pass — rule-of-three is well past. **Fix shape**: introduce `createTestPGlite()` factory in `packages/test-utils/src/setup-pglite.ts` that returns a PGLite instance pre-loaded with the project's standard extension set (`vector`, `citext`, plus whatever comes next). Migrate all 16 call sites to `const pglite = createTestPGlite();`. Benefits: new extension = 1 edit instead of 16; easier to reason about "what extensions are bundled in tests"; closes a rule-of-three gap noted during the citext PR. **Start**: `packages/test-utils/src/setup-pglite.ts` (factory home), `grep -l "new PGlite({ extensions:"` for call sites. Surfaced 2026-04-21 during PR #841 citext migration.
...
- 🐛 `[FIX]` **SSRF: encode dynamic path segments in bot-client `adminFetch` / `adminPutJson` calls (follow-up to PR #859)** — PR #859 swept `callGatewayApi` template-literal URLs, but intentionally excluded the admin-side HTTP helpers as a distinct sweep. Three known sites remain: `services/bot-client/src/commands/preset/api.ts:72` (`adminFetch` — `presetId`), `preset/api.ts:125` (`adminPutJson` — `presetId`), `admin/usage.ts:25` (`adminFetch` — `timeframe`). **Fix shape**: identical to the callGatewayApi sweep — wrap each interpolated segment in `encodeURIComponent`. Low urgency: `presetId` is an API-returned UUID, `timeframe` is a slash-command enum; both are ASCII-safe in practice. But 00-critical.md mandates the encoding as defense in depth. **Start**: `services/bot-client/src/commands/preset/api.ts`, `services/bot-client/src/commands/admin/usage.ts`. Surfaced by claude-bot review on PR #859 — 2026-04-21.
...
- 🧹 `[CHORE]` **Adopt `eslint-plugin-regexp` to catch ReDoS patterns at lint time** — We've shipped five separate ReDoS fixes that all reached CI/CodeQL review before being caught: `01b2bea6f` (ERROR_SPOILER char-class), `376f4f9fa` (ERROR_SPOILER unbounded quantifier), `aa8754cea` (preset clone nested quantifier), `15ece5a31` (CHIMERA_ARTIFACT `[\s]*` → `[\s]{0,50}`), `7f8e0e702` (current PR — `\s*...\s*$` polynomial slide). All five share one class of failure: unbounded `*`/`+` quantifiers (usually on `\s` or character classes) that combine with regex engine backtracking or starting-position slide. CodeQL catches them on PR but only after push; local IDE and pre-commit are blind. **Fix shape**: add `eslint-plugin-regexp` (v3.1.0, ESLint 9.38+ compat, Node 22/24 compat) as devDep. Enable at minimum `regexp/no-super-linear-backtracking` (classic exponential ReDoS) and `regexp/no-super-linear-move` (polynomial-slide case — the rule that would have caught today's bug). Consider the plugin's `recommended` config for broader coverage. **Rollout**: run `pnpm lint` after enabling, audit every flagged regex (likely a dozen or so across ai-worker/bot-client) — for each hit, either fix (preferred) or add a targeted `eslint-disable-next-line` with a concrete justification (see `.claude/rules/02-code-standards.md` Lint Suppression Standards). Then flip the rule severity to `error` in `eslint.config.js`. Gets us local red squiggles + pre-commit block + keeps CodeQL as defense in depth, not primary detection. **Start**: `eslint.config.js` top of file; `package.json` devDependencies; run `pnpm lint` to see initial hit count. Surfaced 2026-04-19 after CodeQL flagged PR #834's preset-clone regex for the same class of bug we'd already fixed 4 times.
...
- 🧹 `[CHORE]` **Extend Pino logger-prefix normalization to ai-worker + api-gateway** — Follow-up to the bot-client sweep: same pattern (module-name-in-message-string), same Pino convention (`createLogger(name)` tag is redundant with in-message prefix). bot-client was normalized on 2026-04-21 (~123 occurrences across 48 files via a single perl pass) with no behavior change — the same perl regex should work across the other packages. **Fix shape**: `find services/{ai-worker,api-gateway}/src -name '*.ts' ! -name '*.test.ts' -exec perl -i -0777 -pe 's/(logger\.\w+\([^'"'"'"`]\*?['"'"'"`])\[[^\]]+\]\s+/$1/g' {} +` then typecheck, test, dedup-log-string check (sonarjs may flag newly-duplicated literals per PR #859's incognito.ts case — extract to a constant when it does). **Start**: same perl one-liner scoped to the new packages; verify no test-file assertions break (bot-client had zero such dependencies, other packages likely similar).
...
- 🧹 `[CHORE]` **Unify "8 MB" vs "8 MiB" terminology across Discord upload-limit references** — `DISCORD_LIMITS.FILE_UPLOAD_MAX_BYTES = 8 * 1024 * 1024` is **8 mebibytes** (8,388,608 bytes), not 8 megabytes (8,000,000 bytes) — but the constant's JSDoc at `packages/common-types/src/constants/discord.ts:72` says `(8 MB)`, and the user-facing notice text in `DiscordResponseSender.fetchTTSFiles` (shipped 2026-04-19 in PR #831) also says "Discord limit 8 MB". The mixing was flagged by PR #831 R1 claude-bot review as non-blocking because (a) Discord's own docs use both terms interchangeably, (b) the practical difference is ~5%, and (c) "8 MB" is the more widely-recognized colloquial form. **Fix shape**: decide on one convention for internal code-comments (prefer MiB for precision) and keep user-facing strings in MB (matches Discord's own UI). Update `packages/common-types/src/constants/discord.ts:72` JSDoc to say `(8 MiB)` — that's the load-bearing one since the constant value is in binary units. Leave the user-facing text in DiscordResponseSender as "8 MB" unless Discord itself switches. **Low urgency**: non-blocking, purely cosmetic. Folded in when the next substantive touch to these files happens (e.g., the Pino logger-normalization pass above, or any extension of the attachment-size handling). Surfaced 2026-04-19 in PR #831 review.
...
- 🧹 `[CHORE]` **Tighten `requireProvisionedUser` shadow-mode to strict 400** — `services/api-gateway/src/services/AuthMiddleware.ts:194-200` already plans this cutover. Safe once Phase 5c PR C (shell-path migration) prod canary stays clean for 48-72h. Changes the middleware to return 400 on missing/malformed user-context headers instead of falling through. Removes the fallback branch from `resolveProvisionedUserId` + `getOrCreateInternalUser`. Surfaced 2026-04-21 as Phase 5c PR C follow-up.

- 🧹 `[CHORE]` **Final Phase 5c cleanup — delete `getOrCreateUserShell` method + canary log + helper fallback branches** — After the shadow-mode tightening above lands and canary confirms zero hits, the shell path is truly dead. Delete `UserService.getOrCreateUserShell` + its `[Identity] Shell path executed` canary log. Both `resolveProvisionedUserId` and `getOrCreateInternalUser` collapse to passthroughs reading `req.provisionedUserId` directly. Surfaced 2026-04-21 as Phase 5c PR C follow-up.

- 🧹 `[CHORE]` **Route-level integration tests exercising the middleware-attached `provisionedUserId` path** — Pre-existing pattern across all api-gateway route tests: `requireProvisionedUser` is mocked as a no-op that calls `next()` without attaching `req.provisionedUserId`. So every route test exercises the shadow-mode fallback branch of `resolveProvisionedUserId` / `getOrCreateInternalUser`, not the common provisioned path. Helper-level tests in `resolveProvisionedUserId.test.ts` + `userHelpers.test.ts` cover the provisioned branch with structural assertions (no DB calls), so there's a safety net. But route-level zero-DB-round-trip property isn't verified at route depth. Flagged by claude-bot review on PR #858. **Fix shape**: for each route that calls `resolveProvisionedUserId` or `getOrCreateInternalUser`, add at least one test that mocks the middleware to attach `provisionedUserId` + `provisionedDefaultPersonaId`, and assert the handler doesn't hit `prisma.user.findUnique` / `$executeRaw`. Pre-existing gap, not introduced by PR #858. **Start**: any of `nsfw.test.ts`, `timezone.test.ts`, `config-overrides.test.ts` for a reference pattern once one is written.

- 🧹 `[CHORE]` **ESLint `no-restricted-syntax` rule banning `UserService.getOrCreateUserShell`** — Blocks reintroduction of the shell path. Do after the method is deleted (above), otherwise the allowlist bookkeeping is noisy. Once deleted, this rule is enforcing "don't bring this back." Surfaced 2026-04-21.
