...
- 🐛 `[FIX]` **Backtick punctuation breaks @mention parsing** — `personalityMentionParser.ts` trailing-punctuation regex `/[.,!?;:)"'*_~|]+$/` and the single-word terminator set `[.,!?;:)"*_~|]` both exclude the backtick. Discord backtick-code formatting around a mention (`` `@Lilith` ``, common for visual emphasis or when users quote a mention name) would fail to parse — the backtick would either attach to the name as a character or prevent the terminator match entirely depending on flow. Surfaced by PR #811 reviewer as a pre-existing issue (out of scope for the periods fix). Fix shape: add `` ` `` to both regex character sets in the module. Minimal risk, backtick is never part of a legitimate personality name. **Start**: `services/bot-client/src/utils/personalityMentionParser.ts` — both `WORD_PUNCTUATION_STRIP_ALL` and the single-word regex's terminator group. Add tests for `` `@Lilith` `` and `` `@Dr. Gregory House` `` (backticks plus periods).
...
- 🐛 `[FIX]` **AbortError in image processing causes long AI-generation delays** — **PROMOTED 2026-04-15 → Current Focus beta.98 bundle, Item B.** This entry is the execution reference: verified log counts, root cause analysis, and step-by-step fix space. The Current Focus bullet is the summary; this entry is what to re-read when starting the work. — Images getting stuck somewhere in the attachment/URL/vision pipeline blocks the full reply path, occasionally making a single message take a very long time end-to-end. Observed in production across 2026-04-12 and 2026-04-13. May share root cause with Pocket TTS intermittent failures (both hint at resource contention or an unbounded concurrent fetch). Related: "Improve Railway log search DX" below — digging into this one is currently painful and is part of why the observation keeps surfacing without a fix.
...
### Identity & Provisioning Hardening Epic — Phase 6 (next session)

Phases 1-5 all shipped today (2026-04-16): PR #803 (Phase 1 tactical heal, beta.97); PRs #807/#808 (Phase 2 provisioning choke point, beta.98); PR #814 (Phase 3 read-only PersonaResolver); PR #816 (Phase 4 killed discord:XXXX); Phase 5 (DB-level invariants — Restrict FK + unique (ownerId, name) + snowflake CHECK constraint) shipping next. Phase 5b (NOT NULL + shell redesign) deferred to a BACKLOG Inbox item — requires a separate ~150-250 LOC change.

Next phase — **Phase 6: integration test coverage for the refactor-regression class**. Goal: the `c88ae5b7` class of regression fails loudly in tests. End-to-end test exercising "user hits HTTP route → later Discord interaction → system prompt correctness assertion." Estimated ~2 days. Entry point: `docs/reference/architecture/epic-identity-hardening.md § Phase 6`.
