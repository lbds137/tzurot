...
- 🧹 `[CHORE]` **Extend `PINO_LOGGER_RULES` to cover `info` and `debug` levels** — The ESLint rule in `eslint.config.js` currently enforces structured-field discipline only for `logger.warn` and `logger.error`. A future developer writing `logger.info(\`Loaded persona ${id}\`)`or`logger.debug(\`Cache hit for ${key}\`)`won't get a lint error, even though the same template-literal-loses-structure anti-pattern applies. Pre-existing gap; not introduced by PR #865, but PR #865 tightened the rule for`warn`/`error`(added`TemplateLiteral`to the selector), making the asymmetry more visible. **Fix shape**: duplicate the two rule blocks in`PINO_LOGGER_RULES`for`info`and`debug`with identical`:matches(Identifier, MemberExpression, TemplateLiteral)`selectors. Consider factoring into a helper that generates the 4 rule variants from a single selector-body since the rules diverge only by level name. **Scope audit first**: run`pnpm lint --force`after adding the rules to see how many`info`/`debug`sites currently violate; if large, stage the fix across multiple PRs (one rule-enable + per-service fix PRs). **Start**:`eslint.config.js` `PINO_LOGGER_RULES` constant (lines ~17-33). Surfaced by claude[bot] review on PR #865 (2026-04-21).

- 🧹 `[CHORE]` **Narrow `PINO_LOGGER_RULES` to also catch `CallExpression` first args (regression from PR #865)** — PR #865 narrowed the selector from `:not(ObjectExpression)` to `:matches(Identifier, MemberExpression, TemplateLiteral)` to allow legitimate bare-string calls. This regressed one edge case the old rule caught: `logger.warn(buildError(), 'msg')` where the first arg is a function call returning an Error. Rare in the codebase today, but would slip through silently if it appeared. **Fix shape**: add `CallExpression` to the `:matches()` list in both `error` and `warn` selectors: `:matches(Identifier, MemberExpression, TemplateLiteral, CallExpression)`. **Caveat**: this will also fire on bare-value producers like `logger.warn(getMessage(), ...)` where `getMessage()` returns a string. If that pattern exists and isn't an Error, the rule will false-positive — suppressions with clear justification would be needed. **Start**: `eslint.config.js` `PINO_LOGGER_RULES` constant. Surfaced by claude[bot] review on PR #865 (2026-04-21). Bundle with the `info`/`debug` extension above since both touch the same constant.
