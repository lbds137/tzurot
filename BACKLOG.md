...
- 🧹 `[CHORE]` **Harmonize `UserService` instantiation pattern across api-gateway routes** — Inconsistency observed on PR #881: `memory.ts` instantiates `userService` in the outer factory closure and threads it into each handler as a parameter, while `usage.ts`, `listKeys.ts`, `removeKey.ts`, `testKey.ts`, `personality/list.ts`, `personality/delete.ts`, and others each call `new UserService(prisma)` inside their own factory function. Functionally equivalent (both run once at startup), but a reader encountering both patterns reasonably wonders whether there's a semantic difference. **Fix shape**: pick one pattern (probably "instantiate in outer factory, thread through handlers" since it's more testable — a fake `UserService` can be injected during test) and harmonize. ~1 hour of mechanical refactor across ~10 files. **Surfaced by**: claude-bot review on PR #881 round 3 (2026-04-23).
...
- 🧹 `[CHORE]` **PGLite integration tests for Phase 5 CHECK constraints** — `personas_name_non_empty` and `personas_name_not_snowflake` (Phase 5, 2026-04-16) are DB-level invariants. Prisma doesn't represent CHECK constraints in the schema, so they have zero automated regression coverage today — a future migration that accidentally drops them (or a `drift-ignore.json` rule that over-matches) would pass all existing tests. **PGLite respects CHECK constraints** even though Prisma doesn't represent them, so no real-Postgres fixture is needed — this can be done in the existing `*.int.test.ts` harness via raw SQL inserts. **Fix shape**: one `.int.test.ts` with 4 tests total (2 invariants × accepts-good-value + rejects-bad-value). ~2h. **Descoped from Phase 6 per council review 2026-04-23**: orthogonal to the "catch Phase 5c drift" goal, and the PGLite-supports-CHECKs finding makes the "real-Postgres fixture" premise obsolete. Surfaced by claude-bot review on PRs #817, #819.
...
- 🧹 **Tighten `requireProvisionedUser` shadow-mode to strict 400** — `services/api-gateway/src/services/AuthMiddleware.ts:194-200` already plans this cutover. Safe once Phase 5c PR C (shell-path migration) prod canary stays clean for 48-72h. Middleware returns 400 on missing/malformed user-context headers instead of falling through. Removes the fallback branch from `resolveProvisionedUserId` + `getOrCreateInternalUser`. **Prerequisite**: the ~35 api-gateway route test files currently mock `requireProvisionedUser` as a no-op that doesn't attach `req.provisionedUserId`, so every one exercises the shadow-mode fallback. When strict mode lands, all 35 break en masse. Phase 6 lands a `setupProvisionedRequestMock(discordId)` helper in `services/api-gateway/test/` with a reference TODO comment showing the migration pattern — this cutover PR is the one that applies the helper to all 35 files before flipping the middleware to strict. Treat the migration and the strict flip as one atomic change.
...
- 🧹 **Route-level integration tests exercising the middleware-attached `provisionedUserId` path** — Pre-existing pattern across all api-gateway route tests: `requireProvisionedUser` is mocked as a no-op that calls `next()` without attaching `req.provisionedUserId`. So every route test exercises the shadow-mode fallback branch of `resolveProvisionedUserId` / `getOrCreateInternalUser`, not the common provisioned path. Helper-level tests in `resolveProvisionedUserId.test.ts` + `userHelpers.test.ts` cover the provisioned branch with structural assertions (no DB calls), so there's a safety net. But route-level zero-DB-round-trip property isn't verified at route depth. Flagged by claude-bot review on PR #858, and re-surfaced on PR #880 where the `auth.test.ts` cleanup made the gap more visible — `createMockReqRes()` builds requests with `userId` but not `provisionedUserId`, so the post-cleanup delete/status tests still exercise the shadow-mode fallback despite the handlers now being Phase-5c-correct. **Fix shape**: for each route that calls `resolveProvisionedUserId` or `getOrCreateInternalUser`, add at least one test that mocks the middleware to attach `provisionedUserId` + `provisionedDefaultPersonaId`, and assert the handler doesn't hit `prisma.user.findUnique` / `$executeRaw`. **Stronger invariant worth considering**: an `auth.test.ts`-style sweep that asserts "for all routes mounted behind `requireProvisionedUser`, no handler calls `prisma.user.findFirst({ where: { discordId } })`" — that would catch the within-file Phase-5c drift (like `createDeleteHandler`/`createStatusHandler` vs. `createStoreHandler` in PR #880) automatically rather than per-handler. **Start**: any of `nsfw.test.ts`, `timezone.test.ts`, `config-overrides.test.ts`, `auth.test.ts` for a reference pattern once one is written.
