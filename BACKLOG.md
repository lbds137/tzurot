...
- 🐛 `[FIX]` **GLM-4.7 untagged thinking-leak (~11% rate, intermittent)** — Observed 2026-04-25 in inspect-log for request `30a5af6d-6c29-458d-a6ed-9f817f1f6364` (Lila personality, GLM-4.7). Model dumped ~2000 chars of meta-planning prose at the top of the response: third-person summary, reasoning bullets ("My reaction as Lila Elyona:"), tone planning, key-themes outline, structure outline, format reminders ("No quotation marks for dialogue"). NO XML tags — purely unstructured prose. The existing `GLM_47_META_PREAMBLE_PATTERN` (PR #888) catches `<user>/<character>/<analysis>` tag-wrapped leaks; this untagged shape slips past entirely. **Diagnostic data (2026-04-25, prod, last 14 days)**: GLM-4.7 = 16/18 = 88.9% successful reasoning-channel extraction (2 leaks). GLM-5.1 = 4/4 = 100%. GLM-4.5-air:free = 19/19 = 100%. So the issue is **GLM-4.7-specific intermittent failure** (~11% leak rate), not a systemic broken pipeline. The rest of the time the `OpenRouterFetch` interceptor correctly extracts `message.reasoning` and injects as `<reasoning>` tags. **Sample size is small** (2 leaks over 2 days at this traffic level); tomorrow's data will refine the rate estimate. **Decision space**: (a) accept 11% leak rate for the 3.3x cost savings vs. GLM-5.1, (b) roll back to GLM-5.1 for 100% reliability, (c) try GLM-4.7-flash (6x cheaper than 4.7) and measure its leak rate, (d) implement heuristic post-processor for untagged meta-prose (risky — false-positive surface against legitimate intro paragraphs). Diagnostic script: `scripts/src/glm47-reasoning-rate.ts` (run via `pnpm ops run --env prod --force tsx scripts/src/glm47-reasoning-rate.ts`). Inspect-log evidence: `/home/deck/.claude/uploads/eb24bfc9-a1a8-4fcc-a360-b166913664d2/019dc301-debugcompact30a5af6d6c29458da6ed9f817f1f6364.json`. Surfaced 2026-04-25 by user inspect-log review post beta.106 release.
