...
- 🧹 `[CHORE]` **Sanity-check ShapesPreflight against `/api/users/info` post-deploy** — The preflight target was swapped from `/api/auth/session` (Better Auth convention, unverified) to `/api/users/info` (known to exist — `ShapesIncUserProfile` type is already referenced in our fetcher type signatures). With this swap the original "does the endpoint exist?" uncertainty is reduced to a sanity-check: on the first `/shapes auth` submit after the swap ships to dev, verify the preflight fires correctly. **Procedure**: (1) run `/shapes auth` with a known-good cookie on dev; (2) `pnpm ops logs --env dev --service api-gateway --lines 1000 | grep -i preflight`; (3) interpret: `"Preflight valid"` with status 200 = fully working, close this item; `"Inconclusive preflight"` with status 404/5xx = unexpected, investigate (endpoint may have moved or shapes.inc changed auth); `"shapes.inc rejected submitted cookie"` on a known-good cookie = real auth issue, investigate. The council-recommended swap (2026-04-22) means this is a verification, not a gamble — we expect 200. **Start**: `services/api-gateway/src/services/ShapesPreflight.ts`.
