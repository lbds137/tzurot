...
- 🧹 `[CHORE]` **Audit Claude's auto-memory vs. project rules/docs** — Claude's auto-memory system in `~/.claude/projects/<project>/memory/` accumulates files across sessions. Some memories eventually represent _durable_ project knowledge that belongs in `.claude/rules/` (always-loaded, git-versioned, reviewable by teammates) or `docs/reference/` (persistent reference material), not in volatile per-user memory. Audit all existing memory files in this project, identify which belong where (rule vs. doc vs. skill vs. stays-in-memory), migrate them, and draft a meta-rule for "what belongs in memory vs. rules vs. docs vs. skills." The driving example is the out-of-scope tracking rule that started as a memory and was promoted to `06-backlog.md` during Session 1. **Start**: `ls ~/.claude/projects/*tzurot*/memory/` (the directory path is a hash of the absolute working directory, so the exact name depends on where the repo is checked out — glob is portable) to enumerate, triage each, draft the meta-rule as a new section in `07-documentation.md` or a new rule file.
...
- 🐛 `[FIX]` **beta.93 release prep** — Voice engine retry resilience (PR #759), security dep bumps (PR #760), ConfigStep channelId fix, test improvements. Ready to cut release.
...
- 🏗️ `[LIFT]` **Rename `channel/settings.ts` → `channel/context.ts` (fix 4-layer naming drift)** — The `/channel context` slash command is handled by a file called `channel/settings.ts`, which defines handlers named `handleChannelContext*`, uses `ENTITY_TYPE = 'channel-settings'`, and `channel/index.ts:172` has `componentPrefixes: ['channel-settings']`. Four layers of naming inconsistency for one feature; grep becomes painful. Pick canonical name (`context` is the user-facing command), then: rename `channel/settings.ts` → `channel/context.ts`, rename `channel/settings.test.ts` → `channel/context.test.ts`, update `ENTITY_TYPE` to `'channel-context'`, update `componentPrefixes` in `channel/index.ts:172` to `['channel-context']`, replace ~12 `'channel-settings'` string occurrences in the test file's custom-ID fixtures, fix all import sites. **Why not now**: orthogonal to the CPD refactor in PR #778 that flagged it; small independent cleanup fits Quick Wins perfectly. **Start**: `services/bot-client/src/commands/channel/settings.ts` (source), `services/bot-client/src/commands/channel/settings.test.ts` (test file + string fixtures), `services/bot-client/src/commands/channel/index.ts:172` (componentPrefixes), then update imports across the bot-client package.
- 🏗️ `[LIFT]` **Rename `api-gateway/src/utils/configOverrideHelpers.ts` → `routeHelpers.ts` (or split)** — flagged by PR #779 review. The file originally housed config-cascade override helpers (`mergeAndValidateOverrides`, `tryInvalidateCache` from PR #769), but PR #779 added `resolveUserIdOrSendError` to the same file — which is used by `wallet/setKey.ts`, `user/shapes/auth.ts`, `user/shapes/export.ts`, `user/shapes/import.ts`, and others that have nothing to do with config overrides. The file name is now misleading for most of its callers. Options: (a) rename the whole file to `routeHelpers.ts` (simple but groups unrelated concerns), or (b) split into `configOverrideHelpers.ts` (keeps `mergeAndValidateOverrides` + `tryInvalidateCache`) and `routeUserResolution.ts` or similar (houses `resolveUserIdOrSendError` alone). Option (b) is cleaner long-term but option (a) is a 1-line git mv + import updates. **Not blocking PR #779** — noted by reviewer as a future housekeeping pass. **Start**: `services/api-gateway/src/utils/configOverrideHelpers.ts`; update ~11 import sites across `services/api-gateway/src/routes/`.
- 🏗️ `[LIFT]` **Tighten `configId` handling on `PUT /admin/llm-config/:id` and `PUT /user/llm-config/:id`** — flagged by PR #779 review round 4. Both update-path call sites construct the `validateLlmConfigModelFields` options with `fallback: { service, configId: configId ?? '' }`. Express routes `PUT /:id` guarantee `configId` is defined, so the `?? ''` is dead defensive code — but if a future routing bug makes `configId` undefined, the empty-string fallback silently passes to `service.getById('')`, which returns `null`, which causes `effectiveModel` to be `undefined`, which causes `validateModelAndContextWindow` to skip model-specific checks entirely instead of failing loudly. Pre-existing pattern (predates PR #779; my extraction preserved it to avoid scope creep). **Fix options**: (a) use `getRequiredParam` at the call sites to guarantee `configId` is a non-empty string and throw a 400 otherwise; (b) tighten the helper's `fallback.configId` type from `string` to a branded `NonEmptyString` / validate at entry; (c) add a precondition check inside the helper that returns false + sends error when `fallback.configId === ''`. **Why not now**: not a regression, reviewer explicitly suggested a backlog note over a targeted fix if it would bloat the PR #779 scope. **Start**: `services/api-gateway/src/routes/admin/llm-config.ts` (PUT `/:id` handler); `services/api-gateway/src/routes/user/llm-config.ts` (PUT `/:id` handler); consider whether to tighten the helper signature or the call sites.
- 🧹 `[CHORE]` **Trim ~18 duplicated guard/parse tests in three bot-client settings test files** — After PR #778 ships the `createSettingsCommandHandlers` factory, `character/overrides.test.ts`, `character/settings.test.ts`, and `channel/settings.test.ts` each contain ~6 test blocks that assert behavior now fully covered by the factory's own test suite — the "returns early on non-matching customId" and "returns early on missing entityId" cases. ~18 redundant test blocks total, ~150 test lines. **These are exact behavioral duplicates of the factory's own tests, not independent coverage — removing them reduces noise without changing the actual coverage surface.** Removing tests that duplicate factory coverage is not the same as modifying tests to hide a failure: the failing-behavior these tests prevent is already guarded by `createSettingsCommandHandlers.test.ts`, so the deletion is strictly about cleanup, not about bypassing `00-critical.md`'s "NEVER modify tests to make them pass" rule. **Why not now**: keeping PR #778 focused on the factory itself makes review easier. Defer until factory lands. **Start**: after PR #778 merge, trim redundant describe/it blocks from the three test files, keeping only file-specific behavior (main handler + configuration divergence tests).
...
**Progress**: 175 → 127 (PRs #599, #665–#668); grew to 152 from features; PR #729 → 146; 2026-04-06 architecture day (PRs #766, #768, #769) → 137; PR #776 (browse footer helpers) → 126. **Current (`develop`): 126.** Session 1 (PRs #778, #779) targets → 118 once merged.
