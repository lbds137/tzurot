...
- 🐛 `[FIX]` **ShapesDataFetcher cookie migration — Auth0 → Better Auth (candidate for 🚨 Production Issues)** — shapes.inc has cut over from `@auth0/nextjs-auth0` to Better Auth. The `ShapesDataFetcher` in `services/ai-worker/src/services/shapes/` still reassembles the old 3-part `appSession` / `appSession.0` / `appSession.1` rolling cookie pair, which no longer exists on the shapes.inc side. The single new cookie is `__Secure-better-auth.session_token` (httpOnly, `__Secure-` prefix). **Blast radius**: blocks `/shapes import` flow — Phases 1-4 of the Character Portability shapes.inc import work are shipped on develop (per BACKLOG line 1100) and this fetcher is load-bearing for all of it. Every user attempting a shapes.inc import post-cutover will hit auth failures. **Not a new design question — implementation guide already written**: see [`docs/proposals/backlog/shapes-inc-auth-cookie-migration.md`](docs/proposals/backlog/shapes-inc-auth-cookie-migration.md) for the full migration spec from web Claude (cookie identity/parsing only, no business-logic changes, explicit out-of-scope rails on OAuth/CSRF/bearer-token paths). **Scope**: primary edits to `shapesCookieParser.ts` (single-cookie handling + `SHAPES_SESSION_COOKIE_NAME` constant + `buildSessionCookie` helper), docstring updates in `ShapesDataFetcher.ts`, test-fixture rewrites, developer-doc cookie-harvesting instructions. Estimated 1-2 hours. **Triage recommendation**: promote to 🚨 Production Issues — shapes.inc import is user-facing and currently broken. Surfaced 2026-04-22.
