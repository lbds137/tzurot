...
- 🐛 `[FIX]` **bot-client submit-job timeout on requests with many attachments — synchronous attachment download in api-gateway exceeds 10s client budget** — Observed 2026-04-23T03:38 UTC. User sent a message with 12 extended-context attachments (mix of screenshots up to 827KB each, several MB total). `api-gateway`'s `POST /ai/generate` handler invoked `AttachmentStorageService` to download all 12 attachments in parallel **before returning to bot-client**. Downloads exceeded 10s; bot-client's `GatewayClient.submitJob` 10s timeout fired (`DOMException: TimeoutError — The operation was aborted due to timeout`). api-gateway kept downloading — some finished at 03:38:50, 30+ seconds after abort. Job eventually completed (`Received job result` at 03:41:41, ~3 min later) but user saw an error because the HTTP call aborted. **Frequency**: 1× in 5000-line log window, so rare-but-real — correlates with attachment count, not a systemic issue. **Root cause**: architectural — api-gateway does expensive I/O (N parallel attachment downloads) inside the `/ai/generate` request handler instead of enqueueing first + downloading lazily during job processing. Request-handler time scales with attachment count; bot-client timeout is fixed. **Fix options**: (a) quick mitigation — bump bot-client `GatewayClient.submitJob` timeout (10s → 30s or 60s) to accommodate heavy-attachment cases; (b) structural fix — move attachment download off the critical path: api-gateway enqueues the job with raw attachment URLs, ai-worker downloads lazily at processing time. **Recommendation**: ship (a) as an immediate unblock, file (b) as a follow-up since it's a real architectural concern (CPU/memory in api-gateway scales with attachment payload under the current design). **Start**: `services/bot-client/src/services/GatewayClient.ts` for the timeout bump; `services/api-gateway/src/routes/ai/generate.ts` + `services/api-gateway/src/services/AttachmentStorageService.ts` for the move-to-worker refactor. **Pre-existing**: not introduced by any PR in v3.0.0-beta.103 → beta.104 range; the attachment-download pattern predates today's work. Surfaced by a real user hitting it right before the beta.104 ship.
...
### Post-deploy DM subscription loss fix (next after Phase 6)
...
- 🧹 `[CHORE]` **Add `userId` context to `getOrCreateInternalUser` shadow-mode throw** — In `services/api-gateway/src/routes/user/userHelpers.ts`, the shadow-mode fallback path throws `new Error('User not found after creation')` with no user context if the fallback fires. Without `userId` in the message, a prod incident would be harder to triage. **Caveat — likely obsolete**: Phase 5c final cleanup deletes the shell path entirely. If that cleanup lands before this polish, the throw site disappears naturally. Only pick this up if Phase 5c cleanup stalls > 1-2 weeks. **Fix**: include `userId` in the Error message or convert to structured `logger.error({ userId, ... }, 'msg')` + throw sentinel. Surfaced by claude-bot review on release PR #867 (2026-04-22). Promoted from Inbox.

- 🧹 `[CHORE]` **Test-mock consistency: replace magic `flags: 64` with `MessageFlags.Ephemeral` across mocked interactions** — Surfaced by claude-bot on PR #876 (2026-04-22, non-blocking). `sessionHelpers.test.ts` imports and uses `MessageFlags.Ephemeral` directly; multiple older command-level mocks (including both `getSessionDataOrReply` and `getSessionDataOrFollowUp` mocks in `services/bot-client/src/commands/preset/dashboardButtons.test.ts`) use the bare `64` value with a `// MessageFlags.Ephemeral` comment. Similar drift likely exists across other `*.test.ts` files in bot-client. Concurrent drift: mock content strings like `'Session expired. Please try again.'` don't match the production `DASHBOARD_MESSAGES.SESSION_EXPIRED` constant `'⏰ Session expired. Please run the command again.'`; tests pass because assertions use loose `stringContaining('Session expired')`. **Fix shape**: (a) sweep bot-client `*.test.ts` for literal `flags: 64` + bare session-expired strings; (b) replace with imported constants (`MessageFlags.Ephemeral`, `DASHBOARD_MESSAGES.SESSION_EXPIRED`); (c) consider whether strict assertions (toHaveBeenCalledWith the real constant) should replace the loose `stringContaining` matchers for better regression coverage. **Why deferred from PR #876**: the magic-number pattern pre-existed in the mock my new tests sat next to, so matching it kept scope tight; fixing it in a refactor PR would've dragged pre-existing test-mock touchup into the defer-first scope. Better as a dedicated test-consistency sweep. **Start**: `grep -rn 'flags: 64' services/bot-client/src/` for the sweep surface. Surfaced 2026-04-22.

- 🧹 `[CHORE]` **Apply `ApiCheck<T>` pattern to the 3 autocomplete caches** — Follow-up to the class-of-bug fix that introduced `services/bot-client/src/utils/apiCheck.ts`. Three medium-severity instances remain in `services/bot-client/src/utils/autocomplete/autocompleteCache.ts`: `getCachedPersonalities` (~line 88), `getCachedPersonas` (~line 141), `getCachedShapes` (~line 192). All three return `[]` on API error — transient failures make users see empty autocompletes and think they have no personalities/personas/shapes. **Fix shape**: widen each cache's return to `ApiCheck<T[]>`; autocomplete handlers render a single non-selectable `[Unable to load — try again]` choice on `{ kind: 'error' }`. **Open product question**: do we want stale-cache-past-TTL as a third fallback, or is the error placeholder enough? **Start**: `services/bot-client/src/utils/autocomplete/autocompleteCache.ts` + the three autocomplete handlers. Surfaced 2026-04-21. Promoted from Inbox 2026-04-22.
