...
- 🏗️ `[LIFT]` **Rename `channel/settings.ts` → `channel/context.ts` (fix 4-layer naming drift)** — The `/channel context` slash command is handled by a file called `channel/settings.ts`, which defines handlers named `handleChannelContext*`, uses `ENTITY_TYPE = 'channel-settings'`, and `character/index.ts` aliases `'channel-settings'` back to `'channel'`. Four layers of naming inconsistency for one feature; grep becomes painful. Pick canonical name (`context` is the user-facing command), rename the file, update `ENTITY_TYPE` to `'channel-context'`, update the alias map in `character/index.ts`, update `componentPrefixes`, fix imports. **Why not now**: orthogonal to the CPD refactor in PR #778 that flagged it; small independent cleanup fits Quick Wins perfectly. **Start**: `services/bot-client/src/commands/channel/settings.ts`, `services/bot-client/src/commands/character/index.ts` for the alias map.
