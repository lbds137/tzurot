...
- 🧹 `[CHORE]` **Refine `PINO_LOGGER_RULES` selector to distinguish single-arg from two-arg first-child Identifier** — PR #866 extended `PINO_LOGGER_RULES` to catch `CallExpression` first args (e.g. `logger.warn(buildError(), 'msg')`) by adding `CallExpression` to the `:matches()` list. Side effect: the rule now also fires on the legitimate `logger.info(prebuiltOpts, 'msg')` pattern where `prebuiltOpts` is an object Identifier holding fields. Worked around at 5 sites in `LLMInvoker.ts`, `ContextWindowManager.ts`, `RetryDecisionHelper.ts`, `PromptBuilder.ts` with `{ ...opts }` inline spread. Spread is semantically a no-op but still allocates a shallow copy on every call — in hot-path loggers (model-completion, context-window-check) the GC pressure isn't zero, especially when the Pino level would otherwise make the call a cheap no-op. **Fix shape**: change the selector to only fire on Identifier/CallExpression when the call is _single-argument_ (where the Identifier genuinely represents a raw error that needs `{ err: ... }` wrapping). Two-arg calls with Identifier/CallExpression first-arg are almost always a prebuilt fields object + msg and should be allowed. Selector sketch: `CallExpression[arguments.length=1] > *.arguments:first-child:matches(Identifier, MemberExpression, CallExpression)` for the error-shape rule, keep the TemplateLiteral rule unchanged (it should fire on any arity). Then revert the 5 `{ ...opts }` spreads back to the original Identifier form. **Start**: `eslint.config.js` `PINO_LOGGER_RULES` constant. Surfaced by claude[bot] review on PR #866 (2026-04-21).
