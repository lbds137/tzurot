...
- 🧹 `[CHORE]` **Post-merge polish for preset-clone fix (PR #834 r3 nits)** — Three non-blocking observations from the r3 review that are cheap to address in a follow-up rather than inflating PR #834 any further: (a) `CloneNameExhaustedError`'s message shows the raw `baseName` (e.g., `"Preset (Copy 5)"`) — using `stripCopySuffix(baseName)` in the constructor would read more naturally as _"Too many copies of 'Preset'..."_. (b) `resolveNonCollidingName`'s filter `startsWith: "${stripped} (Copy"` matches literals like `"Preset (Copycat Theme)"` that can never match a generated candidate but still consume the `take: 21` budget — a tighter `OR: [{ startsWith: "${stripped} (Copy)" }, { startsWith: "${stripped} (Copy " }]` would be precise. (c) The P2002 wrap in `LlmConfigService.create` auto-suffix path catches any P2002, not specifically the `(owner_id, name)` target — UUIDv7 PK collision is astronomical so this is defense-in-depth, but `err.meta?.target?.includes('name')` would scope it airtight. **Start**: `services/api-gateway/src/services/LlmConfigService.ts` — constructor call site of `CloneNameExhaustedError`, the `OR` filter in `resolveNonCollidingName`, and the catch in `create()`. Surfaced 2026-04-19 on PR #834 r3 review.
