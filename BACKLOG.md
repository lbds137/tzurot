...
- 🐛 `[FIX]` **LangChain reasoning extraction silently drops content (cross-model, all reasoning-enabled requests)** — **See [`docs/research/langchain-reasoning-extraction-bug.md`](docs/research/langchain-reasoning-extraction-bug.md) for the full investigation playbook.** TL;DR: our `OpenRouterFetch` interceptor extracts `message.reasoning` and injects `<reasoning>` tags into `message.content` correctly (`reasoningInjected=true` in prod logs for the user's leaked request, 1994 chars extracted). But by the time `model.invoke()` returns, the tags are GONE from `response.content` — the diagnostic captures `hasReasoningTagsInContent: false`. Something between our interceptor's `new Response(JSON.stringify(body), ...)` and LangChain's eventual `BaseMessage.content` is stripping or bypassing our injected tags. Net effect: **reasoning content is silently lost on every reasoning-enabled request**, not just the user-visible "leak" cases. The visible leaks (~11% on GLM-4.7) are the subset where the model ALSO embedded planning prose in `content` directly; the silent loss happens 100% of the time the model returns reasoning. The user's perception that "leaks are happening more often" was a visible symptom of an underlying bug that's been silently degrading reasoning capture for an unknown duration. **Investigation doc has**: smoking-gun evidence (Railway logs from prior deployment), four ranked hypotheses, ordered test plan starting with `scripts/src/test-glm-reasoning-shape.ts`, all diagnostic scripts in `scripts/src/`, links to relevant code paths. Surfaced 2026-04-25 by user inspect-log review post beta.106 release.
