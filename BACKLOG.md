...
- 🐛 `[FIX]` **Voice generation intermittently fails — ROOT CAUSE FOUND 2026-04-19, resolved by PR #TBD** — Fresh reproducer at 13:22 UTC 2026-04-19 (personality lilith-tzel-shani, job `llm-eba5a947-822b-41e8-a8fa-51a05ad3ce33`). Trace: ElevenLabs 60s timeout → voice-engine cold-start fallback (+46s warmup) → voice-engine synth produced **10.35 MB of raw WAV** (2 min of speech @ 5 MB/min) → `DiscordResponseSender.fetchTTSFiles` silently returned `undefined` because 10.35 MB > Discord's 8 MiB attachment limit. User saw text-only response after 4m 19s wait and interpreted it as failure.

  **Fix shipped in PR #TBD (2026-04-19)**:
  - voice-engine transcodes TTS output to Opus-in-Ogg at 64 kbps via ffmpeg (already in Docker image). Opus at 64 kbps ≈ 10× smaller than WAV; ~17 minutes of speech fit under the 8 MiB limit.
  - `?format=wav` query param preserves the legacy WAV path for `ttsSynthesizer.ts` multi-chunk concatenation (extracts PCM, can't concat Opus-in-Ogg losslessly).
  - `DiscordResponseSender.fetchTTSFiles` now returns a tiny `voice_omitted_too_long.txt` attachment on over-size instead of silently returning `undefined` — user gets a visible signal.

  **Remove on merge**: this entry closes when PR ships to prod.
...
- 🐛 `[FIX]` **Near-duplicate consecutive replies slipped past CrossTurnDetection (observed 2026-04-19 with glm-4.5-air:free)** — Two screenshots show Lila Sitnat Bambi emitting two consecutive near-identical responses in the same channel. Bodies are essentially byte-identical (multi-paragraph response about "Lack of skills? That's the voice of terror speaking..."); the only differences: opener adjective ("thoughtful shadows" vs "faint shadows") and one has a voice.wav attachment while the other doesn't. Both use model `z-ai/glm-4.5-air:free`. User's diagnostic (correct): `CrossTurnDetection` should have caught this — it's designed precisely for free-tier API-level caching (see `services/ai-worker/src/utils/crossTurnDetection.ts:4-12`, "typically caused by API-level caching on free-tier models"). Similarity between the two bodies is visibly >95%; the block threshold is `DEFAULT_SIMILARITY_THRESHOLD` = 0.85 (per `duplicateDetection.ts`). The block should have fired, and didn't.

  **Hypotheses for why detection missed** (ranked by likelihood):
  1. **Cross-job vs cross-retry scope**: `CrossTurnDetection` may only run INSIDE a single generation job's retry loop (duplicate-in-same-job → trigger retry), not across separate jobs to the same channel. If the user sent two messages in quick succession and each generated a separate job, the second job would need to pull prior AI responses from the conversation history to compare against — and the history fetch may happen BEFORE the dedup check point, OR the dedup check may only look at the retries-within-this-job buffer. **Start investigation here.** File: `services/ai-worker/src/utils/crossTurnDetection.ts` — trace the callers, see whether the "recent messages" argument is retrieved from persistent history or from in-process state. If in-process only, cross-job dup misses are a structural gap.
  2. **History load timing**: cross-turn dedup works against recent conversation history. If the second job loaded history BEFORE the first job's response was persisted (race window), the second job wouldn't have the first response in its comparison set. Free-tier API caching would then deterministically produce the same output, and dedup would have nothing to compare against. Race window = time between first-job-completes and first-response-persists-to-DB. Could be tens to hundreds of ms.
  3. **Stripped content differing enough to drop below threshold**: the two responses differ in voice attachment and opener adjective. `stripBotFooters` is applied before similarity comparison (per `crossTurnDetection.ts:14` import). But voice-attachment presence wouldn't show in the text itself — only the opener adjective differs in the body. Similarity should still be 95%+. Unlikely, but worth measuring.
  4. **Near-miss log fired but was ignored**: possible the dedup ran, logged NEAR_MISS, but similarity came in below 85% (despite eyeballing suggesting >95%) and so no action was taken. Check prod logs around the incident window for `[CrossTurnDetection] NEAR-MISS` entries.

  **Investigation steps**:
  1. Get the Discord channel ID + approximate time window for the two duplicate replies. Query Railway logs: `railway logs --service ai-worker --json | jq -c 'select(.name == "CrossTurnDetection")'` for that window. If NEAR_MISS entries exist → threshold issue. If no CrossTurnDetection entries at all → scope issue (dedup didn't run on the second job).
  2. Read `CrossTurnDetection` call sites (grep `services/ai-worker/src` for the `CrossTurnDetection` constructor or its check function name) to see where it's wired in the pipeline. Confirm whether it compares against persisted history or in-process retry state.
  3. If scope issue: extend dedup to cover cross-job dedup via persisted history. Consider the race window on history writes.
  4. If threshold issue: add a lower-threshold warn-only log (trigger at 70%) to get visibility into how often high-similarity responses slip the 85% gate.
  5. If caching issue: separately consider a cache-busting signal in the request to free-tier models (e.g., temperature jitter, nonce in system prompt).

  **Why this matters**: user sees two identical responses → feels like the bot is broken. Also wastes a generation call + voice synthesis cost on the duplicate. Free-tier models (glm-4.5-air:free) are the most likely culprits — API-level caching is a known pattern on free tiers.

  **Start**: `services/ai-worker/src/utils/crossTurnDetection.ts` (detection logic), `services/ai-worker/src/utils/duplicateDetection.ts` (thresholds, similarity primitives), + grep for callers of `CrossTurnDetection` to find where it hooks into the generation pipeline. Surfaced 2026-04-19.
...
- 🐛 `[FIX]` **AI parrots user input at the start of its message (observed 2026-04-19 with Baphomet)** — Intermittently, the bot's response literally quotes the user's message verbatim as the first lines of its reply, then starts the actual response below with a blank line separator. Screenshot captured 2026-04-19 shows Baphomet echoing `@Baphomet\nI think now I have to look for my Patron...` — including the `@` mention prefix AND the full message body — before the real response begins with `*hoof taps once...*`. User experience: it looks like the bot is manually quoting, but there's no quote formatting — it's just the message text pasted twice (once as user input, once as the leading block of the bot's output).

  **Existing infrastructure that almost catches this but doesn't**: `services/ai-worker/src/utils/responseArtifacts.ts` already strips several echo patterns — `<from id="...">Name</from>`, `<last_message>...</last_message>`, `<received message>...</received>` (GLM 4.5 Air specifically), and `"Name:"` prefixes. None of these match a plain-text echo of the user's message without XML wrapping. The `@Baphomet` mention echo also isn't caught — `buildArtifactPatterns` has `<message speaker="Name">` but not `^@<PersonalityName>\n`.

  **Fix-shape options** (ranked by effort):
  - **(a) Add a mention-echo pattern** to `buildArtifactPatterns`: `^@<escapedPersonalityName>\s*\n` — catches the leading `@Baphomet\n` prefix specifically. Doesn't fix the body echo but addresses the `@mention` half. Cheapest; ~5 lines.
  - **(b) Prefix-match user input** at post-process time: thread the user's last message text to `stripResponseArtifacts` and strip it if the response begins with it verbatim (or with minor whitespace drift). Requires passing the incoming message text through to post-processing, which is a plumbing change but not architectural. Catches the whole echo.
  - **(c) Prompt-level fix**: add an instruction to the system prompt ("Do not repeat or quote the user's message at the start of your response"). Cheap, unreliable — some models ignore negative instructions or regress after context fills.
  - **(d) Model-specific regex**: if the echo is correlated with a specific model (e.g., always happens with GLM-5 or always with a particular personality config), scope the strip to that model. Only worth doing after observing which models exhibit the behavior.

  **Investigation steps (in order)**:
  1. Reproduce: send the same personality (Baphomet) several messages of varying length. Note which model the personality is configured to use. Check if the echo fires every time or sporadically.
  2. Audit ai-worker logs for the reproducer window: grep for the affected jobId, capture the raw LLM response before `stripResponseArtifacts` runs, confirm the echo is in the raw output (vs. introduced post-processing).
  3. Broaden: does this happen only with Baphomet, only with one model, or across personalities? Screenshot is one data point — need a handful more to scope.
  4. Pick fix shape based on the empirical scope.

  **Why the existing `stripResponseArtifacts` infrastructure matters**: this is the same pattern as the GLM 4.5 Air `<received message>` bug — a model-specific quirk where training data taught the model to format its output with an incoming-message echo. Extending the artifact-strip pattern set is the established path. Surfaced 2026-04-19.

  **Start**: `services/ai-worker/src/utils/responseArtifacts.ts` (existing strip patterns), `services/ai-worker/src/services/ConversationalRAGService.ts` (where `stripResponseArtifacts` is likely called), + a repro run with Baphomet to narrow scope before picking fix-shape.
