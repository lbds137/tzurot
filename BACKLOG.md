...
- 🧹 `[CHORE]` **Remove unused `_config: EnvConfig` parameter from `createPreset`** — `api.ts:158` has an unused `_config` parameter with the `_` prefix escape hatch, but `createPreset` is a standalone exported function, not a callback or interface implementation. Per `02-code-standards.md`, the parameter should be removed and callers updated. Pre-existing, flagged in PR #801 review. **Start**: `services/bot-client/src/commands/preset/api.ts` (createPreset signature), grep for call sites.
