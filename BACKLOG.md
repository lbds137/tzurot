...
- 🏗️ `[LIFT]` **Singleton-hazard guard for `UserService` cache** — `UserService.getOrCreateUserShell` intentionally does NOT write to `this.userCache` to prevent a subtle bug: if `UserService` is ever refactored to a singleton (a reasonable perf improvement), a shell call would cache a `discordId → userId` mapping, causing subsequent `getOrCreateUser` calls to short-circuit out of the cache and skip `runMaintenanceTasks` (username upgrade + persona backfill). Today's per-request instantiation keeps the cache cold, so this is a latent footgun. The hazard is documented in the cache field's JSDoc (`packages/common-types/src/services/UserService.ts`), but filing a dedicated entry so it's not forgotten. Options if UserService is made a singleton: (a) split the caches so shell and full have separate tracking; (b) move username upgrade/persona backfill out of the hot-path and into an explicit "ensure provisioned" method callers invoke. Either works; pick based on usage patterns at promotion time. Flagged by PR #805 review.
...
- 🐛 `[FIX]` **AbortError in image processing causes long AI-generation delays** — Images getting stuck somewhere in the attachment/URL/vision pipeline blocks the full reply path, occasionally making a single message take a very long time end-to-end. Observed in production across 2026-04-12 and 2026-04-13. May share root cause with Pocket TTS intermittent failures (both hint at resource contention or an unbounded concurrent fetch). Related: "Improve Railway log search DX" below — digging into this one is currently painful and is part of why the observation keeps surfacing without a fix.

  **Investigation (2026-04-13)**: five distinct image-handling paths identified, but only one is on the critical blocking path for an AI reply:
  - **Path A (critical)**: vision description at `services/ai-worker/src/services/multimodal/VisionProcessor.ts:127-213`. Synchronous during prompt assembly (via `DependencyStep.ts:201-251` → `MultimodalProcessor.processAttachments`), so any hang here blocks the entire reply. Timeout: `TIMEOUTS.VISION_MODEL = 90s` per attempt × 3 retries via `withRetry()` → **worst-case 270s reply latency**.
  - **Path B (covered)**: audio fetch at `AudioProcessor.ts:51-72`, 30s `AbortController`.
  - **Path C (uncovered, now its own Inbox item below)**: avatar upload at `bot-client/utils/avatarProcessor.ts:56` and `commands/character/avatar.ts:70` has no timeout. Not on reply critical path, but can silently hang command interactions.
  - **Path D (covered)**: voice reference upload at `commands/character/voice.ts:110-117`, 30s.
  - **Path E**: extended-context images — inherits Path A.

  **No shared fetch wrapper**: each path implements its own timeout (or none). `ai-worker/src/utils/retry.ts:367-386` provides `withTimeout()`, but only voice/audio paths use it consistently.

  **Secondary concern — negative cache may mask retries**: when vision times out, `visionDescriptionCache.storeFailure()` stores a "failed" entry with a ~10-minute cooldown. For transient CDN stalls this means the next message with the same image URL returns `[Image temporarily unavailable]` from cache instead of re-attempting. Timeout/abort errors may deserve a shorter cooldown than permanent failures.

  **Root cause (verified against production logs, 2026-04-14)**: cross-site image URLs in channel history that 404 when the vision provider tries to fetch them. NOT specific to Discord CDN — observed URL hosts include `i.redd.it` (Reddit), user-linked images, and arbitrary referenced URLs. The vision provider (Google AI Studio via OpenRouter, model `google/gemma-3-27b-it:free`) surfaces the fetch 404 to us as `BadRequestError: "400 Received 404 status code when fetching image from URL: ..."`.

  **Retry amplification CONFIRMED**. Our classifier at `services/ai-worker/src/utils/retry.ts` marks this error as `errorCategory="bad_request"`, `errorType="transient"`, `shouldRetry=true`. Every bad URL triggers a full retry cycle even though 404s are never going to resolve on retry. Log sample from prod 2026-04-14 02:04:08:

  ```
  statusCode=400 shouldRetry=true
  technicalMessage="400 Received 404 status code when fetching image from URL:
  https://i.redd.it/ks0yn4k5ed6f1.jpeg"
  ```

  **Additional failure modes in the same log window**:
  - **Free-tier vision rate-limits**: `429 google/gemma-3-27b-it:free is temporarily rate-limited upstream`. Classified `rate_limit`/`transient`/`shouldRetry=true` — retry is correct here, but compounds with 404 retries into sustained latency when the free pool is exhausted.
  - **AbortError — the original symptom**: `err={"name":"AbortError","message":"Request was aborted"}`, classified `errorCategory="unknown"`, `errorType="unknown"`, `shouldRetry=true`. So even errors we don't recognize get retried by default, adding to the pileup.

  **Negative cache is working but misconfigured for 404s**: `[VisionDescriptionCache] Stored failure in negative cache ... category="bad_request" permanent=false`. The `permanent=false` flag applies a short TTL (~10 min per earlier investigation), so the same 404'd URL gets re-tried on the next message in the channel. For URLs that will never work, `permanent=true` would be correct.

  The original "270s worst-case vision timeout" framing was directionally right (vision IS the bottleneck on the critical path) but misdiagnosed the mechanism — this is a 404-then-retry cascade, not a single stalled fetch.

  **UPDATE (2026-04-14 — log data shifted priorities)**: pulled 3000 lines of prod vision logs. 99 failures split as **63% AbortError** (the one previously classified as ambiguous), **30% rate_limit** (free-tier `gemma-3-27b-it:free` hitting its quota), **7% 400-on-dead-URL** (what the Root cause section above described). Also: 112 invocations → 13 successes = **~12% success rate when vision is actually invoked**; 128 cache hits in the same window means most vision traffic is cache-served and the problem is concentrated in the miss path. The 404-on-URL fix addresses 7% of failures. **AbortError reclassification is dominant by a wide margin** — reprioritized below.

  **AbortError IS our own 90s timeout, not a third-party abort**. `VisionProcessor.ts:165` does `model.invoke(messages, { timeout: TIMEOUTS.VISION_MODEL })`, passing 90s directly to LangChain's internal mechanism. When that fires, the OpenAI SDK throws `AbortError: "Request was aborted"` — bypassing our `withTimeout` wrapper convention (which would rename it to `TimeoutError`). The classifier at `apiErrorParser.ts` doesn't pattern-match this exact shape, so it falls through to UNKNOWN → `shouldRetry=true`. Retries with another 90s timeout usually fail the same way.

  **Fix space (reordered 2026-04-14 by failure-rate data)**:

  🎯 **PRIMARY (addresses 63%)**: pattern-detect `name === "AbortError" && message === "Request was aborted"` in the classifier at `services/ai-worker/src/utils/apiErrorParser.ts`, reclassify as `TIMEOUT` category (not `UNKNOWN`). Then cap the TIMEOUT retry budget at 1 retry with short backoff, not 3 retries of 90s each. Rationale: if 90s wasn't enough, another 90s usually isn't either; 1 retry covers the transient-provider-hiccup window. Single-file change.

  🎯 **SECONDARY (addresses 7%)**: pattern-detect `"400 Received 404 status code when fetching image from URL"` → reclassify as permanent non-retryable, pass `permanent=true` to `storeFailure()`. Same file as primary, same PR.

  **Prerequisite (ship alongside primary)**: add per-attempt success telemetry to `withRetry` — `logger.info({ attempt, totalTimeMs }, 'retry succeeded')` when a retry recovers. Without this we can't MEASURE whether the primary fix worked; we only see failures today. See separate Inbox item "Add per-attempt success telemetry to `withRetry`" below.

  **Later / not urgent**:
  - HEAD-validate cross-site URLs before vision call — probably unnecessary after primary+secondary; keep as plan B.
  - Default max-age cutoff on history images — policy decision, not a technical fix. See separate Inbox item "Tighten `HARDCODED_CONFIG_DEFAULTS.maxAge`" below.
  - Expand negative-cache coverage — the secondary fix naturally resolves this (permanent=true for 404s).

  **Out of scope for this entry**: the 88% vision invocation failure rate is a signal that the free-tier-only vision strategy is fragile. Paid-tier fallback work belongs to the Model Configuration Overhaul theme, not here. Revisit if post-fix failure rate stays high.

  **Start (when a session picks this up)**:
  1. `services/ai-worker/src/utils/apiErrorParser.ts` `detectCategoryFromMessage()` — add pattern for AbortError + "Request was aborted" → `TIMEOUT`.
  2. Same file — add pattern for `"400 Received 404 status code when fetching image from URL"` → permanent + no-retry.
  3. `services/ai-worker/src/utils/retry.ts` — cap per-category retry budgets (TIMEOUT = 1 retry).
  4. `services/ai-worker/src/utils/retry.ts` `withRetry` — add `logger.info({ attempt, totalTimeMs }, 'retry succeeded')` on recovery.
  5. Validate: re-run `pnpm ops logs --service ai-worker --filter "vision"` after deploy, count AbortError frequency vs. pre-fix baseline. Pairs with the Railway logs `--filter` passthrough entry below — that's what enables the validation query.
...
- 🧹 `[CHORE]` **Improve Railway log search DX for incident digs** — When investigating specific production issues (see the AbortError inbox item above for a live example), the current Railway log surface is painful to search — no easy way to filter by request ID across services, correlate a user-visible symptom with a specific worker job, or scope to a tight time window around a known bad event. Most digs end with "I scrolled through the log stream hoping I'd spot the right line."
  ...
  **Recommended sequence**: 0. **Pass `--filter` through to Railway CLI in the ops wrapper** (~30 min). In `packages/tooling/src/deployment/logs.ts:227`, add `--filter <value>` to the Railway args array when provided (removing or demoting the JS-side substring grep at lines 44-68). Surface query-syntax examples in the tips footer. Unambiguous first move — small, self-contained, no dependencies, biggest delta for immediate usefulness. Once shipped, `pnpm ops logs --filter "vision AND (404 OR 400)"` becomes a powerful ad-hoc diagnostic.
  ...
  **Start**: step 0 is the unambiguous first move. Do that, then reassess whether the other steps are still urgent — most ad-hoc diagnosis will work with `railway logs --filter` directly once step 0 lands.
...
- 🐛 `[FIX]` **Avatar upload fetch has no timeout — silent hang risk** — `services/bot-client/src/utils/avatarProcessor.ts:56` and `services/bot-client/src/commands/character/avatar.ts:70` both do a naked `await fetch(attachment.url)` with no `AbortSignal`. If Discord CDN stalls during an avatar upload, the command interaction silently hangs until Discord's interaction timeout (~15 min). Not on the reply critical path (it's user-initiated, not message processing), but visible to users as "nothing is happening after I uploaded the avatar." Fix: wrap with a 30s `AbortSignal.timeout()` matching the established pattern in `commands/character/voice.ts:110-117`. Surfaced incidentally during the 2026-04-13 AbortError investigation (see AbortError Inbox item above).
- 🐛 `[FIX]` **Cross-channel reference expansion may leak messages across permission boundaries** — `services/bot-client/src/handlers/references/LinkExtractor.ts` `fetchMessageFromLink()` (lines 124-251) uses `sourceMessage.client` (the bot's own Discord credentials) to fetch referenced messages. It verifies the bot has access but does NOT verify the _invoking user_ has access to the source channel. Exploit scenario: the bot is in a private channel (e.g., `#staff` in guild A) that an attacker also has access to. Attacker copies a message link from the private channel and pastes it in a public channel where a victim user is present (or DMs it to the bot). The bot expands the message via its own credentials; its content enters the AI conversation context, potentially surfacing in the AI reply the victim then reads. Applies to any cross-guild / cross-permission link the bot can see but the invoking user cannot. Exact severity depends on how much of the expanded content surfaces in the AI reply, which varies per personality prompt — but the architectural flaw exists regardless.

  **Consider promoting to 🚨 Production Issues** pending user's severity judgment. The flaw is in production today (not theoretical) and independent of whether the range-import feature ships. It also blocks the range-import feature from shipping safely (a range import is `N`-linked-messages-at-once, so the blast radius scales).

  **Fix**: before `channel.messages.fetch(messageId)` in `fetchMessageFromLink()`, check `channel.permissionsFor(invokingMember).has(PermissionsBitField.Flags.ViewChannel)` (and `ReadMessageHistory` for history). If the check fails, silently skip the reference (log at debug) rather than expanding it. This requires threading the invoking member through to `LinkExtractor` — it already receives `sourceMessage` which exposes `.member`, so the plumbing is cheap. Surfaced during the 2026-04-13 cross-channel-context investigation.

  **Start**: `services/bot-client/src/handlers/references/LinkExtractor.ts:124`, plus caller sites that currently pass `sourceMessage` without a user-identity channel check.

- 🧹 `[CHORE]` **Add per-attempt success telemetry to `withRetry`** — `services/ai-worker/src/utils/retry.ts` logs each failed attempt and the final-failure state, but doesn't log when a retry actually recovers. Without this we can't measure retry productivity — we count failures but can't tell which `describeImage` / LLM calls succeeded on attempt 2 or 3 vs. succeeded on first try. Small addition: `logger.info({ attempt, totalTimeMs, operationName }, 'retry succeeded')` when the retry loop returns successfully after at least one prior failure. Prerequisite for validating whether the AbortError classifier fix (see AbortError Inbox entry above) actually reduces retry latency — and valuable generally because any retry-driven operation (voice engine, API gateway fetches, LLM invocations) benefits from success-path observability. Surfaced during 2026-04-14 prod log investigation when I realized I could count failures but not retry-recoveries. **Start**: `services/ai-worker/src/utils/retry.ts` around the successful-return path in `withRetry` (near the end of the retry loop, just before returning the `RetryResult`).
...
- 🐛 `[FIX]` **"Taking longer" notification fires too late and never cleans up** — `services/bot-client/src/services/JobTracker.ts:17` defines `MAX_JOB_AGE_MS = 10 * 60 * 1000` (10 minutes). When a job exceeds that window, the typing indicator stops and line 85-88 sends `"⏱️ This is taking longer than expected. I'm still working on it — you'll get a response when it's ready!"` to the channel. Two issues: (a) 10 minutes is too late — by then users have often given up waiting or stopped paying attention. Notification should fire at ~5 minutes when the wait is still actionable. (b) When the job finally completes, the "taking longer" message stays in the channel forever, cluttering the conversation — should be deleted on completion. **Fix**: (1) either lower `MAX_JOB_AGE_MS` to `5 * 60 * 1000`, OR decouple the notification timer from the typing-indicator cutoff by introducing `TAKING_LONGER_NOTIFY_MS = 5 * 60 * 1000` so typing can stop at the original 10-min bound but user notification fires earlier. Decoupling is cleaner if the two cutoffs have different rationales. (2) Capture the returned `Message` object from the `channel.send(...)` call at JobTracker.ts line 85-88 and store it on the tracked job record. (3) In the job-completion path (same file), call `.delete()` on that stored message before clearing the job. Handle the delete failing gracefully (user may have dismissed the message, bot may lack perms in the channel, etc.) — log-and-continue, don't throw. **Start**: `services/bot-client/src/services/JobTracker.ts:17` (constant), `:83-88` (send site, needs Message capture and storage on the job record), find the completion/cleanup path in the same file to add the delete call.
