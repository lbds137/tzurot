...
const API_GATEWAY_URL =
  process.env.API_GATEWAY_URL || 'https://api-gateway-development-83e8.up.railway.app';
...
async function main() {
  ...
  const lilithCustomFields = {
    ...
    wackMessage:
      '*dissolves into a swirl of darkness and owl feathers* Even chaos bends to my will...',
    errorMessage:
      '*laughs darkly* The mysteries of existence sometimes exceed even my grasp... ||*(an error has occurred)*||',
    sleepMessage:
      '*draws a sigil in the air with a knowing smirk* Until our paths cross again in the shadows...',
    favoriteReacts: ['🌙', '🦉', '🩸', '🌹', '🔥', '👁️', '💫', '🖤'],
    searchDescription:
