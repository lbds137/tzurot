...
async function fixMemories() {
  ...
  console.log(`🔧 Replacing "User (${USER_ID}):" with "User (${username}):" in ${memoryIds.length} memories\n`);
  ...
  for (const memoryId of memoryIds) {
    ...
    try {
      ...
      const newContent = content.replace(new RegExp(searchPattern.replace(/[()]/g, '\\$&'), 'g'), replacePattern);
      ...
      const occurrences = (content.match(new RegExp(searchPattern.replace(/[()]/g, '\\$&'), 'g')) || []).length;
