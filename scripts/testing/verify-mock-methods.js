...
function extractMockedMethods(testFile) {
  ...
  const patterns = [/mock([A-Z]\w+)\.(\w+)\.(mock\w+)/g, /(\w+)\.(\w+) = jest\.fn\(/g];
...
function verifyMocks(testFiles) {
  ...
  testFiles.forEach(testFile => {
    ...
    mockedMethods.forEach(({ mockName, methodName, line }) => {
      ...
      if (!sourceFile) {
        console.log(
          `  ${colors.yellow}⚠ Cannot determine source for mock '${mockName}'${colors.reset}`
        );
        unverifiedMocks++;
        return;
      }
      ...
      if (verification.exists) {
        ...
        console.log(
          `  ${colors.red}✗ ${mockName}.${methodName} - ${verification.reason}${colors.reset}`
        );
