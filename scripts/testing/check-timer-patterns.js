...
function isInsideDefaultParam(content, matchIndex) {
  ...
  return /(?:=|\|\|)\s*\(?\([^)]*\)\s*=>\s*(?:new\s+)?Promise\s*\(\s*(?:resolve|res)\s*=>\s*$/.test(
    before
  );
}
...
function isInjectableDefinition(content, matchIndex) {
  ...
  return (
    ...
    /(?:scheduler|delay|timer|interval)(?:Fn)?\s*(?:=|\|\|)\s*(?:\([^)]*\)\s*=>)?\s*$/.test(
      before
    ) ||
...
const TIMER_PATTERNS = [
  ...
  },
  {
    ...
    filter: (content, match, index) => {
      ...
      if (
        /(?:this\.|this\._|options\.|context\.|timerFunctions\.|global\.|timers\.|timer\.)$/.test(
          before
        )
      )
        return false;
  ...
  },
  {
    ...
    pattern:
      /^(?!.*(?:function|const|let|var|class|return|timerFunctions\.)).*(?:setTimeout|setInterval)\s*\(/gm,
...
// Files to exclude
const EXCLUDE_PATTERNS = ['**/node_modules/**', '**/*.test.js', '**/*.spec.js', '**/tests/**'];
