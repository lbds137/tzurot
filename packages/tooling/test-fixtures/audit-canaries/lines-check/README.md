...
its own. `CLAUDE.md` is a small placeholder, kept comfortably under the
hard instructions ceiling — it exists so that ceiling measures without a
hollow-measurement failure, without contributing any findings of its own.
The canary test in
...
reports `status: 'fail'` with EXACTLY four findings (the two over-budget
