...
const CASES: GoldenCase[] = [
  { site: 'common-types/attachmentCacheKey.ts', expected: FULL },
  { site: 'common-types/characterCardChecksum.ts', expected: FULL },
  { site: 'common-types/feedbackNormalization.ts', expected: FULL },
  { site: 'tooling/db/check-migration-drift.ts', expected: FULL },
  { site: 'tooling/db/fix-migration-drift.ts', expected: FULL },
  { site: 'ai-worker/archiveSummaryFeedback.ts', expected: FULL },
  { site: 'bot-client/ErrorChannelReporter.ts', expected: FULL },
  { site: 'bot-client/commandRegistrationGate.ts', expected: FULL },
  { site: 'tooling/db/create-safe-migration.ts', opts: { encoding: 'utf-8' }, expected: FULL },
  { site: 'common-types/constants/memory.ts', opts: { length: 32 }, expected: LEN_32 },
  {
    site: 'common-types/deterministicUuid.ts generateMemoryChunkGroupUuid',
    opts: { length: 32 },
    expected: LEN_32,
  },
  {
    site: 'common-types/deterministicUuid.ts generatePendingMemoryUuid',
    opts: { length: 32 },
    expected: LEN_32,
  },
  {
    site: 'common-types/deterministicUuid.ts generateMemoryFactUuid',
    opts: { length: 32 },
    expected: LEN_32,
  },
  { site: 'embeddings/LocalEmbeddingService.ts', opts: { length: 16 }, expected: LEN_16 },
  { site: 'ai-worker/duplicateDetection.ts', opts: { length: 16 }, expected: LEN_16 },
  { site: 'api-gateway/RedisDeduplicationCache.ts', opts: { length: 16 }, expected: LEN_16 },
  { site: 'tooling/audits/baseline-meta.ts', opts: { length: 12 }, expected: LEN_12 },
  { site: 'common-types/logContentPreview.ts', opts: { length: 12 }, expected: LEN_12 },
  { site: 'common-types/slugUtils.ts', opts: { length: 6 }, expected: LEN_6 },
  {
    site: 'ai-worker/cacheObservability.ts',
