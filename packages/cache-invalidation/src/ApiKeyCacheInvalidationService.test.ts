...
describe('ApiKeyCacheInvalidationService', () => {
  ...
  describe('subscribe dispatch (wire-contract validation)', () => {
    async function makeSubscribed(): Promise<{
      ...
      const dispatchService = new ApiKeyCacheInvalidationService(redisForDispatch);
      const callback = vi.fn();
      await dispatchService.subscribe(callback);
      return {
        deliver: (
          raw: string,
          channel: string = REDIS_CHANNELS.API_KEY_CACHE_INVALIDATION
        ): void => {
    ...
    it.each([
      ...
      ['an unknown event type', JSON.stringify({ type: 'key' })],
