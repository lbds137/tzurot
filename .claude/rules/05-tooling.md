...
The monitor command (PR number is `N`, `<sha>` is the **full 40-character** SHA you just pushed — `actions/runs?head_sha=` does NOT accept an abbreviated SHA; it returns `total_count: 0`, which this gate reads as "keep waiting" and spins to the timeout without ever firing):

```bash
SHA=<sha>; until gh api "repos/{owner}/{repo}/actions/runs?head_sha=$SHA" --jq '[.workflow_runs[]|select(.name=="CI" and .status=="completed" and .conclusion!="startup_failure")]|length' | grep -qE '^[1-9]'; do sleep 30; done; gh pr checks N --watch --interval=30 > /dev/null 2>&1; sleep 5; echo "CI_COMPLETE"; gh pr checks N
```

**This command is duplicated in three places on purpose** — here, the hook heredoc, and `/tzurot-git-workflow` — because a pointer would hide it exactly where a reader needs to read it. `pnpm ops guard:monitor-command` is what makes that safe: it normalizes the placeholders (`<sha>`/`N` vs the hook's interpolated values) and fails CI if the three copies differ in anything else. So **change all three or CI stops you** — the hook is the reference copy, since it is the one actually emitted.
...
**Gate on the RUN, not a `sleep`, because run creation is what lags.** `gh pr checks --watch` snapshots the check list at start time — it does NOT wait for checks that register after polling begins, so if it starts early it watches only the fast-registering checks (GitGuardian passes in ~1s) and exits with everything else still pending. A fixed startup `sleep` was the previous mitigation and it does not hold: a workflow run can be **created minutes after the push lands** — measured at 4m14s on a normal docs push — so `sleep 60` and `sleep 120` both expired early on consecutive pushes and both emitted a `CI_COMPLETE` over a one-entry check list. No constant bounds a queue.

The `until` gate above waits for the `CI` workflow RUN to reach `completed`, which cannot fire early: a run that does not exist yet returns `0`, and the `head_sha` index lag documented below also returns nothing — both keep the loop waiting rather than releasing it. By the time CI (the slowest workflow, ~20 min) is done, every other check has long since registered, so the `--watch` handoff is finally operating on a full list. The trailing 5s lets state propagate before the final query.

**Two details in the predicate are load-bearing, not incidental.** `.conclusion!="startup_failure"` is what keeps the gate honest against the died-before-dispatch case documented two paragraphs below: such a run reaches `completed` **instantly with zero jobs**, so a naive `status=="completed"` test would release into `--watch` against a nearly-empty check list — reproducing the exact premature-`CI_COMPLETE` bug the gate replaced. Excluding it keeps the loop waiting (a genuine `failure` still releases), and the 30-min timeout is the backstop when no real run ever arrives. `grep -qE '^[1-9]'` accepts **any** positive count rather than exactly one: today no path produces two `CI` runs for one SHA (push-only trigger, branch-scoped `cancel-in-progress`, and `gh run rerun` reuses the run id), but an exact-match test would silently spin for the full timeout if that ever changed.

This is not the reinvented watch loop this rule warns against: **custom `until` loops parsing `gh pr checks --json bucket` are still wrong** — they race partial check lists that momentarily read "all non-pending." Gating on run status is a different signal that has no such intermediate state. Note the gate does not make the whole command SHA-pinned; the trailing `gh pr checks N` still reports the PR's current state, which is why one-monitor-per-PR above still holds.

If the `CI` workflow legitimately never runs for a PR, the loop will spin until `timeout_ms` — that timeout is the backstop and a re-arm signal, not a hang to muzzle.
...
**Timeout handling**: if the monitor output doesn't contain `CI_COMPLETE`, the 30-min `timeout_ms` fired before CI finished. Re-arm the monitor rather than assuming CI passed.
