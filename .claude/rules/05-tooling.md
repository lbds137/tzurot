...
**Merge mechanics.** The merge is the tail of a verify-then-merge `&&` chain, never a bare `gh pr merge`, so a failed verification halts before the destructive step. When the target branch moved under a long-lived feature branch, re-derive diffstats and counts against the MERGE-BASE, not the target's current HEAD.
