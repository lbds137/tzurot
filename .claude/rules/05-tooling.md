...
pnpm ops guard:workflow-sync         # .github/workflows/ must be byte-identical to origin/main
```

The first five run in the CI `lint` job; all guards hard-fail on findings (including `guard:boundaries` — its `--summary` mode is still pending (tracked in `backlog/cold/follow-ups.md`), but hard-fail is independent of `--summary`). `guard:workflow-sync` runs in `pnpm quality` AND `.husky/pre-push`; it skips itself on main-cut branches (detected by topology — no develop-exclusive history — with `--base main` as the explicit override), because main-cut workflow PRs are the sanctioned path. Its CI `lint`-job step is itself a workflow-file change and therefore rides the next main-cut PR rather than landing via develop. It exists because a workflow change reaching develop first silently disables claude-review on every PR (green ~15s no-op) until the next release. `guard:proposal-links` and `guard:audit-tool-docs` also support `--summary` for the future aggregator. `guard:audit-tool-docs` self-registers and runs the bidirectional check (every registered tool has a WHY.md AND every `*.WHY.md` is either registered or on `UNREGISTERED_WHY_PATHS`).
