...
**The promise ledger — file at the moment of utterance.** Any in-flight "I'll do X later / after this PR / when the release is done" — in chat, a plan, or a PR description — must land in a tracker task or the appropriate backlog file THE MOMENT it is said, not at session end — or, for low-priority residue of process work, in the PR body's `## Residue` section, the same exception as above. A promise that exists only in chat prose does not exist: it dies at the next compaction. The session-end gates below are the backstop, not the mechanism. The reply that files the task names its id back to the owner ("filed as TASK-N") — the ledger proves the task exists, and only the id in the reply stops the owner re-asking whether it does.
