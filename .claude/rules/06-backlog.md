...
- **An architecture/infrastructure premise stated in a description cites the file:line that verifies it** — a task filed on an unverified premise plants that premise in a durable surface (the same cite-the-read bar PR bodies carry; a task asserting "we route through OpenRouter" was filed while a whole z.ai-direct path existed).
...
- **State** (set at filing; exactly one) — the _reachability_ axis, answering "what has to happen before anyone can pick this up": `state:ready` (startable today) · `state:observable` (a signal arrives on its own — watches) · `state:dependent` (a named internal blocker) · `state:owner` (the owner's decision queue). Size says how big, priority says how much it matters, state says whether it can be started at all — none substitutes for another. The drain query: `pnpm tracker task list -s "To Do" -l state:ready -l size:S --priority high --plain`.
  - There is deliberately no state for "the only trigger is next time someone touches this" — that is the admission bar's **do it now** case, so a task in it should never have been filed. The value existed, collected 15 open items no query could reach, and was retired when the owner overrode all 15 at once. If an item genuinely has no reachable trigger, that is evidence it belongs in the diff that surfaced it, not in the pool.
...
- **Finishing**: `-s Done` at ship — **the digest excludes Done, `task list` does NOT**, so every selection query needs an explicit `-s "To Do"` (without it the drain query silently offers closed work as pickable). Archive during periodic sweeps. For **obsolete** or **ruled-out** exits, archive with the reason in the removing commit — same evidence bar as before.
