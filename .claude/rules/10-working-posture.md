...
Trigger: when unreleased **merged PRs that touch runtime** (`services/`,
`prisma/`, `.github/workflows/`, `package.json`) reach roughly ten, or the
...
commits**: rebase-only merging makes those diverge badly, and counting commits
once inflated the trigger by two thirds and produced a recommendation a council
overturned. A tooling- or docs-only batch dilutes nothing — the second look
